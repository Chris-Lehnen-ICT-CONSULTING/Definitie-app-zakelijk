...
class ExternalSourcesTab:
    ...
    def render(self):
        ...
        # Main interface
        tab1, tab2, tab3, tab4 = st.tabs(["📋 Bronnen", "🔍 Zoeken", "📥 Import", "⚙️ Configuratie"])
            ...
            for info in source_info:
                with st.expander(f"🔌 {info['source_name']}", expanded=False):
                    ...
                    with col1:
                        ...
                        st.write(f"**Status:** {status_color} {'Verbonden' if info['connected'] else 'Niet verbonden'}")
                    
                    with col2:
                        if st.button(f"🔍 Test", key=f"test_{info['source_id']}"):
                    ...
                    with col3:
                        if st.button(f"🗑️ Verwijder", key=f"remove_{info['source_id']}"):
        ...
        if st.button("🔍 Zoeken in Alle Bronnen", type="primary"):
            with st.spinner("Zoeken in externe bronnen..."):
                try:
                    ...
                    # Search all sources
                    results = manager.search_all_sources(**search_params, limit_per_source=20)
                    
                    # Store results
                    SessionStateManager.set_value("external_search_results", {
                        "results": results,
    ...
    def _display_search_results(self, results: Dict[str, List]):
        ...
        for source_id, source_results in results.items():
            if source_results:
                with st.expander(f"📁 {source_id} ({len(source_results)} resultaten)", expanded=True):
                    for i, ext_def in enumerate(source_results):
                        with st.container():
                            ...
                            with col1:
                                ...
                                st.markdown(f"*{ext_def.definitie[:100]}...*" if len(ext_def.definitie) > 100 else f"*{ext_def.definitie}*")
                                st.caption(f"Context: {ext_def.context} | Status: {ext_def.status}")
                            
                            with col2:
                                st.caption(f"Categorie: {ext_def.categorie}")
                                if ext_def.metadata:
                                    st.caption(f"Metadata: {len(ext_def.metadata)} items")
                            
                            with col3:
                                if st.button(f"📥 Import", key=f"import_{source_id}_{i}"):
    ...
    def _render_import_interface(self):
        ...
        with col1:
            ...
            selected_source = st.selectbox(
                "Selecteer Bron",
                options=[""] + [info['source_id'] for info in st.session_state.external_source_manager.get_source_info()],
        ...
        with col2:
            ...
            import_options = {
                ...
                "overwrite_existing": st.checkbox("🔄 Bestaande overschrijven", value=False)
            }
    ...
    def _render_import_history(self):
        ...
        if import_history:
            for i, entry in enumerate(import_history[-5:]):  # Show last 5
                with st.expander(f"📥 Import {i+1}: {entry.get('timestamp', 'Onbekend')}", expanded=False):
                    st.write(f"**Bron:** {entry.get('source_id', 'Onbekend')}")
                    st.write(f"**Geïmporteerd:** {entry.get('imported_count', 0)} definities")
    ...
    def _render_source_configuration(self):
        ...
        with st.expander("🔧 Handmatige Bron Configuratie", expanded=False):
            ...
            with col2:
                ...
                api_key = st.text_input("API Key (optioneel)", type="password", key="manual_api_key")
                timeout = st.number_input("Timeout (seconden)", min_value=1, max_value=300, value=30, key="manual_timeout")
            
            if st.button("➕ Configureer Bron"):
                if source_id and source_name and connection_string:
                    self._create_manual_source(
                        source_id, source_name, source_type, 
                        connection_string, api_key, timeout
                    )
                else:
    ...
    def _import_external_definition(self, ext_def, source_id: str):
        ...
        try:
            ...
            if existing and not options.get("overwrite_existing", False):
                st.warning(f"⚠️ Definitie '{ext_def.begrip}' bestaat al. Schakel 'Bestaande overschrijven' in om te vervangen.")
                return
            ...
            if existing and options.get("overwrite_existing", False):
                ...
                if new_id:
                    st.success(f"✅ Definitie '{ext_def.begrip}' geïmporteerd (ID: {new_id})")
    ...
    def _perform_bulk_import(self, source_id: str, limit: int):
        ...
        with st.spinner(f"Importeer tot {limit} definities van {source_id}..."):
            try:
                ...
                # Show results
                if imported_count > 0:
                    st.success(f"✅ {imported_count} van {len(ext_definitions)} definities geïmporteerd")
    ...
    def _create_manual_source(self, source_id: str, source_name: str, 
                             source_type: str, connection_string: str, 
        ...
        try:
            ...
            for source_info in sources:
                try:
                    ...
                    st.info(f"⚠️ Import van bron '{source_info['source_name']}' vereist handmatige herconfiguratie")
                    imported_count += 1
                
                except Exception as e:
                    errors.append(f"Fout bij {source_info.get('source_name', 'onbekend')}: {str(e)}")
