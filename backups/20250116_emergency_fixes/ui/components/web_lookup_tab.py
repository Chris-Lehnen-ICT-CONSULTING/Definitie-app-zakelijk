...
class WebLookupTab:
    ...
    def _init_lookup_modules(self):
        ...
        try:
            ...
            from bron_lookup import BronZoeker, herken_bronnen_in_definitie, valideer_definitie_bronnen
            from definitie_lookup import DefinitieZoeker, zoek_definitie, detecteer_duplicaten
    ...
    def render(self):
        ...
        # Main interface
        tab1, tab2, tab3, tab4 = st.tabs(["🔍 Definitie Zoeken", "📚 Bron Lookup", "⚖️ Juridische Lookup", "🔧 Validatie Tools"])
    ...
    def _render_definitie_lookup(self):
        ...
        with st.expander("🔧 Geavanceerde Zoek Opties", expanded=False):
            ...
            with col1:
                include_internal = st.checkbox("🏠 Interne database", value=True, key="include_internal")
                include_external = st.checkbox("🌐 Externe bronnen", value=True, key="include_external")
            ...
            with col3:
                zoek_gerelateerde = st.checkbox("🔗 Zoek gerelateerde", value=True, key="zoek_gerelateerde")
                ...
                with st.spinner("Zoeken naar definities..."):
                    try:
                        ...
                        resultaat = loop.run_until_complete(
                            self.zoek_definitie(zoek_term, max_resultaten=max_resultaten)
                        )
                        
                        # Store results
                        SessionStateManager.set_value("definitie_zoek_resultaat", {
                            "resultaat": resultaat,
    ...
    def _display_definitie_zoek_resultaten(self, resultaat):
        ...
        # Duplicaat analyse
        if resultaat.duplicaat_analyse and resultaat.duplicaat_analyse.get("gevonden_duplicaten", 0) > 0:
            with st.expander("⚠️ Mogelijke Duplicaten", expanded=False):
                for dup in resultaat.duplicaat_analyse["duplicaten"]:
                    st.warning(f"🔄 {dup['definitie1']} ↔ {dup['definitie2']} (gelijkenis: {dup['gelijkenis']:.2f})")
    ...
    def _render_definitie_item(self, definitie):
        ...
        with st.container():
            ...
            with col3:
                ...
                if st.button("📋 Kopieer", key=f"copy_{definitie.begrip}_{hash(definitie.bron)}"):
    ...
    def _render_bron_lookup(self):
        ...
        if "Zoek bronnen" in zoek_mode:
                ...
                if zoek_query:
                    with st.spinner("Zoeken naar bronnen..."):
                        try:
                            ...
                            resultaat = loop.run_until_complete(
                                bron_zoeker.zoek_bronnen(zoek_query, max_resultaten=max_bronnen)
                            )
                            
                            SessionStateManager.set_value("bron_zoek_resultaat", {
                                "resultaat": resultaat,
                                "timestamp": datetime.now().isoformat()
                            })
            ...
            if st.button("✅ Valideer Bronnen", type="primary", key="valideer_bronnen_btn"):
                if tekst_input:
                    with st.spinner("Analyseren van bronverwijzingen..."):
                        try:
                            # Herken bronnen in tekst
                            gevonden_bronnen = self.herken_bronnen_in_definitie(tekst_input)
                            ...
                            SessionStateManager.set_value("bron_validatie_resultaat", {
                                "bronnen": gevonden_bronnen,
                                "validatie": validatie_resultaat,
                                "timestamp": datetime.now().isoformat()
                            })
                            
                            self._display_bron_validatie_resultaten(gevonden_bronnen, validatie_resultaat)
        ...
        if "Zoek bronnen" in zoek_mode:
            ...
            if previous_results:
                ...
                self._display_bron_validatie_resultaten(
                    previous_results["bronnen"], 
                    previous_results["validatie"]
                )
    ...
    def _display_bron_zoek_resultaten(self, resultaat):
        ...
        with col3:
            if resultaat.gevonden_bronnen:
                avg_betrouwbaarheid = sum(b.betrouwbaarheid for b in resultaat.gevonden_bronnen) / len(resultaat.gevonden_bronnen)
    ...
    def _display_bron_validatie_resultaten(self, bronnen, validatie):
        ...
        with col4:
            st.metric("🔗 Toegankelijk", f"{validatie['toegankelijke_bronnen']}/{validatie['aantal_bronnen']}")
    ...
    def _render_juridische_lookup(self):
        ...
        if st.button("⚖️ Analyseer Juridische Verwijzingen", type="primary", key="juridische_analyse_btn"):
            if tekst_input:
                with st.spinner("Analyseren van juridische verwijzingen..."):
                    try:
                        ...
                        SessionStateManager.set_value("juridische_resultaten", {
        ...
        if resultaten:
            ...
            with st.expander("🔍 Gedetailleerde Verwijzingen", expanded=False):
                for i, result in enumerate(resultaten):
                    ...
                    with col2:
                        st.write(f"**Herkenning:** {result.get('herkend_via', 'Onbekend')}")
    ...
    def _render_validatie_tools(self):
        ...
        tool_type = st.selectbox(
            "Selecteer tool",
            ["🔄 Duplicaat Detectie", "📊 Definities Vergelijken", "🧹 Tekst Opschoning"],
    ...
    def _render_duplicaat_detectie(self):
        ...
        if st.button("🔄 Detecteer Duplicaten", type="primary", key="detecteer_dup_btn"):
            if begrip_input and definitie_input:
                with st.spinner("Zoeken naar mogelijke duplicaten..."):
                    try:
                        ...
                        resultaat = loop.run_until_complete(
                            self.detecteer_duplicaten(begrip_input, definitie_input, threshold)
                        )
    ...
    def _display_duplicaat_resultaten(self, resultaat):
        ...
        if resultaat["duplicaat_gevonden"]:
            st.warning(f"⚠️ {len(resultaat['mogelijke_duplicaten'])} mogelijke duplicaten gevonden!")
            
            for dup in resultaat["mogelijke_duplicaten"]:
                with st.expander(f"🔄 {dup['begrip']} (gelijkenis: {dup['gelijkenis']:.2f})", expanded=True):
                    col1, col2 = st.columns(2)
    ...
    def _render_tekst_opschoning(self):
        ...
        with st.expander("🔧 Opschoon Opties", expanded=True):
            ...
            with col1:
                remove_extra_spaces = st.checkbox("Remove extra spaces", value=True, key="remove_spaces")
                normalize_punctuation = st.checkbox("Normalize punctuation", value=True, key="norm_punct")
                fix_capitalization = st.checkbox("Fix capitalization", value=False, key="fix_caps")
            
            with col2:
                remove_line_breaks = st.checkbox("Remove line breaks", value=False, key="remove_breaks")
                standardize_quotes = st.checkbox("Standardize quotes", value=True, key="std_quotes")
                remove_extra_dots = st.checkbox("Remove extra dots", value=True, key="remove_dots")
            ...
            if tekst_input:
                try:
                    ...
                    with col1:
                        ...
                        st.text_area("Origineel", tekst_input, height=100, disabled=True, key="orig_display")
                    
                    with col2:
                        st.markdown("**Opgeschoonde tekst:**")
                        st.text_area("Opgeschoond", cleaned_text, height=100, key="cleaned_display")
                    
                    # Statistics
                    orig_length = len(tekst_input)
                    clean_length = len(cleaned_text)
                    reduction = ((orig_length - clean_length) / orig_length * 100) if orig_length > 0 else 0
