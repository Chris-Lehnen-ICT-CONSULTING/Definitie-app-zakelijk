...
class HistoryTab:
    ...
    def _render_filter_controls(self):
        ...
        with col1:
            ...
            date_range = st.selectbox(
                "Tijdsperiode",
                ["Laatste week", "Laatste maand", "Laatste 3 maanden", "Alle tijd", "Aangepast..."],
        ...
        SessionStateManager.set_value("history_filters", {
    ...
    def _render_history_overview(self):
        ...
        try:
            ...
            with col2:
                established = len([d for d in filtered_definitions if d.status == DefinitieStatus.ESTABLISHED.value])
                st.metric("✅ Vastgesteld", established)
            
            with col3:
                in_review = len([d for d in filtered_definitions if d.status == DefinitieStatus.REVIEW.value])
                st.metric("🔄 In Review", in_review)
            
            with col4:
                if filtered_definitions:
                    avg_score = sum(d.validation_score for d in filtered_definitions if d.validation_score) / len([d for d in filtered_definitions if d.validation_score])
    ...
    def _render_detailed_history(self):
        ...
        try:
            ...
            # Pagination
            items_per_page = 10
            total_pages = (len(filtered_definitions) + items_per_page - 1) // items_per_page
            ...
            # Pagination info
            if total_pages > 1:
                st.caption(f"Pagina {page + 1} van {total_pages} | Toon {len(page_definitions)} van {len(filtered_definitions)} definities")
    ...
    def _render_history_item(self, definitie: DefinitieRecord):
        ...
        with st.container():
            ...
            with col1:
                ...
                definitie_preview = definitie.definitie[:100] + "..." if len(definitie.definitie) > 100 else definitie.definitie
            ...
            with col3:
                ...
                if definitie.created_at:
                    st.caption(f"📅 Gemaakt:")
                    st.caption(definitie.created_at.strftime("%Y-%m-%d %H:%M"))
                
                if definitie.updated_at and definitie.updated_at != definitie.created_at:
                    st.caption(f"🔄 Gewijzigd:")
                    st.caption(definitie.updated_at.strftime("%Y-%m-%d %H:%M"))
            
            with col4:
                # Actions
                if st.button(f"👁️", key=f"view_{definitie.id}", help="Bekijk details"):
                    self._show_definition_details(definitie)
                
                if st.button(f"📊", key=f"history_{definitie.id}", help="Bekijk historie"):
    ...
    def _render_timeline_chart(self, definitions: List[DefinitieRecord]):
        ...
        try:
            ...
            if df_data:
                ...
                timeline_data = df.groupby(["datum", "status"]).size().reset_index(name="count")
                
                # Create timeline chart
                if not timeline_data.empty:
                    chart_data = timeline_data.pivot(index="datum", columns="status", values="count").fillna(0)
        ...
        with st.expander("📊 Gedetailleerde Statistieken", expanded=False):
            try:
                ...
                if stats.get('average_validation_score'):
                    st.markdown(f"**Gemiddelde Kwaliteitsscore:** {stats['average_validation_score']}")
        ...
        with st.expander(f"📋 Details: {definitie.begrip}", expanded=True):
            ...
            with col2:
                ...
                if definitie.approved_by:
                    ...
                    st.write(f"**Goedgekeurd op:** {definitie.approved_at.strftime('%Y-%m-%d %H:%M') if definitie.approved_at else 'Onbekend'}")
    
    def _show_audit_trail(self, definitie: DefinitieRecord):
        """Toon audit trail voor definitie.""" 
        st.info(f"🔍 Audit trail voor '{definitie.begrip}' (ID: {definitie.id}) - Feature komt binnenkort beschikbaar")
        # TODO: Implement audit trail query from definitie_geschiedenis table
    
    def _get_filtered_definitions(self, filters: Dict[str, Any]) -> List[DefinitieRecord]:
