...
class MonitoringTab:
    ...
    def render(self):
        ...
        # Main interface
        tab1, tab2, tab3, tab4 = st.tabs(["🔴 Real-time", "📊 Metrics", "💰 Kosten", "⚠️ Alerts"])
        ...
        try:
            ...
            if metrics['active_alerts']:
                ...
                for alert in metrics['active_alerts']:
                    ...
                    st.warning(f"{severity_color} **{alert['name']}** - Laatste trigger: {alert['last_triggered']} (Count: {alert['trigger_count']})")
    ...
    def _render_metrics_dashboard(self):
        ...
        try:
            ...
            if time_range == "Laatste uur":
                ...
                time_points = [now - timedelta(minutes=30*i) for i in range(12, 0, -1)]
            else:
        ...
        try:
            ...
            if st.button("📊 Generate Cost Report", type="primary"):
                with st.spinner("Genereer cost optimization rapport..."):
                    ...
                    if 'error' in report:
                        ...
                        # Store report
                        SessionStateManager.set_value("cost_report", {
                            "data": report,
            ...
            if cost_report_data:
                ...
                with col3:
                    st.metric(
                        "📞 Total Calls",
                        report.get('total_calls', 0)
                    )
                
                with col4:
                    avg_cost_per_call = report.get('total_cost', 0) / max(report.get('total_calls', 1), 1)
                    st.metric(
                        "💸 Avg Cost/Call",
                        f"${avg_cost_per_call:.6f}"
                    )
                ...
                with col2:
                    ...
                    if wasted_cost > 0:
                        waste_percentage = (wasted_cost / report.get('total_cost', 1)) * 100
                ...
                if recommendations:
                    ...
                    for i, rec in enumerate(recommendations):
                        ...
                        with st.expander(f"{priority_color} {rec.get('type', 'Unknown').title()} - {rec.get('priority', 'low').title()} Priority", expanded=i==0):
            ...
            with col2:
                if st.button("💰 Calculate Monthly Cost"):
                    monthly_cost = self.CostCalculator.estimate_monthly_cost(daily_requests, avg_tokens)
                    st.success(f"📊 Estimated Monthly Cost: ${monthly_cost:.2f}")
                    
                    # Show breakdown
                    st.info(f"📈 Daily cost: ${monthly_cost/30:.3f}")
                    st.info(f"📉 Cost per request: ${monthly_cost/(daily_requests*30):.6f}")
                ...
                with st.expander(f"⚠️ {alert.name}", expanded=False):
                    ...
                    with col3:
                        ...
                        if alert.last_triggered:
                            st.write(f"**Last:** {alert.last_triggered.strftime('%H:%M:%S')}")
    ...
    def simulate_api_call(self):
        ...
        try:
            ...
            # Simulate API call
            asyncio.run(self.record_api_call(
                endpoint="simulation",
