...
@pytest.mark.parametrize('module_path', sorted(p for p in VALIDATORS_DIR.glob('*.py') if p.name != '__init__.py'))
def test_validator_modules_expose_validate(module_path):
    ...
    for cls in classes:
        ...
        assert params and params[0].name in {'config', 'cfg'}, f"{cls.__name__} should accept config dict"
