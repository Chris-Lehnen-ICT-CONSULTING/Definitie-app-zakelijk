...
async def test_arai02_container_without_specificity_fails_and_with_specificity_passes():
    ...
    assert any(v.get("code") == "ARAI-02" for v in res_bad.get("violations", [])), res_bad
    ...
    assert not any(v.get("code") == "ARAI-02" for v in res_ok.get("violations", [])), res_ok
...
async def test_str01_body_start_detects_verb_via_arai01():
    ...
    assert any(v.get("code") == "ARAI-01" for v in res_bad.get("violations", [])), res_bad
    ...
    assert not any(v.get("code") == "ARAI-01" for v in res_ok.get("violations", [])), res_ok
