...
class PerformanceMonitor:
    ...
    def get_summary(self):
        ...
        return {
            ...
            'avg_operation_time': sum(m['duration'] for m in self.measurements) / len(self.measurements) if self.measurements else 0,
            'slowest_operation': max(self.measurements, key=lambda x: x['duration']) if self.measurements else None,
            'memory_intensive_operation': max(self.measurements, key=lambda x: x['memory_delta']) if self.measurements else None
        }
...
class TestAIToetserPerformance:
    ...
    def test_single_validation_performance(self):
        ...
        assert perf_result['duration'] < 0.5, f"Single validation too slow: {perf_result['duration']:.3f}s"
        assert perf_result['memory_delta'] < 10 * 1024 * 1024, f"Memory usage too high: {perf_result['memory_delta'] / 1024 / 1024:.2f}MB"
    ...
    def test_batch_validation_performance(self):
        ...
        assert perf_result['duration'] < 10.0, f"Batch validation too slow: {perf_result['duration']:.3f}s"
        
        # Calculate throughput
        throughput = len(definitions) / perf_result['duration']
        assert throughput > 5, f"Validation throughput too low: {throughput:.2f} def/sec"
    ...
    def test_concurrent_validation_performance(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(validate_worker, def_text) for def_text in definitions]
        ...
        assert all(r is not None for r in results)
        assert perf_result['duration'] < 5.0, f"Concurrent validation too slow: {perf_result['duration']:.3f}s"
    ...
    def test_memory_usage_under_load(self):
        ...
        for i in range(100):
            ...
            if i % 20 == 0:
                ...
                assert memory_growth < 50 * 1024 * 1024, f"Memory growth too high: {memory_growth / 1024 / 1024:.2f}MB at iteration {i}"
...
class TestCachePerformance:
    ...
    def test_cache_hit_performance(self):
        ...
        assert hit_perf['duration'] < miss_perf['duration'] * 0.1, "Cache hit not significantly faster than miss"
        assert hit_perf['duration'] < 0.001, f"Cache hit too slow: {hit_perf['duration']:.6f}s"
    ...
    def test_cache_scalability(self):
        ...
        # Assertions
        assert perf_result['duration'] < 2.0, f"Cache scalability test too slow: {perf_result['duration']:.3f}s"
    ...
    def test_cache_memory_efficiency(self):
        ...
        assert memory_usage < 50 * 1024 * 1024, f"Cache memory usage too high: {memory_usage / 1024 / 1024:.2f}MB"
...
class TestConfigurationPerformance:
    ...
    def test_config_loading_performance(self):
        ...
        # Assertions
        assert perf_result['duration'] < 1.0, f"Config loading too slow: {perf_result['duration']:.3f}s"
        
        # Calculate loading rate
        loading_rate = 50 / perf_result['duration']
        assert loading_rate > 50, f"Config loading rate too low: {loading_rate:.2f} loads/sec"
    ...
    def test_toetsregels_loading_performance(self):
        ...
        # Assertions
        assert perf_result['duration'] < 2.0, f"Toetsregels loading too slow: {perf_result['duration']:.3f}s"
...
class TestSanitizationPerformance:
    ...
    def test_sanitization_speed(self):
        ...
        assert perf_result['duration'] < 1.0, f"Sanitization too slow: {perf_result['duration']:.3f}s"
        
        # Calculate sanitization rate
        sanitization_rate = 100 / perf_result['duration']
        assert sanitization_rate > 100, f"Sanitization rate too low: {sanitization_rate:.2f} ops/sec"
    ...
    def test_large_content_sanitization(self):
        ...
        assert result is not None
        assert len(result) > 0
        assert perf_result['duration'] < 0.5, f"Large content sanitization too slow: {perf_result['duration']:.3f}s"
...
class TestDocumentProcessingPerformance:
    ...
    def test_text_extraction_performance(self):
        ...
        assert all(r is not None for r in results)
        assert perf_result['duration'] < 1.0, f"Text extraction too slow: {perf_result['duration']:.3f}s"
    
    def test_concurrent_document_processing(self):
        """Test concurrent document processing performance."""
        test_documents = [
            (f"Document content {i} " * 50).encode('utf-8')
            for i in range(20)
        ]
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(process_document, content) for content in test_documents]
            results = [future.result() for future in futures]
        ...
        assert len(results) == 20
        assert all(r is not None for r in results)
        assert perf_result['duration'] < 2.0, f"Concurrent document processing too slow: {perf_result['duration']:.3f}s"


@pytest.mark.skipif(not RESILIENCE_AVAILABLE, reason="Resilience components not available")
class TestResiliencePerformance:
    ...
    def test_circuit_breaker_performance(self):
        ...
        assert perf_result['duration'] < 1.0, f"Circuit breaker overhead too high: {perf_result['duration']:.3f}s"
    ...
    def test_rate_limiter_performance(self):
        ...
        assert perf_result['duration'] < 0.1, f"Rate limiter too slow: {perf_result['duration']:.3f}s"
...
class TestSystemPerformanceIntegration:
    ...
    def test_end_to_end_performance(self):
        ...
        assert perf_result['duration'] < 1.0, f"End-to-end workflow too slow: {perf_result['duration']:.3f}s"
    ...
    def test_sustained_load_performance(self):
        ...
        while time.time() - start_time < 30 and operations < 1000:
            definition = f"Load test definitie {operations} voor sustained performance testing."
        ...
        assert throughput >= 10, f"Sustained throughput too low: {throughput:.2f} ops/sec"
        
        # Memory should not grow excessively
        assert perf_result['memory_delta'] < 100 * 1024 * 1024, f"Memory growth too high: {perf_result['memory_delta'] / 1024 / 1024:.2f}MB"
...
class TestPerformanceRegression:
    ...
    def test_performance_baselines(self):
        ...
        assert result is not None
        assert validation_time < self.baselines['single_validation'], \
        ...
        assert config_manager is not None
        assert config_time < self.baselines['config_loading'], \
        ...
        assert extraction_time < self.baselines['text_extraction'], \
        ...
        assert sanitization_time < self.baselines['sanitization'], \
