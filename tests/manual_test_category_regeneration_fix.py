...
def test_category_change_flow():
    ...
    if SessionStateManager.get_value("last_generation_result"):
        ...
        st.info(
            f"**Huidige categorie**: {current_result.get('determined_category', 'onbekend')}"
        )
        ...
        with st.expander("🐛 Debug Info"):
            ...
            st.write(
                f"- show_category_selector: {SessionStateManager.get_value('show_category_selector', False)}"
            )
