...
class TestAIToetserSystem:
    ...
    def test_validation_with_context(self):
        ...
        result = self.toetser.validate_definition(
            definition, self.toetsregels, begrip="test", contexten=context
        )
        assert result is not None
        assert isinstance(result, list)
...
class TestSystemIntegration:
    ...
    def test_end_to_end_definition_validation(self):
        ...
        result_with_context = toetser.validate_definition(
            test_definition, toetsregels, begrip="autorisatie", contexten=context
        )
...
class TestPerformanceBaseline:
    ...
    def test_validation_performance_baseline(self):
        ...
        test_definitions = [
            f"Test definitie nummer {i} voor performance testing." for i in range(50)
        ]
        ...
        ), f"Validation too slow: {validation_time:.2f}s for 50 definitions"

        # Calculate rate
        validation_rate = len(test_definitions) / validation_time
        assert (
            validation_rate > 5
        ), f"Validation rate too low: {validation_rate:.2f} def/sec"
