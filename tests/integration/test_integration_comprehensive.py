...
class TestBasicWorkflowIntegration:
    ...
    def test_complete_definition_generation_workflow(self):
        ...
        def scenario():
            ...
            validation_results = self.suite.toetser.validate_definition(test_definition, toetsregels)
            components.append('modular_toetser')
            data_flow.append('definition -> validation_results')
            
            # Step 4: Mock definition generation
            with patch('generation.definitie_generator.generate_definition') as mock_gen:
            ...
            return {
                ...
                'validation_passed': len(validation_results) >= 0  # Has validation results
            }
        ...
        assert result.duration < 2.0, f"Complete workflow too slow: {result.duration:.3f}s"
    ...
    def test_document_upload_integration_workflow(self):
        ...
        def scenario():
            ...
            extracted_text = extract_text_from_file(
                test_document_content.encode('utf-8'),
                'test_wet.txt',
                'text/plain'
            )
            ...
            with patch('document_processing.document_processor.DocumentProcessor') as mock_processor:
        ...
        assert result.duration < 1.5, f"Document workflow too slow: {result.duration:.3f}s"
    ...
    def test_hybrid_context_integration_workflow(self):
        ...
        def scenario():
            ...
            with patch('generation.definitie_generator.create_hybrid_generation_context') as mock_hybrid_gen:
        ...
        assert result.duration < 1.0, f"Hybrid context workflow too slow: {result.duration:.3f}s"
...
class TestErrorHandlingIntegration:
    ...
    def test_cascading_error_handling(self):
        ...
        def scenario():
            ...
            return {
                'success': len(errors) < 3,  # Some errors expected but system should handle gracefully
                'components': components,
                'data_flow': data_flow,
                'errors': errors
            }
        ...
        assert 'sanitizer' in result.components_tested  # Basic components should still work
    ...
    def test_recovery_mechanisms(self):
        ...
        assert result.duration < 0.5, f"Recovery mechanisms too slow: {result.duration:.3f}s"
    ...
    def test_end_to_end_performance_benchmark(self):
        ...
        def scenario():
            ...
            for i in range(10):  # 10 iterations
                ...
                validation = self.suite.toetser.validate_definition(f"Test definitie {i}", self.suite.toetsregels)
        ...
        assert result.success is True
        assert result.duration < 2.0, f"Performance benchmark too slow: {result.duration:.3f}s"
    ...
    def test_concurrent_workflow_performance(self):
        ...
        def scenario():
            ...
            def worker_workflow(worker_id):
                ...
                sanitized = self.suite.sanitizer.sanitize_user_input({'begrip': f'worker_{worker_id}'})
                validation = self.suite.toetser.validate_definition(f"Worker {worker_id} definitie", self.suite.toetsregels)
        ...
        assert result.success is True
        assert result.duration < 3.0, f"Concurrent performance too slow: {result.duration:.3f}s"
...
class TestDataFlowIntegration:
    ...
    def test_data_transformation_pipeline(self):
        ...
        def scenario():
            ...
            validation_results = self.suite.toetser.validate_definition(sanitized, self.suite.toetsregels)
            ...
            with patch('generation.definitie_generator.enrich_definition') as mock_enrich:
            ...
            data_integrity_check = all(
                t[1] is not None for t in transformations
            )
    ...
    def test_cross_component_state_management(self):
        ...
        def scenario():
            ...
            sanitized = self.suite.sanitizer.sanitize_user_input(workflow_state['data']['user_input'])
            ...
            validation = self.suite.toetser.validate_definition("Test", self.suite.toetsregels)
            ...
            return {
                ...
                'state_consistency': all(k in workflow_state['data'] for k in ['user_input', 'sanitized', 'validation'])
            }
        
        result = self.suite.run_scenario("state_management", scenario)
        
        assert result.success is True
        assert result.duration < 0.3, f"State management too slow: {result.duration:.3f}s"
...
class TestAsyncIntegration:
    ...
    async def test_async_workflow_integration(self):
        ...
        async def async_scenario():
            ...
            validate_task = async_validate("Async test definition", self.suite.toetsregels)
...
class TestRealWorldScenarios:
    ...
    def test_dutch_government_terminology_scenario(self):
        ...
        def scenario():
            ...
            for term in government_terms:
                ...
                validation = self.suite.toetser.validate_definition(definition, self.suite.toetsregels)
        ...
        assert result.success is True
        assert result.duration < 3.0, f"Government terminology scenario too slow: {result.duration:.3f}s"
    ...
    def test_bulk_processing_scenario(self):
        ...
        def scenario():
            ...
            bulk_data = [
                {'begrip': f'begrip_{i}', 'definitie': f'Definitie nummer {i} voor bulk processing.'}
                for i in range(50)
            ]
            ...
            for i in range(0, len(bulk_data), batch_size):
                ...
                for item in batch:
                    ...
                    validation = self.suite.toetser.validate_definition(item['definitie'], self.suite.toetsregels)
                    processed_count += 1
        ...
        assert result.success is True
        assert result.duration < 5.0, f"Bulk processing too slow: {result.duration:.3f}s"
