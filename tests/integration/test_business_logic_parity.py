...
class TestBusinessLogicParity:
    ...
    async def test_database_error_handling(
        ...
        mock_instance = Mock()
        mock_instance.save_definition.side_effect = Exception("Database connection failed")
    ...
    async def test_concurrent_request_handling(
        ...
        async def make_request(service, begrip):
            with patch('prompt_builder.stuur_prompt_naar_gpt', return_value=f"Def voor {begrip}"):
        ...
        legacy_success_count = sum(1 for r in legacy_results
                                  if isinstance(r, dict) and r.get('success'))
        new_success_count = sum(1 for r in new_results
                               if isinstance(r, dict) and r.get('success'))
