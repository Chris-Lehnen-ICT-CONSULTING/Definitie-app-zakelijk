...
try:
    from security.security_middleware import (
        SecurityMiddleware, ValidationRequest, ValidationResponse,
        get_security_middleware, security_middleware_decorator
    )
...
class AsyncSecurityTestHarness:
    ...
    async def create_test_request(self, endpoint="test", data=None, source_ip="127.0.0.1", malicious=False):
...
class TestAsyncSecurityMiddleware:
    ...
    async def test_async_request_validation_performance(self):
        ...
        assert perf_result['duration'] < 0.1, f"Async validation too slow: {perf_result['duration']:.3f}s"
    ...
    async def test_concurrent_request_validation(self):
        ...
        tasks = [
            self.harness.middleware.validate_request(req) 
            for req in requests
        ]
        ...
        assert len(successful_results) >= 18, f"Too many failed validations: {len(successful_results)}/20"
        assert total_time < 2.0, f"Concurrent validation too slow: {total_time:.3f}s"
        
        # Calculate throughput
        throughput = len(successful_results) / total_time
        assert throughput > 10, f"Validation throughput too low: {throughput:.2f} req/sec"
    ...
    async def test_async_security_pipeline(self):
        ...
        assert pipeline_time < 0.5, f"Async security pipeline too slow: {pipeline_time:.3f}s"
...
class TestAsyncThreatDetection:
    ...
    async def test_concurrent_threat_detection(self):
        ...
        async def detect_threats(scenario):
            request = await self.harness.create_test_request(
                data=scenario["data"],
                malicious=(scenario["type"] != "normal")
            )
            ...
            if hasattr(self.harness.middleware, 'validate_request'):
                ...
                return {
                    ...
                    'result': result,
                    'detected': hasattr(result, 'threats_detected') and len(getattr(result, 'threats_detected', [])) > 0
                }
            else:
        ...
        assert detection_time < 1.0, f"Concurrent threat detection too slow: {detection_time:.3f}s"
    ...
    async def test_adaptive_threat_response(self):
        ...
        for threat_level in threat_levels:
            ...
            async def process_threat_level():
                ...
                for i in range(threat_level["requests"]):
                    is_malicious = (i / threat_level["requests"]) < threat_level["threat_probability"]
        ...
        for level_result in all_results:
            assert level_result['processing_time'] < 3.0, f"Threat level {level_result['level']} too slow: {level_result['processing_time']:.3f}s"
            
            # Should handle all requests without crashing
            successful_count = len([r for r in level_result['results'] if not isinstance(r, Exception)])
            assert successful_count >= level_result['requests_count'] * 0.8, f"Too many failures in {level_result['level']} threat level"
...
class TestAsyncRateLimiting:
    ...
    async def test_distributed_rate_limiting(self):
        ...
        successful_requests = [r for r in all_request_results if not isinstance(r, Exception)]
        
        # Assertions
        total_requests = len(ip_sources) * requests_per_ip
        assert len(all_request_results) == total_requests
        assert len(successful_requests) >= total_requests * 0.8, f"Too many rate limited: {len(successful_requests)}/{total_requests}"
        assert total_time < 5.0, f"Distributed rate limiting too slow: {total_time:.3f}s"
    ...
    async def test_burst_protection(self):
        ...
        if len(successful_burst) < burst_size:
            ...
            assert len(successful_burst) < burst_size * 0.8, "Rate limiting not effective against burst"
...
class TestAsyncSecurityIntegration:
    ...
    async def test_async_security_with_caching(self):
        ...
        assert second_call_time < first_call_time * 0.5, "Cache not providing performance benefit"
    ...
    async def test_async_security_error_handling(self):
        ...
        error_scenarios = [
            ...
            {"name": "service_unavailable", "exception": ConnectionError("Service unavailable")},
        ...
        assert error_handling_time < 2.0, f"Error handling too slow: {error_handling_time:.3f}s"
        
        # Check that system handled errors gracefully
        successful_scenarios = [r for r in scenario_results if not isinstance(r, Exception)]
        assert len(successful_scenarios) >= len(error_scenarios) * 0.7, "Too many unhandled errors"
...
class TestAsyncSecurityPerformance:
    ...
    async def test_high_concurrency_security(self):
        ...
        assert len(all_requests) >= total_expected_requests * 0.8, f"Lost requests under load: {len(all_requests)}/{total_expected_requests}"
        assert len(successful_requests) >= len(all_requests) * 0.8, f"Too many failed requests: {len(successful_requests)}/{len(all_requests)}"
        assert total_time < 10.0, f"High concurrency test too slow: {total_time:.3f}s"
        
        # Calculate throughput
        throughput = len(successful_requests) / total_time
        assert throughput > 50, f"Throughput too low under load: {throughput:.2f} req/sec"
    ...
    async def test_security_latency_distribution(self):
        ...
        valid_latencies = [l for l in latencies if not isinstance(l, Exception) and isinstance(l, (int, float))]
        ...
        if valid_latencies:
            ...
            # Assertions
            assert len(valid_latencies) >= num_requests * 0.9, f"Too many failed latency measurements: {len(valid_latencies)}/{num_requests}"
            assert avg_latency < 0.1, f"Average latency too high: {avg_latency:.3f}s"
            assert p95_latency < 0.2, f"95th percentile latency too high: {p95_latency:.3f}s"
            assert p99_latency < 0.5, f"99th percentile latency too high: {p99_latency:.3f}s"
...
class TestAsyncSecurityDecorator:
    ...
    async def test_async_security_decorator(self):
        ...
        def async_security_decorator(endpoint_name=""):
            def decorator(func):
                async def wrapper(*args, **kwargs):
                    ...
                    # Validate
                    validation_result = await self.harness.middleware.validate_request(request)
                    
                    # If validation fails, raise error
                    if hasattr(validation_result, 'allowed') and not validation_result.allowed:
        ...
        assert execution_time < 0.1, f"Decorated function too slow: {execution_time:.3f}s"
