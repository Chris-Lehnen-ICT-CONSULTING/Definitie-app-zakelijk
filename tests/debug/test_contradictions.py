...
def load_validator_module(rule_name):
    ...
    module_path = Path(__file__).parent.parent.parent / f"src/toetsregels/regels/{rule_name}.py"
...
def init_validators():
    ...
    # STR-01
    str01_path = Path(__file__).parent.parent.parent / "src/toetsregels/regels/STR-01.json"
    with open(str01_path, 'r', encoding='utf-8') as f:
    ...
    # ESS-02
    ess02_path = Path(__file__).parent.parent.parent / "src/toetsregels/regels/ESS-02.json"
    with open(ess02_path, 'r', encoding='utf-8') as f:
    ...
    # ARAI-02
    arai02_path = Path(__file__).parent.parent.parent / "src/toetsregels/regels/ARAI-02.json"
    with open(arai02_path, 'r', encoding='utf-8') as f:
    ...
    for definitie, begrip in test_cases:
        ...
        if "is een activiteit" in definitie:
            if not str01_result[0] and ess02_result[0]:
                print(f"  ⚠️ CONTRADICTION CONFIRMED: STR-01 blocks 'is', ESS-02 wants it")
            else:
                print(f"  ✅ No contradiction: Both accept or both reject")
...
def test_contradiction_2():
    ...
    test_cases = [
        ...
        ("proces dat gegevens verzamelt", "dataverzameling"),  # "proces" with specification
        ("activiteit waarbij gegevens worden verzameld", "observatie"),  # ESS-02 preferred
    ...
    for definitie, begrip in test_cases:
        ...
        if "proces" in definitie and not arai02_result[0]:
            print(f"  ⚠️ ARAI-02 blocks 'proces' (as claimed)")
        if "activiteit waarbij" in definitie and ess02_result[0]:
            print(f"  ⚠️ ESS-02 accepts 'activiteit waarbij' (as claimed)")
...
def test_contradiction_3():
    ...
    error_prev_path = Path(__file__).parent.parent.parent / "src/services/prompts/modules/error_prevention_module.py"
    with open(error_prev_path, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    validators = init_validators()
    ess02_config_path = Path(__file__).parent.parent.parent / "src/toetsregels/regels/ESS-02.json"
    with open(ess02_config_path, 'r', encoding='utf-8') as f:
        ess02_config = json.load(f)
    ...
    for example in ess02_config.get("goede_voorbeelden_proces", []):
        ...
        if "waarbij" in example:
            print(f"    ⚠️ CONTRADICTION: Uses 'waarbij' despite ErrorPrevention")
        if "die" in example:
            print(f"    ⚠️ CONTRADICTION: Uses 'die' despite ErrorPrevention")
...
def test_contradiction_4():
    ...
    print(f"  Line 146: {lines[145].strip()}")  # "Begin niet met lidwoorden ('de', 'het', 'een')"
        ...
        for example in ess02_config.get(key, []):
            if "een" in example:
                ...
                if example.startswith("is een"):
                    print(f"    ⚠️ STARTS with 'is een' - violates both STR-01 and ErrorPrevention!")
