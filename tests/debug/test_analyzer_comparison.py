...
async def compare_analyzers():
    ...
    for begrip in test_begrippen:
        ...
        print(
            f"   Full:   {full_cat.value:12} - confidence: {confidence:.2f} ({elapsed:.3f}s)"
        )
    ...
    print(
        f"Overeenstemming: {len(agreements)} ({len(agreements)/len(test_begrippen)*100:.1f}%)"
    )
    print(
        f"Verschillen: {len(differences)} ({len(differences)/len(test_begrippen)*100:.1f}%)"
    )
    ...
    if differences:
        ...
        for diff in differences:
            print(
                f"   • {diff['begrip']:15} Quick={diff['quick']:10} Full={diff['full']:10} (conf={diff['confidence']:.2f})"
            )
            if diff["scores"]:
