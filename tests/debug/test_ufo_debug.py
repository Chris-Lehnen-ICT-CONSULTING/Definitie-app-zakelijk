...
def test_input_validation():
    ...
    print(f"  Empty term result: {result.primary_category}, confidence: {result.confidence}")

    # Test None
    print("\n2. None input test:")
    try:
        result = classifier.classify(None, "definition")
        print(f"  None term result: {result.primary_category}, confidence: {result.confidence}")
    ...
    try:
        ...
        print(f"  Number term result: {result.primary_category}, confidence: {result.confidence}")
    except Exception as e:
    ...
    print(f"  Long input truncated to: term={len(result.term)}, def={len(result.definition)}")
    print(f"  MAX_TEXT_LENGTH constant: {classifier._normalize_text('x' * 15000) == 'x' * 10000}")
...
def test_division_by_zero():
    ...
    print(f"No-match result: {result.primary_category}, confidence: {result.confidence}")
...
def test_memory_reference():
    ...
    print(f"Compiled patterns are instance variable: {id(classifier.compiled_patterns)}")
    classifier2 = UFOClassifierService()
    print(f"Second instance patterns ID: {id(classifier2.compiled_patterns)}")
    print(f"Patterns are shared: {id(classifier.compiled_patterns) == id(classifier2.compiled_patterns)}")
...
def test_pattern_matching():
    ...
    for term, definition, expected in test_cases:
        ...
        print(f"  {term}: Expected={expected.value}, Got={result.primary_category.value}, Match={match}")
