...
class TestLegacyVsNewParity:
    ...
    def new_service(self):
        ...
        with patch.dict('os.environ', {'USE_NEW_SERVICES': 'true'}):
            ...
            assert type(service).__name__ == 'ServiceAdapter', f"Should return ServiceAdapter, got {type(service).__name__}"
            return service
...
Voorbeelden van authenticatiemechanismen zijn wachtwoorden, biometrie, tokens en certificaten."""
    ...
    async def test_validation_consistency(
        ...
        assert 'toetsresultaten' in legacy_result or 'validation_errors' in legacy_result
    ...
    @pytest.mark.asyncio
    async def test_response_structure_compatibility(
        self,
        legacy_service,
        new_service
    ):
    ...
    async def test_monitoring_compatibility(
        ...
        with patch('config.Config.monitoring_enabled', True), \
