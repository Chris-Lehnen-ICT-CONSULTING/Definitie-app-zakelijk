...
class TestUIPreviewRejection:
    ...
    def test_prompt_service_blocks_emoji_contamination(self):
        ...
        request = GenerationRequest(
            begrip="test",
            organisatorische_context=["📋 OM", "DJI"]  # Emoji in data!
        )
    ...
    def test_context_formatter_enforces_separation(self):
        ...
        assert ui_preview.startswith("[UI_ONLY]"), "UI preview not marked as display-only"

        # AND: Trying to use it as input should fail
        with pytest.raises(ValueError, match="UI preview cannot be used as data source"):
