...
class TestPercentageRollout:
    ...
    def test_percentage_rollout_distribution(self, feature_flags):
        ...
        assert (
            28 < percentage < 32
        ), f"Rollout percentage {percentage}% not close to 30%"
    ...
    def test_gradual_rollout_increase(self, feature_flags):
        ...
        for percentage in percentages:
            ...
            assert previous_enabled.issubset(
                currently_enabled
...
class TestABTesting:
    ...
    def test_ab_test_groups(self):
        ...
        # Configure A/B test
        flags.configure_ab_test(
            "context_flow_experiment",
            {
...
class TestFlagOverrides:
    ...
    def test_organization_override(self):
        ...
        # Check with context
        request_dji = GenerationRequest(begrip="test", organisatorische_context=["DJI"])
...
class TestMultiFlagInteractions:
    ...
    def test_mutually_exclusive_flags(self):
        ...
        assert not (
            flags.is_enabled("legacy_context_flow")
            and flags.is_enabled("modern_context_flow")
        )
    ...
    def test_flag_combinations(self):
        ...
        valid_combinations = [
            {
                "modern_context_flow": True,
                "anders_option_fix": True,
                "legacy_route_removal": True,
            },
            {
                "modern_context_flow": False,
                "anders_option_fix": False,
                "legacy_route_removal": False,
            },
            {
                "modern_context_flow": True,
                "anders_option_fix": False,
...
class TestFlagMonitoring:
    ...
    def test_flag_conversion_tracking(self):
        ...
        flags.track_conversion(
            "modern_context_flow", "definition_generated", success=True
        )
        flags.track_conversion(
            "modern_context_flow", "definition_generated", success=False
        )
