...
from security.security_middleware import (
    ...
    SecurityLevel, ThreatType, get_security_middleware,
    security_middleware_decorator
)
from validation.sanitizer import (
    ContentSanitizer, ContentType, SanitizationLevel,
    get_sanitizer, sanitize_content, sanitize_for_definition,
    sanitize_user_input, detect_threats
)
...
class TestInputValidation:
    ...
    def test_definition_input_validation(self):
        ...
        # Test valid input
        results = self.validator.validate(valid_definition_data, "definition_generation")
        ...
        error_results = [r for r in results if hasattr(r, 'severity') and r.severity == ValidationSeverity.ERROR]
    ...
    async def test_complete_security_pipeline(self):
        ...
        malicious_request = ValidationRequest(
            ...
            data={
                ...
                "context_dict": {
                    "organisatorisch": ["test with kut words"]
                }
            },
            headers={"User-Agent": "Mozilla/5.0"},
...
class TestSecurityPerformance:
    ...
    async def test_security_validation_performance(self):
        ...
        assert total_time < 5.0, f"Security validation too slow: {total_time:.2f}s for 50 requests"
        
        # Calculate average time per request
        avg_time = total_time / 50
        assert avg_time < 0.1, f"Average validation time too slow: {avg_time:.3f}s per request"
    ...
    def test_sanitization_performance(self):
        ...
        assert total_time < 2.0, f"Sanitization too slow: {total_time:.2f}s for 100 operations"
