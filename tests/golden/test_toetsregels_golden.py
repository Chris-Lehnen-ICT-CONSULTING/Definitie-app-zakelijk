...
class TestToetsregelsGolden:
    ...
    def test_ESS_002_begint_met_lidwoord(self):
        ...
        for definitie in [
            ...
            ess_002 = next((r for r in result.regel_resultaten if r.regel_id == "ESS-002"), None)
            if ess_002:
    ...
    def test_STR_002_minimale_lengte(self):
        ...
        str_002 = next((r for r in result.regel_resultaten if r.regel_id == "STR-002"), None)
        if str_002:
    ...
    def test_STR_003_eindigt_met_punt(self):
        ...
        result = toets_definitie(
            term="test",
            definitie="Een definitie die netjes eindigt met een punt."
        )

        str_003 = next((r for r in result.regel_resultaten if r.regel_id == "STR-003"), None)
        if str_003:
        ...
        result = toets_definitie(
            term="test",
            definitie="Een definitie zonder punt"
        )
    ...
    def test_CON_001_geen_tegenstrijdigheden(self):
        ...
        for definitie in contradictory_definitions:
            result = toets_definitie(term="test", definitie=definitie)
            con_001 = next((r for r in result.regel_resultaten if r.regel_id == "CON-001"), None)
            if con_001:
                assert not con_001.voldaan, f"Should fail for contradiction: {definitie[:30]}..."
    ...
    def test_VER_001_geen_vage_termen(self):
        ...
        for definitie in vague_definitions:
            result = toets_definitie(term="test", definitie=definitie)
            ver_001 = next((r for r in result.regel_resultaten if r.regel_id == "VER-001"), None)
            if ver_001:
                assert not ver_001.voldaan, f"Should fail for vague: {definitie[:30]}..."
    ...
    def test_VER_002_geen_voorbeelden_in_definitie(self):
        ...
        for definitie in example_definitions:
            result = toets_definitie(term="test", definitie=definitie)
            ver_002 = next((r for r in result.regel_resultaten if r.regel_id == "VER-002"), None)
            if ver_002:
                assert not ver_002.voldaan, f"Should fail for examples: {definitie[:30]}..."
    ...
    def test_INT_001_juridische_correctheid(self):
        ...
        int_001 = next((r for r in result.regel_resultaten if r.regel_id == "INT-001"), None)
        if int_001:
    ...
    def test_edge_case_special_characters(self):
        ...
        result = toets_definitie(
            term="test",
            definitie="Een definitie met speciale karakters: § ® © ™."
        )
    ...
    def test_edge_case_very_long_definition(self):
        ...
        str_rules = [r for r in result.regel_resultaten if r.regel_id.startswith("STR-")]
...
class TestGoldenDefinitions:
    ...
    @pytest.mark.parametrize("term,definition_type", [
        ("hypotheek", "good"),
        ("eigendom", "good"),
        ("overeenkomst", "good"),
    ])
    def test_golden_good_definitions(self, term, definition_type):
        ...
        assert result.totaal_score > 0.7, f"Good definition for {term} should score > 0.7"
        ...
        assert passing / total > 0.7, f"Good definition should have >70% rules passing"
    ...
    def test_golden_bad_definitions(self, term, definition_type):
        ...
        assert result.totaal_score < 0.6, f"Bad definition ({definition_type}) for {term} should score < 0.6"

        # Should have multiple failures
        failing = sum(1 for r in result.regel_resultaten if not r.voldaan)
        assert failing >= 2, f"Bad definition should have at least 2 failing rules"