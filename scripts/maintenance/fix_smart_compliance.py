...
class SMARTComplianceFixer:
    ...
    def __init__(self, dry_run: bool = False, backup: bool = True):
        ...
        if self.backup and not self.dry_run:
            self.backup_dir = (
                Path("backups") / f'smart_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
            )
    ...
    def add_smart_section(
        ...
        if re.search(
            ...
            return self.enhance_existing_smart_section(
                content, missing_criteria, file_type
            )

        # Generate new SMART section based on file type
        if file_type == "story":
            smart_section = self.generate_story_smart_section(
                file_path, missing_criteria
            )
        elif file_type == "epic":
            smart_section = self.generate_epic_smart_section(
                file_path, missing_criteria
            )
        elif file_type == "requirement":
            smart_section = self.generate_requirement_smart_section(
                file_path, missing_criteria
            )
        else:
    ...
    def generate_story_smart_section(
        self, file_path: Path, missing_criteria: list[SMARTCriterion]
    ) -> str:
    ...
    def enhance_existing_smart_section(
        self, content: str, missing_criteria: list[SMARTCriterion], file_type: str
    ) -> str:
        ...
        for criterion in missing_criteria:
            ...
            if not re.search(rf"###?\s*{criterion_name}", content, re.IGNORECASE):
                ...
                if smart_match:
                    ...
                    suggestion = self.generate_suggestion_based_on_type(
                        criterion, file_type
                    )
                    new_subsection = f"\n### {criterion.value}\n{suggestion}\n"
                    ...
                    content = (
                        content[:insert_pos] + new_subsection + content[insert_pos:]
                    )

        return content

    def generate_suggestion_based_on_type(
        self, criterion: SMARTCriterion, file_type: str
    ) -> str:
    ...
    def fix_file(self, file_path: Path) -> SMARTAnalysis:
        ...
        try:
            ...
            if score >= 4:  # Consider 4/5 as compliant
                logger.info(
                    f"File {file_path.name} is already SMART compliant (score: {score}/5)"
                )
            ...
            fixed_content = self.add_smart_section(
                original_content, file_path, missing_criteria
            )

            # Update change log
            if (
                "wijzigingslog" in fixed_content.lower()
                or "change log" in fixed_content.lower()
            ):
            ...
            error_msg = f"Error fixing {file_path}: {e!s}"
    ...
    def generate_report(self, results: list[SMARTAnalysis]) -> str:
        ...
        for score in sorted(score_distribution.keys(), reverse=True):
            ...
            report.append(
                f"{status} Score {score}/5: {count} files ({percentage:.1f}%)"
            )
        report.append("")
        ...
        for criterion, count in criterion_stats.items():
            percentage = count / len(results) * 100 if results else 0
            report.append(
                f"{criterion.value}: {count}/{len(results)} ({percentage:.1f}%)"
            )
        report.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fix SMART compliance in documentation"
    )
    parser.add_argument(
    ...
    # Initialize fixer
    fixer = SMARTComplianceFixer(dry_run=args.dry_run, backup=not args.no_backup)
    ...
    report_path = (
        Path("logs")
        / f'smart_compliance_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    )
