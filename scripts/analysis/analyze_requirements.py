...
def analyze_requirements():
    ...
    for req in requirements:
        ...
        if not req["criteria"]:
            ...
            word in req["criteria"].lower()
            for word in ["moet", "binnen", "maximaal", "minimaal"]
        ):
...
def validate_epic_story_links(requirements):
    ...
    master_file = Path(
        "/Users/chrislehnen/Projecten/Definitie-app/docs/stories/MASTER-EPICS-USER-STORIES.md"
    )
...
def analyze_domain_requirements():
    ...
    for i in range(13, 23):
        req_file = Path(
            f"/Users/chrislehnen/Projecten/Definitie-app/docs/requirements/REQ-{i:03d}.md"
        )
        if req_file.exists():
            req_data = parse_requirement_file(req_file)
            if req_data:
    ...
    for req in domain_reqs:
        ...
        if not any(
            ...
            for word in ["nederlandse", "juridisch", "wetgeving", "recht"]
        ):
...
def main():
    ...
    total_issues = (
        len(missing_files)
        + len(invalid_links)
        + len(issues["orphaned_reqs"])
        + len(issues["missing_criteria"])
        + len(issues["no_smart_criteria"])
    )

    print(f"\n📊 Total Issues Found: {total_issues}")
    print(f"  - Critical (must fix): {len(missing_files) + len(invalid_links)}")
    print(
        f"  - Warnings (should fix): {len(issues['orphaned_reqs']) + len(issues['missing_criteria'])}"
    )
    print(
        f"  - Quality improvements: {len(issues['no_smart_criteria']) + len(issues['duplicates'])}"
    )
    ...
    with open(
        "/Users/chrislehnen/Projecten/Definitie-app/docs/requirements/verification_results.json",
        "w",
    ) as f:
