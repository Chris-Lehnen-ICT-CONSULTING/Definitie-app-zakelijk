    ...
    for req_file in sorted(req_dir.glob("REQ-*.md")):
        ...
        if "Conflicten" in content:
            ...
            if conflict_match:
                ...
                if (
                    conflict_text
                    and "Geen conflicten" not in conflict_text
                    and "geen" not in conflict_text.lower()
                ):
...
def check_validation_rules_coverage():
    ...
    rules_dir = Path(
        "/Users/chrislehnen/Projecten/Definitie-app/src/toetsregels/regels"
    )
    actual_rules = []
...
def main():
    ...
    print(
        f"  - Total actual validation rules: {validation_coverage['total_actual_rules']}"
    )
    print(
        f"  - Validation requirements: {validation_coverage['validation_req_count']} (REQ-023 to REQ-037)"
    )
    print(
        f"  - Categories covered: {', '.join(validation_coverage['categories_covered'])}"
    )
    print(
        f"  - Sample rule files: {', '.join(validation_coverage['actual_rule_files'])}"
    )
