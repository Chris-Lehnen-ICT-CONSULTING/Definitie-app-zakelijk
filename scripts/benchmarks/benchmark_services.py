...
print(f"Difference:     {((new_inst['mean'] - legacy_inst['mean']) / legacy_inst['mean'] * 100):.1f}%")
...
def test_legacy_processing(mock_api):
    ...
    result = service.generate_definition(
        begrip="test",
        context_dict={'organisatorisch': ['benchmark']},
        force_sync=True
    )
    return result
...
print(f"Difference:        {((new_proc['mean'] - legacy_proc['mean']) / legacy_proc['mean'] * 100):.1f}%")
