...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/traceability.log'),
...
class TraceabilityAnalyzer:
    ...
    def __init__(self, dry_run: bool = False, backup: bool = True):
        ...
        if self.backup and not self.dry_run:
            self.backup_dir = Path('backups') / f'traceability_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
            self.backup_dir.mkdir(parents=True, exist_ok=True)

    def extract_document_id(self, file_path: Path) -> Optional[str]:
    ...
    def extract_epic_assignment(self, content: str) -> Optional[str]:
    ...
    def analyze_file(self, file_path: Path) -> Optional[Document]:
        ...
        try:
            ...
            doc = Document(
                ...
                epic_id=self.extract_epic_assignment(content) if doc_type == DocumentType.STORY else None,
    ...
    def suggest_epic_for_story(self, story: Document) -> str:
        ...
        epic_keywords = {
            'EPIC-001': ['generatie', 'generate', 'definition', 'definitie', 'create', 'maken'],
            'EPIC-002': ['validatie', 'validation', 'toets', 'check', 'quality', 'kwaliteit'],
            ...
            'EPIC-006': ['security', 'beveiliging', 'auth', 'login', 'permission', 'role'],
            'EPIC-007': ['performance', 'prestatie', 'scale', 'speed', 'optimize', 'cache'],
    ...
    def check_circular_dependencies(self):
        ...
        def find_cycles(doc_id: str, visited: Set[str], path: List[str]) -> Optional[List[str]]:
        ...
        for doc_id, doc in self.documents.items():
            if doc.doc_type == DocumentType.STORY:
                ...
                if story_num > 1:
                    ...
                    if prev_story in self.documents and prev_story not in doc.dependencies:
    ...
    def build_traceability_matrix(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        stories = [d for d in self.documents.values() if d.doc_type == DocumentType.STORY]
        requirements = [d for d in self.documents.values() if d.doc_type == DocumentType.REQUIREMENT]

        # Epic to Stories mapping
        for epic in epics:
            epic_stories = [s.doc_id for s in stories if s.epic_id == epic.doc_id]
            epic_reqs = [r.doc_id for r in requirements if epic.doc_id in r.references_to]
    ...
    def fix_orphaned_story(self, story: Document, suggested_epic: str) -> bool:
        ...
        try:
            # Backup file
            if self.backup:
                backup_path = self.backup_dir / story.path.relative_to(story.path.parents[2])
            ...
            if 'afhankelijkheden' not in content.lower() and 'dependencies' not in content.lower():
            ...
            error_msg = f"Error fixing orphaned story {story.doc_id}: {str(e)}"
            logger.error(error_msg)
            self.stats['errors'].append(error_msg)
            return False
    ...
    def fix_broken_link(self, doc: Document, broken_ref: str) -> bool:
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would fix broken link to {broken_ref} in {doc.doc_id}")
            return False

        try:
            # Backup file
            if self.backup:
                backup_path = self.backup_dir / doc.path.relative_to(doc.path.parents[2])
                backup_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            error_msg = f"Error fixing broken link in {doc.doc_id}: {str(e)}"
            logger.error(error_msg)
            self.stats['errors'].append(error_msg)
            return False
    ...
    def add_missing_dependency(self, doc: Document, dependency: str) -> bool:
        ...
        try:
            ...
            if dep_section:
                ...
                test_match = re.search(r'##\s*(?:Test|Scenario)', content, re.IGNORECASE)
                if test_match:
            ...
            error_msg = f"Error adding dependency to {doc.doc_id}: {str(e)}"
            logger.error(error_msg)
            self.stats['errors'].append(error_msg)
            return False
    ...
    def apply_fixes(self):
        ...
        for issue in self.issues:
            if issue.issue_type == "orphaned_story" and issue.severity in ["HIGH", "MEDIUM"]:
    ...
    def generate_report(self) -> str:
        ...
        # Count valid links
        valid_links = sum(
            len(doc.references_to) for doc in self.documents.values()
        ...
        if not self.dry_run:
            ...
            report.append("-" * 40)
            report.append(f"Orphaned stories assigned: {self.stats['orphans_assigned']}")
        ...
        # Orphaned stories
        orphaned = [d for d in self.documents.values()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Update and fix traceability in documentation')
    parser.add_argument('--dry-run', action='store_true', help='Preview changes without modifying files')
    parser.add_argument('--no-backup', action='store_true', help='Skip creating backups')
    parser.add_argument('--auto-fix', action='store_true', help='Automatically apply suggested fixes')
    parser.add_argument('--export-matrix', help='Export traceability matrix to JSON file')
    ...
    # Save report
    report_path = Path('logs') / f'traceability_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
