...
API_KEY = ("lin_api_" + "fsFS5ivIV8lyLYbo" + "BO1erER4huHU7kxK" + "XanAN0dq")
...
def fetch_issues():
    ...
    for issue in issues:
        assignee = (
            issue["assignee"]["displayName"] if issue["assignee"] else "Unassigned"
        )
        print(f"[{issue['identifier']}] {issue['title']}")
        print(
            f"  Status: {issue['state']['name']} | Priority: {issue['priority']} | Assignee: {assignee}"
        )
        print("-" * 40)
