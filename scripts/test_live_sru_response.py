                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        record_tags = [
                            e
                            for e in all_elems
                            if e.tag.endswith("record")
                            and "recordData" in [c.tag.split("}")[-1] for c in e]
                        ]
                        print(f"  Fallback (no namespace): {len(record_tags)} records")

                        # Check for numberOfRecords
                        for ns_dict in [ns_12, ns_20]:
                            num_records_elem = root.find(
                                ".//srw:numberOfRecords", ns_dict
                            )
                            if num_records_elem is not None:
                                print(
                                    f"  numberOfRecords element: {num_records_elem.text}"
                                )
                                break
