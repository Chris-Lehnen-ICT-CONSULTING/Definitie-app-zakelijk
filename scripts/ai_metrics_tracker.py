        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if issues:
                for issue in issues:
                    ...
                    """, (metric.agent_name, issue.get('check', 'unknown'), 
                    ...
                    if result:
                        ...
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?)
                        """, (metric.agent_name, issue.get('check', 'unknown'),
                              issue.get('severity', 'unknown'), datetime.now()))
...
def create_streamlit_dashboard():
    ...
    st.set_page_config(
        page_title="AI Code Review Metrics",
        page_icon="🤖",
        layout="wide"
    )
    ...
    success_rate = (df['successful'].sum() / df['reviews'].sum() * 100) if df['reviews'].sum() > 0 else 0
    ...
    daily_success['success_rate'] = (daily_success['successful'] / daily_success['reviews'] * 100)
...
def main():
    ...
    parser.add_argument('command', choices=['record', 'report', 'dashboard'],
