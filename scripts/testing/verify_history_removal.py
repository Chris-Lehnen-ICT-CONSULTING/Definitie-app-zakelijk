...
class HistoryRemovalVerifier:
    ...
    def _check_remnants(self) -> bool:
            ...
            if import_pattern.search(content):
                ...
                for line_num, line in enumerate(content.splitlines(), 1):
                    if import_pattern.search(line) and not line.strip().startswith('#'):
                        history_imports.append(f"{py_file.relative_to(self.project_root)}:{line_num}")
        ...
        for py_file in self.project_root.glob('src/**/*.py'):
            ...
            if instantiation_pattern.search(content):
                for line_num, line in enumerate(content.splitlines(), 1):
                    if instantiation_pattern.search(line) and not line.strip().startswith('#'):
                        history_instances.append(f"{py_file.relative_to(self.project_root)}:{line_num}")
        ...
        for py_file in self.project_root.glob('src/**/*.py'):
            ...
            if config_pattern.search(content):
                print(f"  ⚠️  'history' found in tab config: {py_file.relative_to(self.project_root)}")
    ...
    def _check_session_state(self) -> bool:
        ...
        try:
            ...
            with patch('streamlit.session_state', {}):
                ...
                if history_keys:
                    suspicious = [k for k in history_keys if any(x in str(k).lower() for x in ['tab', 'view', 'page'])]
                    if suspicious:
                        print(f"  ⚠️  Found history-related session keys: {suspicious}")
                        self.results['warnings'].append(f"History session keys: {suspicious}")
                        return False
                    else:
                        print(f"  ✅ Session state clean (found {len(history_keys)} legacy keys, but not tab-related)")
                else:
    ...
    def _measure_and_compare_performance(self):
        ...
        if self.baseline_file.exists():
            ...
            if base_metrics:
                import_diff = metrics['import_time'] - base_metrics.get('import_time', 0)
                memory_diff = metrics['memory_mb'] - base_metrics.get('memory_mb', 0)

                print(f"\n  📊 Comparison with baseline:")
                print(f"     Import time: {'+' if import_diff > 0 else ''}{import_diff:.3f}s")
                print(f"     Memory: {'+' if memory_diff > 0 else ''}{memory_diff:.2f} MB")
    ...
    def _generate_report(self, all_passed: bool):
        ...
        # Save report
        report_file = self.project_root / f'history_removal_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
...
def main():
    ...
    parser.add_argument('--verify', action='store_true', help='Run verification (default)')
