...
def main():
        ...
        if directory.exists():
            ...
            if results["criteria_scores"]:
                ...
                for criterion, count in sorted(results["criteria_scores"].items()):
                    percentage = (
                        (count / results["total_files"] * 100)
                        if results["total_files"] > 0
                        else 0
                    )
                    print(
                        f"    - {criterion.capitalize()}: {count}/{results['total_files']} ({percentage:.1f}%)"
                    )
    ...
    print(
        f"✅ Fully SMART compliant: {overall_stats['fully_compliant']} ({overall_stats['fully_compliant']/overall_stats['total_files']*100:.1f}%)"
    )
    print(
        f"⚠️  Partially compliant: {overall_stats['partial_compliant']} ({overall_stats['partial_compliant']/overall_stats['total_files']*100:.1f}%)"
    )
    print(
        f"❌ Non-compliant: {overall_stats['non_compliant']} ({overall_stats['non_compliant']/overall_stats['total_files']*100:.1f}%)"
    )

    # Determine overall status
    compliance_rate = (
        (overall_stats["fully_compliant"] + overall_stats["partial_compliant"])
