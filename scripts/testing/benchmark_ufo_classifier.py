...
class UFOClassifierBenchmark:
    ...
    def _load_test_data(self) -> List[Tuple[str, str, UFOCategory]]:
        ...
        return [
            # KIND examples
            ("Persoon", "Een natuurlijk mens met rechtspersoonlijkheid", UFOCategory.KIND),
            ("Organisatie", "Een rechtspersoon zoals een bedrijf of instelling", UFOCategory.KIND),
            ("Document", "Een schriftelijk stuk met juridische waarde", UFOCategory.KIND),
            ...
            # EVENT examples
            ("Arrestatie", "Het proces waarbij een verdachte wordt aangehouden tijdens onderzoek", UFOCategory.EVENT),
            ("Zitting", "Een bijeenkomst van de rechtbank waar een zaak wordt behandeld", UFOCategory.EVENT),
            ("Procedure", "Een reeks handelingen die in bepaalde volgorde worden uitgevoerd", UFOCategory.EVENT),
            ("Onderzoek", "Het proces van het verzamelen en analyseren van informatie", UFOCategory.EVENT),
            ("Verhoor", "Het ondervragen van een verdachte of getuige", UFOCategory.EVENT),

            # ROLE examples
            ("Verdachte", "Een persoon in de hoedanigheid van mogelijke dader", UFOCategory.ROLE),
            ("Advocaat", "Een persoon in de rol van juridisch vertegenwoordiger", UFOCategory.ROLE),
            ("Rechter", "Een persoon fungerend als onpartijdige beslisser", UFOCategory.ROLE),
            ("Getuige", "Iemand in de rol van informatie verschaffer", UFOCategory.ROLE),
            ("Curator", "Een persoon aangesteld als beheerder van een faillissement", UFOCategory.ROLE),

            # RELATOR examples
            ("Contract", "Een overeenkomst tussen twee of meer partijen", UFOCategory.RELATOR),
            ("Huwelijk", "Een formele verbintenis tussen twee personen", UFOCategory.RELATOR),
            ("Vergunning", "Een toestemming verleend door een bevoegd gezag", UFOCategory.RELATOR),
            ("Dagvaarding", "Een oproep om voor de rechter te verschijnen", UFOCategory.RELATOR),
            ("Mandaat", "Een bevoegdheid verleend aan iemand om namens een ander te handelen", UFOCategory.RELATOR),

            # MODE examples
            ("Gezondheid", "De fysieke en mentale toestand van een persoon", UFOCategory.MODE),
            ("Locatie", "De plaats waar iets of iemand zich bevindt", UFOCategory.MODE),
            ("Status", "De actuele toestand van een zaak of procedure", UFOCategory.MODE),
            ("Eigenschap", "Een kenmerk dat aan iets wordt toegeschreven", UFOCategory.MODE),

            # QUANTITY examples
            ("Bedrag", "Een hoeveelheid geld uitgedrukt in euro's", UFOCategory.QUANTITY),
            ("Percentage", "Een deel uitgedrukt als fractie van 100", UFOCategory.QUANTITY),
            ("Duur", "De tijdspanne tussen begin en einde", UFOCategory.QUANTITY),
            ("Aantal", "Een meetbare hoeveelheid van discrete items", UFOCategory.QUANTITY),

            # QUALITY examples
            ("Betrouwbaarheid", "De mate waarin iets te vertrouwen is", UFOCategory.QUALITY),
            ("Ernst", "De graad van zwaarte van een situatie", UFOCategory.QUALITY),
            ("Relevantie", "De mate van belangrijkheid voor de zaak", UFOCategory.QUALITY),
            ("Waarschijnlijkheid", "De kans dat iets waar of juist is", UFOCategory.QUALITY),

            # PHASE examples
            ("Voorlopig", "Een tijdelijke fase in afwachting van definitieve status", UFOCategory.PHASE),
            ("Gesloten", "De eindstatus van een afgeronde zaak", UFOCategory.PHASE),
            ("In behandeling", "De fase waarin actief aan iets wordt gewerkt", UFOCategory.PHASE),
    ...
    def benchmark_caching_effectiveness(self) -> Dict:
        ...
        return {
            ...
            'speedup_factor': statistics.mean(times_no_cache) / statistics.mean(times_cached),
            'cache_info': str(self.classifier.pattern_matcher.find_matches.cache_info())
        }
    ...
    def print_report(self, results: Dict):
        ...
        for cat, stats in acc['category_accuracy'].items():
            print(f"    • {cat}: {stats['accuracy']:.1%} ({stats['total_samples']} samples)")
