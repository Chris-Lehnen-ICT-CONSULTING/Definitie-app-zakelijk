...
class DocumentNormalizer:
    ...
    def should_process_file(self, filepath: Path) -> bool:
        ...
        if filepath.name.endswith('.backup') or filepath.name.endswith('.linkfix_backup'):
            return False
        return True
    ...
    def normalize_content(self, content: str, filepath: Path) -> Tuple[str, bool]:
        ...
        if 'US-' in str(filepath) or 'EPIC-' in str(filepath):
            ...
            content = re.sub(r'^## Epic (\d+):', r'## Episch Verhaal \1:', content, flags=re.MULTILINE)
            content = re.sub(r'^### Story (\d+\.\d+):', r'### Verhaal \1:', content, flags=re.MULTILINE)

            # Acceptatiecriteria format
            content = re.sub(r'^\s*Given\s+', '   Gegeven ', content, flags=re.MULTILINE)
    ...
    def process_directory(self, directory: str):
        ...
        for filepath in base_path.rglob('*.md'):
            ...
            try:
                ...
                normalized_content, was_modified = self.normalize_content(content, filepath)
                ...
                if was_modified:
                    ...
                    if not self.dry_run:
                        ...
                        print(f'[DRY RUN] Zou normaliseren: {filepath.relative_to(base_path)}')
    ...
    def generate_report(self) -> str:
        ...
        if self.stats['translations']:
            ...
            for term, count in sorted(self.stats['translations'].items(), key=lambda x: x[1], reverse=True):
                report.append(f'- {term}: {count}x')

        if self.stats['typos']:
            report.append('\n## Gecorrigeerde Typefouten')
            for typo, count in sorted(self.stats['typos'].items(), key=lambda x: x[1], reverse=True):
                report.append(f'- {typo}: {count}x')

        if self.stats['justice_terms']:
            report.append('\n## Geharmoniseerde Justice Terminologie')
            for term, count in sorted(self.stats['justice_terms'].items(), key=lambda x: x[1], reverse=True):
                report.append(f'- {term}: {count}x')
        ...
        total_changes = sum(self.stats['translations'].values()) + \
...
def main():
    ...
    # Sla rapport op
    if not dry_run:
        report_path = '/Users/chrislehnen/Projecten/Definitie-app/docs/NORMALISATIE_RAPPORT.md'
