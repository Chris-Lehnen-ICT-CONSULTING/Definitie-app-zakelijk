...
class BronZoeker:
    ...
    def _filter_en_sorteer_bronnen(
        self, bronnen: List[BronReferentie], max_resultaten: int
    ) -> List[BronReferentie]:
    ...
    def _genereer_aanbevelingen(self, bronnen: List[BronReferentie]) -> List[str]:
        ...
        if not bronnen:
            aanbevelingen.append(
                "Geen bronnen gevonden. Probeer een bredere zoekopdracht."
            )
        ...
        if BronType.WETGEVING in type_counts:
            aanbevelingen.append(
                f" {type_counts[BronType.WETGEVING]} wettelijke bron(nen) gevonden"
            )

        if BronType.INTERNE_DEFINITIE in type_counts:
            aanbevelingen.append(
                f"= {type_counts[BronType.INTERNE_DEFINITIE]} interne definitie(s) beschikbaar"
            )
        ...
        if hoge_betrouwbaarheid:
            aanbevelingen.append(
                f"P {len(hoge_betrouwbaarheid)} bronnen met hoge betrouwbaarheid"
            )
        ...
        if niet_toegankelijk:
            aanbevelingen.append(
                f"� {len(niet_toegankelijk)} bronnen mogelijk niet direct toegankelijk"
            )
...
async def zoek_bronnen_voor_begrip(
    begrip: str, context: Optional[Dict[str, Any]] = None, max_resultaten: int = 5
) -> BronZoekResultaat:
    """
...
async def valideer_definitie_bronnen(
    definitie: str, verwachte_bronnen: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
    ...
    if gevonden_bronnen:
        ...
        resultaat["gemiddelde_betrouwbaarheid"] = sum(betrouwbaarheden) / len(
            betrouwbaarheden
        )
        ...
        if resultaat["validatie_score"] > 0.8:
            ...
            resultaat["aanbevelingen"].append(
                "� Bronverwijzingen kunnen verbeterd worden"
            )

        if not resultaat["bevat_wettelijke_bronnen"]:
            resultaat["aanbevelingen"].append(
                "=� Overweeg toevoegen van wettelijke bronnen"
            )

    else:
        resultaat["aanbevelingen"].append(
            "� Geen bronverwijzingen gevonden in definitie"
        )
...
if __name__ == "__main__":
    ...
    async def test_bron_lookup():
        ...
        print(
            f"   Gevonden: {len(zoek_resultaat.gevonden_bronnen)} van {zoek_resultaat.totaal_gevonden}"
        )
        ...
        test_definitie = (
            "Authenticatie volgens artikel 1 Wet op de identificatie is het proces..."
        )
