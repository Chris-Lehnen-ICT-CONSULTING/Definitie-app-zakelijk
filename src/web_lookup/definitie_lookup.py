...
class DefinitieGelijkenisAnalyzer:
    ...
    def _normaliseer_tekst(self, tekst: str) -> List[str]:
        ...
        gefilterde_woorden = [
            w for w in woorden if w not in self.stop_woorden and len(w) > 2
        ]

        return gefilterde_woorden

    def _bereken_woord_gelijkenis(
        self, woorden1: List[str], woorden2: List[str]
    ) -> float:
    ...
    def _bereken_structuur_gelijkenis(
        self, woorden1: List[str], woorden2: List[str]
    ) -> float:
        ...
        # Lengte gelijkenis
        lengte_ratio = min(len(woorden1), len(woorden2)) / max(
            len(woorden1), len(woorden2)
        )
    ...
    def _bereken_semantische_gelijkenis(
        self, woorden1: List[str], woorden2: List[str]
    ) -> float:
        ...
        return (
            gemeenschappelijke_clusters / totale_clusters
            if totale_clusters > 0
            else 0.0
        )

    def _vind_semantische_clusters(
        self, woorden: List[str], clusters: Dict[str, Set[str]]
    ) -> Set[str]:
...
class DefinitieZoeker:
    ...
    async def zoek_definities(
        ...
        resultaat = self._analyseer_zoekresultaten(begrip, alle_definities, opties)
        ...
        # Cache resultaat
        self.cache[cache_key] = {"result": resultaat, "timestamp": datetime.now()}

        return resultaat

    async def _zoek_interne_definities(
        self, begrip: str, context: Optional[Dict[str, Any]] = None
        ...
        try:
            ...
            fuzzy_resultaten = repository.search_definities(
                query=begrip, fuzzy=True, limit=10
            )
            ...
            for definitie in alle_resultaten:
                gevonden_def = GevondenDefinitie(
                    ...
                    metadata={
                        ...
                        "created_at": (
                            definitie.created_at.isoformat()
    ...
    async def _zoek_externe_definities(
        ...
        for extern_begrip, definitie_info in juridische_definities.items():
            if (
                begrip_lower in extern_begrip.lower()
                or extern_begrip.lower() in begrip_lower
            ):
                gevonden_def = GevondenDefinitie(
                    ...
                    context=self._bepaal_context(
                        definitie_info.get("rechtsgebied", "")
                    ),
    ...
    def _analyseer_zoekresultaten(
        ...
        gerelateerde_begrippen = self._vind_gerelateerde_begrippen(
            query, alle_definities
        )
    ...
    def _remove_duplicates(
        self, definities: List[GevondenDefinitie]
    ) -> List[GevondenDefinitie]:
    ...
    def _vind_gerelateerde_begrippen(
        self, query: str, alle_definities: List[GevondenDefinitie]
    ) -> List[str]:
        ...
        for definitie in alle_definities:
            ...
            if (
                query_woorden & begrip_woorden
                and definitie.begrip.lower() != query.lower()
            ):
    ...
    def _genereer_zoek_suggesties(
        self, query: str, top_definities: List[GevondenDefinitie]
    ) -> List[str]:
        ...
        externe_bronnen = [
            d for d in top_definities if "Interne Database" not in d.bron
        ]
        if externe_bronnen:
            suggesties.append(
                f"= {len(externe_bronnen)} externe bron(nen) beschikbaar"
            )

        hoge_betrouwbaarheid = [d for d in top_definities if d.betrouwbaarheid >= 0.9]
        if hoge_betrouwbaarheid:
            suggesties.append(
                f"P {len(hoge_betrouwbaarheid)} definities met hoge betrouwbaarheid"
            )

        return suggesties

    def _analyseer_duplicaten(
        self, definities: List[GevondenDefinitie]
    ) -> Dict[str, Any]:
...
async def zoek_definitie(
    begrip: str, context: Optional[Dict[str, Any]] = None, max_resultaten: int = 10
) -> DefinitieZoekResultaat:
    """
...
async def detecteer_duplicaten(
    begrip: str, definitie: str, threshold: float = 0.8
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    async def test_definitie_lookup():
        ...
        print("ð Zoekresultaat voor 'authenticatie':")
        print(f"   Zoektijd: {zoek_resultaat.zoek_tijd:.3f}s")
        print(
            f"   Gevonden: {len(zoek_resultaat.gevonden_definities)} van {zoek_resultaat.totaal_gevonden}"
        )
        ...
        for definitie in zoek_resultaat.gevonden_definities:
            ...
            print(
                f"     Relevantie: {definitie.relevantie:.2f}, Betrouwbaarheid: {definitie.betrouwbaarheid:.2f}"
            )
            print(f"     {definitie.definitie[:80]}...")

        # Test 2: Duplicaat detectie
        test_definitie = (
            "Het proces van identiteitsvaststelling door verificatie van gegevens"
        )
        duplicaat_resultaat = await detecteer_duplicaten(
            "authenticatie", test_definitie
        )

        print("\nð Duplicaat analyse:")
        print(f"   Duplicaten gevonden: {duplicaat_resultaat['duplicaat_gevonden']}")
        print(
            f"   Aantal mogelijke duplicaten: {len(duplicaat_resultaat['mogelijke_duplicaten'])}"
        )
