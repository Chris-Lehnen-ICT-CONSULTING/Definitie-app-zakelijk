...
class UIComponentsAdapter:
    ...
    def export_definition(self, format: str = "txt") -> bool:
        ...
        try:
            ...
            st.error(f"❌ Technische fout bij export: {str(e)}")
            return False
    ...
    def _collect_ui_data_for_export(self) -> dict[str, Any]:
        ...
        ui_data["voorbeeld_zinnen"] = SessionStateManager.get_value("voorbeeld_zinnen", [])
        ui_data["praktijkvoorbeelden"] = SessionStateManager.get_value("praktijkvoorbeelden", [])
        ui_data["tegenvoorbeelden"] = SessionStateManager.get_value("tegenvoorbeelden", [])
        ...
        ui_data["beoordeling_gen"] = SessionStateManager.get_value("beoordeling_gen", [])
        ui_data["toetsresultaten"] = SessionStateManager.get_value("toetsresultaten", {})
        
        # Bronnen
        ui_data["bronnen_gebruikt"] = SessionStateManager.get_value("bronnen_gebruikt", "")
        ...
        ui_data = {k: v for k, v in ui_data.items() if v is not None}
    ...
    def prepare_for_review(self, definition_id: int, notes: Optional[str] = None) -> bool:
        """
    ...
    if SessionStateManager.has_generated_definition():
        ...
        with col1:
            selected_format = st.selectbox(
                ...
                format_func=lambda x: next(f["label"] for f in formats if f["value"] == x),
