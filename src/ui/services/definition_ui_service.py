...
class DefinitionUIService:
    ...
    def __init__(
        ...
        data_aggregation_service: Optional[DataAggregationService] = None
    ):
        """
        ...
        self.data_aggregation_service = data_aggregation_service or DataAggregationService(repository)
    ...
    def export_definition(
        ...
        try:
            ...
            additional_data = self._prepare_ui_data_for_export(ui_data) if ui_data else None
    ...
    def _prepare_ui_data_for_export(self, ui_data: dict[str, Any]) -> dict[str, Any]:
        ...
        elif any(key in ui_data for key in ["organisatorisch_context", "juridisch_context", "wettelijk_context"]):
        ...
        if "metadata" in ui_data:
            ...
            metadata = {}
            metadata_fields = [
                "datum_voorstel", "voorgesteld_door", "organisatie",
                "categorie", "domein", "status", "versie"
            ]
    ...
    def prepare_definition_for_review(
        ...
        try:
            ...
            if not self.workflow_service.can_change_status(
                current_status=definitie.status,
                new_status="REVIEW"
            ):
                return {
                    "success": False,
    ...
    def get_definition_summary(self, definitie_id: int) -> Optional[dict[str, Any]]:
        """
        ...
        try:
            ...
            return {
                ...
                "definitie": definitie.definitie[:200] + "..." if len(definitie.definitie) > 200 else definitie.definitie,
                "status": definitie.status,
                "categorie": definitie.categorie,
                "created_at": definitie.created_at,
                "created_by": definitie.created_by,
                "can_edit": self.workflow_service.can_change_status(definitie.status, "DRAFT"),
                "can_review": self.workflow_service.can_change_status(definitie.status, "REVIEW"),
                "can_approve": self.workflow_service.can_change_status(definitie.status, "APPROVED")
            }
