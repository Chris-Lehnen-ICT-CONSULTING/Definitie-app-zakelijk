...
class ExportTab:
    ...
    def _render_export_section(self):
        ...
        with col3:
            date_range = st.selectbox(
                "Tijdsperiode",
                ["Alle", "Laatste week", "Laatste maand", "Laatste 3 maanden", "Aangepast..."],
        ...
        with col1:
            if st.button("🔍 Preview Export", help="Bekijk export voordat je download"):
                self._preview_export(export_format, status_filter, date_range, context_filter, 
                                   categorie_filter, min_score, include_metadata, start_date, end_date)
        
        with col2:
            if st.button("📤 Export", type="primary", help="Download export bestand"):
                self._execute_export(export_format, status_filter, date_range, context_filter,
                                   categorie_filter, min_score, include_metadata, start_date, end_date)
    ...
    def _render_bulk_operations(self):
        ...
        with st.expander("Bulk Status Updates", expanded=False):
            ...
            if st.button("🔄 Update Status (Bulk)", key="bulk_update"):
                self._execute_bulk_status_update(bulk_status_filter, new_status, bulk_reason)
        ...
        with st.expander("Bulk Import", expanded=False):
            ...
            if uploaded_file:
                ...
                if st.button("📥 Import", key="bulk_import"):
                    self._execute_bulk_import(uploaded_file, import_mode, duplicate_handling)
    ...
    def _render_database_management(self):
        ...
        with st.expander("Database Status", expanded=False):
            try:
                ...
                with col4:
                    ...
                    st.metric("📈 Gem. Score", f"{avg_score:.2f}" if avg_score else "N/A")
        ...
        with st.expander("Database Operaties", expanded=False):
            ...
            with col1:
                if st.button("🗑️ Cleanup Duplicates", help="Verwijder exacte duplicaten"):
            ...
            with col3:
                if st.button("📊 Analyze Database", help="Analyseer database prestaties"):
            ...
            if recent_defs:
                ...
                for def_rec in recent_defs:
                    activity_data.append({
                        'Datum': def_rec.created_at.strftime('%Y-%m-%d %H:%M') if def_rec.created_at else 'Onbekend',
                        'Begrip': def_rec.begrip,
                        'Status': def_rec.status,
                        'Context': def_rec.organisatorische_context,
                        'Score': f"{def_rec.validation_score:.2f}" if def_rec.validation_score else "N/A"
                    })
    ...
    def _preview_export(self, format_type, status_filter, date_range, context_filter, 
                       categorie_filter, min_score, include_metadata, start_date, end_date):
        """Preview export data."""
        try:
            # Get filtered data
            filtered_data = self._get_filtered_export_data(
                status_filter, date_range, context_filter, categorie_filter, 
    ...
    def _execute_export(self, format_type, status_filter, date_range, context_filter,
                       categorie_filter, min_score, include_metadata, start_date, end_date):
        """Voer export uit."""
        try:
            # Get filtered data
            ...
            export_data = self._generate_export_data(filtered_data, format_type, include_metadata)
    ...
    def _get_filtered_export_data(self, status_filter, date_range, context_filter,
                                categorie_filter, min_score, start_date, end_date) -> List[DefinitieRecord]:
        ...
        if min_score > 0:
            definitions = [d for d in definitions if d.validation_score and d.validation_score >= min_score]
        ...
        if date_range != "Alle":
            ...
            definitions = [d for d in definitions 
                         if d.created_at and d.created_at >= cutoff_date]
        
        return definitions
    
    def _generate_export_data(self, definitions: List[DefinitieRecord], format_type: str, include_metadata: bool):
    ...
    def _generate_json_export(self, definitions: List[DefinitieRecord], include_metadata: bool):
        ...
        for def_rec in definitions:
            ...
            if include_metadata:
                record.update({
                    ...
                    'created_at': def_rec.created_at.isoformat() if def_rec.created_at else None,
    ...
    def _generate_csv_export(self, definitions: List[DefinitieRecord], include_metadata: bool):
        """Genereer CSV export."""
        data = []
        for def_rec in definitions:
            record = {
    ...
    def _generate_txt_export(self, definitions: List[DefinitieRecord], include_metadata: bool):
        """Genereer TXT export."""
        lines = []
        for def_rec in definitions:
            ...
            lines.append(f"ORGANISATORISCHE CONTEXT: {def_rec.organisatorische_context}")
            ...
            if include_metadata:
                ...
                if def_rec.created_at:
                    lines.append(f"GEMAAKT: {def_rec.created_at.strftime('%Y-%m-%d %H:%M')}")
    ...
    def _generate_excel_export(self, definitions: List[DefinitieRecord], include_metadata: bool):
    ...
    def _execute_bulk_status_update(self, status_filter: str, new_status: str, reason: str):
        ...
        try:
            ...
            definitions = self.repository.search_definities(
                status=DefinitieStatus(status_filter),
                limit=1000
            )
            ...
            if not st.session_state.get('bulk_update_confirmed', False):
                st.warning(f"⚠️ Dit zal {len(definitions)} definities wijzigen van {status_filter} naar {new_status}")
    ...
    def _execute_bulk_import(self, uploaded_file, import_mode: str, duplicate_handling: str):
