...
class OrchestrationTab:
    ...
    def _render_iterative_improvement(self):
        ...
        # Start orchestration
        if st.button(
            "🚀 Start Iterative Verbetering", type="primary", key="start_orchestration"
        ):
            if begrip_input:
    ...
    async def _run_iterative_improvement(
        ...
        try:
            with st.spinner("🤖 Initialiseer Definitie Agent..."):
                ...
                categorie_enum = cat_mapping.get(
                    categorie.lower(), self.OntologischeCategorie.TYPE
                )
    ...
    def _simulate_agent_run(
        ...
        for i in range(max_iteraties):
            if real_time and progress_bar:
                ...
                if status_text:
                    status_text.text(
                        f"🔄 Iteratie {i + 1}/{max_iteraties}: Genereren en valideren..."
                    )
    ...
    def _render_agent_analytics(self):
        ...
        successful_runs = len(
            [r for r in historical_results if r.get("success", False)]
        )
        avg_iterations = (
            sum(r.get("iteration_count", 0) for r in historical_results) / total_runs
        )
        avg_final_score = (
            sum(r.get("final_score", 0) for r in historical_results) / total_runs
        )
            ...
            for i, result in enumerate(historical_results[-10:]):  # Show last 10
                with st.expander(
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**Status:** {'✅ Succesvol' if result.get('success') else '❌ Mislukt'}"
                        )
                        st.write(
                            f"**Timestamp:** {result.get('timestamp', 'Onbekend')}"
                        )

                    with col2:
                        st.write(
                            f"**Finale Score:** {result.get('final_score', 0):.3f}"
                        )
                        st.write(f"**Iteraties:** {result.get('iteration_count', 0)}")
                        st.write(
                            f"**Target Bereikt:** {'✅' if result.get('target_reached') else '❌'}"
                        )

                    with col3:
                        st.write(
                            f"**Processing Time:** {result.get('total_processing_time', 0):.1f}s"
                        )
                        if st.button("🔄 Herhaal", key=f"repeat_{i}"):
                            # Set values for repeat
                            SessionStateManager.set_value(
                                "orchestration_begrip", result.get("begrip", "")
                            )
        ...
        if recent_result:
            ...
            with col3:
                if st.button("🔄 Herhaal met Andere Parameters"):
                    # Reset for new run
                    SessionStateManager.set_value(
                        "orchestration_begrip", recent_result["begrip"]
                    )
                    st.rerun()
    ...
    def _render_performance_trends(self, results: list[dict[str, Any]]):
        ...
        # Score trend
        fig_score = px.line(
            df, x="Run", y="Final Score", title="Finale Score Trend", markers=True
        )
        ...
        with col1:
            fig_iter = px.bar(df, x="Run", y="Iterations", title="Iteraties per Run")
        ...
        with col2:
            ...
            success_stats = {
                ...
                "Gem. Score": [
                    ...
                    / max(total - successful, 1),
                ],
                "Gem. Iteraties": [
                    sum(
                        r.get("iteration_count", 0)
                        for r in results
                        if r.get("success", False)
                    )
                    / max(successful, 1),
