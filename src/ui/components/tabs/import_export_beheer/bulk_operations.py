...
class BulkOperations:
    ...
    def render(self):
        ...
        if from_status and to_status and from_status != to_status:
            ...
            st.info(f"Dit zal {count} definities wijzigen van '{from_status}' naar '{to_status}'")
    ...
    def _execute_bulk_status_change(self, from_status: str, to_status: str):
        ...
        with st.spinner("Status wijzigen..."):
            try:
                ...
                st.success(f"✅ {updated} definities bijgewerkt naar status '{to_status}'")
                st.rerun()

            except Exception as e:
                st.error(f"Fout bij bulk wijziging: {str(e)}")