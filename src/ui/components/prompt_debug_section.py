...
class PromptDebugSection:
    ...
    @staticmethod
    def render(generation_result: Optional[Any] = None, voorbeelden_prompts: Optional[Dict[str, str]] = None):
        """
        ...
        with st.expander("🔍 Debug: Gebruikte Prompts", expanded=False):
            ...
            st.caption("Deze sectie toont de exacte prompts die naar de AI zijn gestuurd voor analyse doeleinden.")
            ...
            with col1:
                if generation_result and hasattr(generation_result, 'prompt_template'):
                    st.metric("Definitie Prompt Lengte", f"{len(generation_result.prompt_template)} chars")
            
            with col2:
                if voorbeelden_prompts:
                    avg_length = sum(len(p) for p in voorbeelden_prompts.values()) / len(voorbeelden_prompts)
            ...
            st.caption("Kopieer een prompt hierboven en test met verschillende parameters.")
...
def capture_voorbeelden_prompts(begrip: str, definitie: str, context_dict: Dict[str, List[str]]) -> Dict[str, str]:
    """
    ...
    """
    from voorbeelden.unified_voorbeelden import UnifiedExamplesGenerator, ExampleRequest, ExampleType
