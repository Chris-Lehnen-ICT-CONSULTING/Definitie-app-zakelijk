...
class ExpertReviewTab:
        ...
        try:
            ...
            with col3:
                context_filter = st.selectbox(
                    "Filter context",
                    ["Alle"] + list(set(d.organisatorische_context for d in pending_reviews)),
                    key="review_context_filter"
                )
            
            # Apply filters
            filtered_reviews = self._apply_filters(pending_reviews, search_filter, context_filter, sort_by)
        ...
        with st.container():
            ...
            with col2:
                ...
                if definitie.validation_score:
                    score_color = "green" if definitie.validation_score > 0.8 else "orange" if definitie.validation_score > 0.6 else "red"
                    st.markdown(f"Score: <span style='color: {score_color}'>{definitie.validation_score:.2f}</span>", 
            ...
            with col4:
                # Action buttons
                if st.button(f"📝 Review", key=f"review_{definitie.id}"):
                    SessionStateManager.set_value("selected_review_definition", definitie)
                    st.rerun()
                
                if st.button(f"👁️ Preview", key=f"preview_{definitie.id}"):
    ...
    def _render_definition_details(self, definitie: DefinitieRecord):
        ...
        with st.expander("📋 Definitie Details", expanded=True):
            ...
            with col2:
                ...
                if definitie.created_at:
                    st.write(f"**Gemaakt:** {definitie.created_at.strftime('%Y-%m-%d %H:%M')}")
    ...
    def _render_validation_issues(self, definitie: DefinitieRecord):
        ...
        if critical_issues:
            ...
            for issue in critical_issues:
                st.write(f"- {issue.get('rule_id', 'Unknown')}: {issue.get('description', 'No description')}")
        ...
        if other_issues:
            with st.expander(f"📋 Overige Issues ({len(other_issues)})", expanded=False):
    ...
    def _render_comparison_view(self, definitie: DefinitieRecord):
        ...
        with col2:
            ...
            if edited_definitie != definitie.definitie:
                ...
                SessionStateManager.set_value(f"edited_definition_{definitie.id}", edited_definitie)
            else:
    ...
    def _render_review_form(self, definitie: DefinitieRecord):
        ...
        with col1:
            if st.button("✅ Submit Review", type="primary", key=f"submit_{definitie.id}"):
                self._submit_review(definitie, review_decision, review_comments, reviewer_name)
                ...
                if recent_reviews:
                    for definitie in recent_reviews:
                        if definitie.approved_by and definitie.approved_at:
                            with st.expander(f"{definitie.begrip} - Goedgekeurd door {definitie.approved_by}", expanded=False):
                                st.write(f"**Definitie:** {definitie.definitie}")
                                st.write(f"**Goedgekeurd op:** {definitie.approved_at.strftime('%Y-%m-%d %H:%M')}")
                                if definitie.approval_notes:
                                    st.write(f"**Notities:** {definitie.approval_notes}")
                else:
    ...
    def _submit_review(self, definitie: DefinitieRecord, decision: str, comments: str, reviewer: str):
    ...
    def _save_review_draft(self, definitie: DefinitieRecord, decision: str, comments: str):
    ...
    def _show_definition_preview(self, definitie: DefinitieRecord):
        ...
        with st.expander(f"👁️ Preview: {definitie.begrip}", expanded=True):
            st.info(definitie.definitie)
            st.caption(f"Context: {definitie.organisatorische_context} | Score: {definitie.validation_score:.2f if definitie.validation_score else 'N/A'}")
    
    def _apply_filters(self, reviews: List[DefinitieRecord], search: str, context: str, sort_by: str) -> List[DefinitieRecord]:
        """Apply filters en sorting to review list."""
        filtered = reviews
        
        # Search filter
        if search.strip():
            filtered = [r for r in filtered if search.lower() in r.begrip.lower() or search.lower() in r.definitie.lower()]
