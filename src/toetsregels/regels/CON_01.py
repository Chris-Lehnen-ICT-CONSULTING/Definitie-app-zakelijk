...
class CON01Validator:
    ...
    def __init__(self, config: Dict):
        ...
        self.naam = config.get('naam', 'Contextspecifieke formulering zonder expliciete benoeming')
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        if expliciete_hits:
            ...
            return False, f"❌ {self.id}: opgegeven context letterlijk in definitie herkend ('{gevonden}')", 0.0
        ...
        if contextuele_term_hits:
            if foute_match:
                return False, f"❌ {self.id}: bredere contexttermen herkend ({', '.join(sorted(contextuele_term_hits))}), en lijkt op fout voorbeeld", 0.0
            return False, f"🟡 {self.id}: bredere contexttaal herkend ({', '.join(sorted(contextuele_term_hits))}), formulering mogelijk vaag", 0.5
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Formuleer de definitie contextneutraal zonder expliciete verwijzingen")
        hints.append("Vermijd termen zoals: 'binnen de context van', 'juridisch', 'beleidsmatig'")
