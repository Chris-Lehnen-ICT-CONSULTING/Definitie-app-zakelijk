    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        for fout in self.foute_voorbeelden:
            if fout.lower() in tekst_lc:
                return False, f"❌ {self.id}: afkorting zonder uitleg aangetroffen in voorbeeld ('{fout}')", 0.0
        ...
        if zonder_toelichting:
            ...
            return False, f"❌ {self.id}: geen toelichting voor afkorting(en): {labels}", 0.0
        
        return True, f"✔️ {self.id}: alle afkortingen voorzien van directe toelichting of link", 1.0
