...
class ARAI02SUB1Validator:
    """Validator voor ARAI02SUB1."""
    ...
    def __init__(self, config: Dict):
        ...
        Args:
            config: Dictionary met configuratie uit ARAI02SUB1.json
        """
        self.config = config
        self.id = config.get("id", "ARAI02SUB1")
    ...
    def validate(
        ...
        """
        Valideer definitie volgens ARAI02SUB1 regel.
        ...
        try:
            ...
            if not container_termen:
                if goed:
                    result = "✔️ ARAI02SUB1: geen lexicale containerbegrippen, definitie sluit aan bij goed voorbeeld"
                else:
                    result = "✔️ ARAI02SUB1: geen containerbegrippen aangetroffen"

            else:
                if fout:
                    result = f"❌ ARAI02SUB1: containerbegrippen gevonden ({', '.join(container_termen)}), zoals in fout voorbeeld"
                else:
                    result = f"❌ ARAI02SUB1: containerbegrippen gevonden ({', '.join(container_termen)}), onvoldoende concreet"
...
def create_validator(config_path: str = None) -> ARAI02SUB1Validator:
    ...
    if not config_path:
        ...
        config_path = os.path.join(current_dir, "ARAI02SUB1.json")

    with open(config_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    return ARAI02SUB1Validator(config)
