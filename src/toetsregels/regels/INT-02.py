...
class INT02Validator:
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        for fout in self.foute_voorbeelden:
            if fout.lower() in tekst:
                return False, f"❌ {self.id}: voorwaardelijke formulering aangetroffen (komt precies overeen met fout voorbeeld)", 0.0
        
        # 2️⃣ Expliciete goede voorbeelden daarna
        for goed in self.goede_voorbeelden:
            if goed.lower() in tekst:
                return True, f"✔️ {self.id}: voorbeeldtekst komt overeen met goed voorbeeld (geen beslisregel)", 1.0
        ...
        return True, f"✔️ {self.id}: geen beslisregels of voorwaardelijke formuleringen aangetroffen", 1.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Vermijd voorwaardelijke formuleringen zoals 'indien', 'mits', 'tenzij'")
