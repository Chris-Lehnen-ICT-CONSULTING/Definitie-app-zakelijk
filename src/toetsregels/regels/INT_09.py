...
class INT09Validator:
    ...
    def __init__(self, config: Dict):
        ...
        self.naam = config.get('naam', 'Opsomming in extensionele definitie is limitatief')
        ...
        for pattern in self.herkenbaar_patronen:
            try:
                self.compiled_patterns.append(re.compile(rf"\b{pattern}\b", re.IGNORECASE))
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        if not ongewenste_termen:
            if uitleg_aanwezig:
                return True, f"✅ {self.id}: geen ongewenste termen, komt overeen met goed voorbeeld", 1.0
            else:
                return True, f"✅ {self.id}: geen ongewenste opsommingstermen gevonden", 1.0
        
        # 5️⃣ Wel ongewenste termen gevonden
        termen_str = ", ".join(sorted(ongewenste_termen))
        
        if uitleg_aanwezig:
            return True, f"✅ {self.id}: opsommingswoorden ({termen_str}) voorkomen, maar correct limitatief verwoord", 0.8
        
        if fout_aanwezig:
            return False, f"❌ {self.id}: opsommingswoorden ({termen_str}) lijken op fout voorbeeld", 0.0
        
        return False, f"❌ {self.id}: opsommingswoorden ({termen_str}) zonder duidelijke limitatieve aanduiding", 0.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Vermijd woorden zoals: 'bijvoorbeeld', 'zoals', 'onder andere', 'enz.'")
        hints.append("Gebruik formuleringen als: 'uitsluitend', 'alleen', 'precies deze'")
        hints.append("Als de lijst niet compleet is, gebruik dan een intensionele definitie")
