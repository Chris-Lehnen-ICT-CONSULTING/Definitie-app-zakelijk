...
class ARAI02Validator:
    """Validator voor ARAI02."""
    ...
    def __init__(self, config: dict):
        ...
        Args:
            config: Dictionary met configuratie uit ARAI02.json
        """
        self.config = config
        self.id = config.get("id", "ARAI02")
    ...
    def validate(
        ...
        """
        Valideer definitie volgens ARAI02 regel.
...
def create_validator(config_path: str | None = None) -> ARAI02Validator:
    """
    ...
    if not config_path:
        ...
        config_path = os.path.join(current_dir, "ARAI02.json")

    with open(config_path, encoding="utf-8") as f:
        config = json.load(f)

    return ARAI02Validator(config)
