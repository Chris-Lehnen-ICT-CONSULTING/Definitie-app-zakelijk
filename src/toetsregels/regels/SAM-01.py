...
class SAM01Validator:
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        for goed in self.goede_voorbeelden:
            if goed.lower() in tekst_lc:
                if gevonden_kwalificaties:
                    ...
                    return True, f"✔️ {self.id}: geen misleidende kwalificaties aangetroffen", 1.0
        ...
        return True, f"✔️ {self.id}: geen potentieel misleidende kwalificaties aangetroffen", 1.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Gebruik kwalificaties (technisch, juridisch, etc.) alleen als ze de betekenis verduidelijken")
        hints.append("Zorg dat de definitie consistent blijft met algemeen gebruik van de term")
        hints.append("Vermijd semantisch misleidende kwalificaties")
        hints.append("Als een kwalificatie nodig is, leg dan uit hoe deze de betekenis beïnvloedt")
