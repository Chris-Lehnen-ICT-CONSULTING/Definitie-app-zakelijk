...
class INT01Validator:
    ...
    def __init__(self, config: Dict):
        ...
        self.uitleg = config.get('uitleg', 'Een definitie is compact en in één enkele zin geformuleerd.')
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        goede_aanwezig = any(
            voorbeeld.lower() in definitie_lower
            for voorbeeld in self.goede_voorbeelden
        )
        foute_aanwezig = any(
            voorbeeld.lower() in definitie_lower
            for voorbeeld in self.foute_voorbeelden
        )
        
        # Bepaal resultaat volgens legacy logica
        if not complexiteit_gevonden:
            if goede_aanwezig:
                return True, f"✔️ {self.id}: definitie is compact en komt overeen met goed voorbeeld", 1.0
            else:
                return True, f"✔️ {self.id}: geen complexe elementen herkend – mogelijk goed geformuleerd", 0.9
