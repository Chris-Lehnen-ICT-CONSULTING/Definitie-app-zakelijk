...
class STR01Validator:
    """Validator voor STR-01."""
    ...
    def __init__(self, config: Dict):
        ...
        Args:
            config: Dictionary met configuratie uit STR-01.json
        """
        self.config = config
        self.id = config.get("id", "STR-01")
    ...
    def validate(
        ...
        """
        Valideer definitie volgens STR-01 regel.
...
def create_validator(config_path: str = None) -> STR01Validator:
    ...
    if not config_path:
        ...
        config_path = os.path.join(current_dir, "STR-01.json")

    with open(config_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    return STR01Validator(config)
