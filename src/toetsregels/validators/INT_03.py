...
class INT03Validator:
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        if not gevonden:
            return True, f"✔️ {self.id}: geen onduidelijke voornaamwoord-verwijzing aangetroffen", 1.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Vermijd voornaamwoorden zoals 'deze', 'dit', 'die' zonder duidelijk antecedent")
        hints.append("Als je voornaamwoorden gebruikt, maak dan direct duidelijk waarnaar ze verwijzen")
        hints.append("Herhaal liever het zelfstandig naamwoord dan een onduidelijk voornaamwoord te gebruiken")
        hints.append("Zorg dat de definitie zelfstandig leesbaar is zonder externe context")
