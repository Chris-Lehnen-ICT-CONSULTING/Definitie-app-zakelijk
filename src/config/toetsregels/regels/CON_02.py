...
class CON02Validator:
    ...
    def __init__(self, config: Dict):
        ...
        for pattern in self.bronpatronen_specifiek:
            try:
                ...
                logger.warning(f"Ongeldig specifiek patroon in {self.id}: {pattern} - {e}")
        
        for pattern in self.bronpatronen_algemeen:
            try:
                self.compiled_algemeen.append(re.compile(pattern, re.IGNORECASE))
            except re.error as e:
                logger.warning(f"Ongeldig algemeen patroon in {self.id}: {pattern} - {e}")
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        if not bronnen_gebruikt or not bronnen_gebruikt.strip():
            return False, f"❌ {self.id}: geen opgegeven bronnen gevonden (veld 'bronnen_gebruikt' is leeg of ontbreekt)", 0.0
        ...
        for pattern in self.compiled_specifiek:
            if pattern.search(lc):
                return True, f"✔️ {self.id}: bronvermelding voldoende specifiek → {bg}", 1.0
        
        # 3️⃣ 🟡 check algemene patronen
        for pattern in self.compiled_algemeen:
            if pattern.search(lc):
                return False, f"🟡 {self.id}: bronvermelding aanwezig ({bg}), maar mogelijk te algemeen", 0.5
        
        # 4️⃣ ❌ fallback
        return False, f"❌ {self.id}: bronvermelding gevonden ({bg}), maar niet herkend als authentiek of specifiek", 0.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Voorbeelden: 'art. 3.2 Besluit justitiële gegevens', 'Titel 1.4 Awb'")
        hints.append("Vermijd te algemene verwijzingen zoals alleen 'de AVG' of 'wetgeving'")
