...
class ESS02Validator:
        ...
        for category, pattern_key in pattern_mapping.items():
            ...
            for pattern in patterns:
                try:
                    ...
                    logger.warning(f"Ongeldig regex patroon in {self.id} voor {category}: {pattern} - {e}")
    ...
    def validate(self, definitie: str, begrip: str, context: Optional[Dict] = None) -> Tuple[bool, str, float]:
        ...
        if marker:
            m = marker.lower()
            if m in {"soort", "type"}:
                return True, f"✔️ {self.id}: eenduidig als soort gedefinieerd (via metadata)", 1.0
            if m in {"exemplaar", "particulier"}:
                return True, f"✔️ {self.id}: eenduidig als exemplaar gedefinieerd (via metadata)", 1.0
            if m in {"proces", "activiteit"}:
                return True, f"✔️ {self.id}: eenduidig als proces gedefinieerd (via metadata)", 1.0
            if m in {"resultaat", "uitkomst"}:
                return True, f"✔️ {self.id}: eenduidig als resultaat gedefinieerd (via metadata)", 1.0
            return False, f"❌ {self.id}: ongeldige marker '{marker}'; gebruik soort/exemplaar/proces/resultaat", 0.0
        ...
        for cat, key in categories:
            for voorbeeld in self.config.get(key, []):
                if voorbeeld.lower() in d:
                    return False, (f"❌ {self.id}: expliciet fout voorbeeld voor {cat} gevonden "
                                  f"– vermijd deze formulering"), 0.0
        ...
        if len(hits) == 1:
            ...
            return True, f"✔️ {self.id}: eenduidig als {cat} gedefinieerd ({unieke})", 1.0
        
        # 4️⃣ Meerdere categorieën → ❌ ambiguïteit
        if len(hits) > 1:
            found = ", ".join(sorted(hits.keys()))
            return False, (f"❌ {self.id}: ambiguïteit – meerdere categories herkend ({found}); "
                          "kies één betekenislaag"), 0.0
        ...
        for cat, key in good_keys.items():
            for voorbeeld in self.config.get(key, []):
                if voorbeeld.lower() in d:
                    return True, f"✔️ {self.id}: eenduidig als {cat} gedefinieerd (voorbeeld match)", 1.0
        
        # 6️⃣ Fallback → geen marker
        return False, (f"❌ {self.id}: geen duidelijke ontologische marker "
                      "(type, particulier, proces of resultaat) gevonden"), 0.0
    ...
    def get_generation_hints(self) -> List[str]:
        ...
        hints.append("Geef duidelijk aan of het begrip een type, particulier, proces of resultaat is")
