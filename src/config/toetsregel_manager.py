...
class ToetsregelManager:
    ...
    def _load_configuration(self):
        ...
        try:
            if self.config_file.exists():
                ...
                logger.warning(f"Configuratie bestand niet gevonden: {self.config_file}")
    ...
    def get_regels_voor_prioriteit(self, prioriteit: RegelPrioriteit) -> List[Dict[str, Any]]:
    ...
    def validate_regel(self, regel_data: Dict[str, Any]) -> List[str]:
        ...
        toegestane_prioriteiten = config.get('validatie', {}).get('toegestane_prioriteiten', [])
        ...
        toegestane_aanbevelingen = config.get('validatie', {}).get('toegestane_aanbevelingen', [])
        if aanbeveling and aanbeveling not in toegestane_aanbevelingen:
            errors.append(f"Ongeldige aanbeveling: {aanbeveling}")
        
        return errors
    
    def create_custom_set(self, naam: str, regel_ids: List[str], beschrijving: str = "") -> RegelSet:
