...
class APIConfigAdapter(ConfigAdapter):
    ...
    def get_model_config(self, model: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            'model': model,
            'temperature': model_settings.get('temperature', self.config.default_temperature),
            'max_tokens': model_settings.get('max_tokens', self.config.default_max_tokens),
            'cost_per_token': model_settings.get('cost_per_token', 0.0)
        }
    
    def get_gpt_call_params(self, model: Optional[str] = None, **overrides) -> Dict[str, Any]:
