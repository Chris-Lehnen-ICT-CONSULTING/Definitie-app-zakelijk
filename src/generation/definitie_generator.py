...
try:
    ...
    logger.warning(
        "Hybrid context module niet beschikbaar - fallback naar standaard generatie"
    )
...
class GenerationContext:
    ...
    hybrid_context: Optional[Any] = (
        None  # HybridContext object voor uitgebreide context
    )
    ...
    document_context: Optional[Dict[str, Any]] = (
        None  # Context uit geüploade documenten
    )
...
class DefinitieGenerator:
    ...
    def __init__(self):
        ...
        self.category_templates = {
            ...
            OntologischeCategorie.EXEMPLAAR: {
                ...
                "focus": [
                    "specifieke_identificatie",
                    "categorie_behorend",
                    "unieke_eigenschappen",
                ],
            },
        }
    ...
    def generate(
        ...
        """
        logger.info(
            f"Generating definitie voor '{context.begrip}' categorie {context.categorie.value}"
        )
        ...
        logger.info(
            f"Definitie opgeschoond: {len(definitie)} -> {len(opgeschoonde_definitie)} karakters"
        )
    ...
    def _load_generation_instructions(
        self, context: GenerationContext
    ) -> List[GenerationInstruction]:
        """
        Laad en converteer relevante toetsregels naar generatie instructies.
        """
        instructies = []

        try:
            # Haal alle relevante regels op voor deze categorie
            relevante_regels = self.rule_manager.get_regels_voor_categorie(
                context.categorie.value
            )

            # Interpreteer elke regel als generatie instructie
            for regel in relevante_regels:
                if regel.get("gebruik_voor_generatie", True):  # Default: gebruik regel
                    instructie = self.interpreter.for_generation(regel)
                    instructies.append(instructie)

            # Voeg feedback-gebaseerde instructies toe indien aanwezig
            if context.feedback_history:
                for feedback in context.feedback_history:
                    if feedback.instructie:
                        instructies.append(
                            GenerationInstruction(
                                rule_id=f"FEEDBACK_{feedback.bron}",
                                guidance=feedback.instructie,
                                priority="hoog",
                            )
                        )
            ...
            # Fallback naar basis instructies
            instructies = [
                GenerationInstruction(
                    rule_id="DEFAULT",
    ...
    def _call_gpt(
        ...
        logger.info(
            f"Calling GPT with model {gpt_params['model']}, temp {gpt_params['temperature']} (override was {temperature})"
        )
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Je bent een expert in het schrijven van definitieven Nederlandse definities voor juridische en beleidsbegrippen.",
                    },
    ...
    def generate_with_feedback(
        self, context: GenerationContext, max_iterations: int = 3
    ) -> GenerationResult:
        """
    ...
    def _enhance_with_hybrid_context(
        self, context: GenerationContext
    ) -> GenerationContext:
        """
        ...
        try:
            ...
            logger.info(
                f"Hybrid context enhanced (confidence: {context.hybrid_context.confidence_score:.2f})"
            )
            return context
    ...
    def _build_generation_prompt(
        self, context: GenerationContext, instructies: List[GenerationInstruction]
    ) -> str:
        """
        ...
        if category_template:
            ...
            prompt_sections.append(
                f"Basis structuur: {category_template.get('base_structure', '')}"
            )
            prompt_sections.append(
                f"Focus op: {', '.join(category_template.get('focus', []))}"
            )
        ...
        if instructies:
            ...
            for instructie in instructies:
                ...
                if instructie.focus_areas:
                    prompt_sections.append(
                        f"  Focus: {', '.join(instructie.focus_areas)}"
                    )
        ...
        prompt_sections.append(
            f"Organisatorische context: {context.organisatorische_context}"
        )
        ...
        prompt_sections.append("\n=== OPDRACHT ===")
        prompt_sections.append(
            f"Genereer een definitie voor '{context.begrip}' volgens bovenstaande richtlijnen."
        )
        ...
        if context.hybrid_context:
            ...
            if confidence > 0.8:
                prompt_sections.append(
                    "- Integreer de rijke context uit bronnen optimaal"
                )
            elif confidence > 0.6:
                prompt_sections.append("- Gebruik beschikbare context waar relevant")
            else:
                prompt_sections.append(
                    "- Gebruik context zorgvuldig, valideer waar mogelijk"
                )
        ...
        if context_verboden:
            ...
            prompt_sections.append(
                "❌ De opgegeven context termen mogen NIET letterlijk in de definitie voorkomen!"
            )
    ...
    def _genereer_context_verboden(self, context: GenerationContext) -> List[str]:
        ...
        if verboden:
            verboden.insert(
                0,
                "🚫 CONTEXT VERBODEN (gebruik deze termen NIET letterlijk in de definitie):",
            )
            verboden.append(
                "❌ Context en bronnen mogen NIET letterlijk of herleidbaar in de definitie voorkomen!"
            )
    ...
    def _voeg_contextverbod_toe(self, verboden: List[str], term: str):
        ...
        for kandidaat in kandidaten:
            ...
            if kandidaat and sleutel not in self.geziene_context_termen:
                verboden.append(
                    f"• Gebruik de term '{kandidaat}' of varianten daarvan NIET in de definitie"
                )
    ...
    def _build_hybrid_context_section(self, hybrid_context) -> str:
        ...
        try:
            ...
            sections.append(
                f"Betrouwbaarheidscore: {hybrid_context.confidence_score:.2f}"
            )
            ...
            if hybrid_context.web_sources:
                all_sources.extend(
                    [f"Web: {src}" for src in hybrid_context.web_sources]
                )
            if hybrid_context.document_sources:
                all_sources.extend(
                    [
                        f"Doc: {src.get('filename', 'onbekend')}"
                        for src in hybrid_context.document_sources
                    ]
                )
            ...
            if all_sources:
                sections.append("\n=== BRONNEN ===")
                sections.append(
                    f"Primaire bronnen: {', '.join(hybrid_context.primary_sources)}"
                )
                if hybrid_context.supporting_sources:
                    sections.append(
                        f"Ondersteunende bronnen: {', '.join(hybrid_context.supporting_sources)}"
                    )
            ...
            if hybrid_context.confidence_score > 0.8:
                sections.append(
                    "Gebruik deze rijke context optimaal voor een uitgebreide, goed onderbouwde definitie."
                )
            elif hybrid_context.confidence_score > 0.6:
                sections.append(
                    "Integreer relevante elementen uit deze context in je definitie."
                )
            else:
                sections.append(
                    "Gebruik deze context ondersteunend, maar blijf kritisch op relevantie."
                )

            if hybrid_context.conflicts_resolved > 0:
                sections.append(
                    f"Let op: {hybrid_context.conflicts_resolved} conflict(en) zijn opgelost in deze context."
                )
...
def generate_definitie(
    begrip: str, organisatorische_context: str, categorie: str = "type", **kwargs
) -> str:
    """
