...
class ContextFusion:
    ...
    def fuse_contexts(
        ...
        try:
            ...
            context_analysis = self._analyze_available_context(
                web_context, document_context
            )
            ...
            quality_assessment = self._assess_fusion_quality(
                fusion_result, context_analysis
            )
            ...
            logger.info(
                f"Context fusion voltooid voor '{begrip}' (strategy: {strategy_name}, quality: {quality_assessment})"
            )
            return result
    ...
    def _assess_document_context_quality(self, document_context: Dict[str, Any]) -> str:
        ...
        if (
            doc_count >= 2
            and total_length > 5000
            and keyword_count > 15
            and concept_count > 5
        ):
            return "high"
    ...
    def _find_contradictory_terms(
        ...
        for negative_pattern, positive_pattern in conflicting_patterns:
            ...
            if web_has_negative and doc_has_positive:
                return {
                    "web_terms": [
                        term for term in web_terms if term in negative_pattern
                    ],
                    "doc_terms": [
                        term for term in doc_terms if term in positive_pattern
                    ],
                }
    ...
    def _balanced_merge_fusion(
        ...
        if document_context.get("document_count", 0) > 0:
            ...
            if concepts:
                sections.append(
                    f"Geïdentificeerde concepten: {', '.join(concepts[:5])}"
                )
        ...
        for source_name, source_data in web_context.items():
            if isinstance(source_data, dict) and "tekst" in source_data:
                ...
                content = (
                    source_data["tekst"][:300] + "..."
                    if len(source_data["tekst"]) > 300
                    else source_data["tekst"]
                )
    ...
    def _balanced_with_resolution_fusion(
        ...
        if conflicts:
            ...
            for conflict in conflicts:
                ...
                if conflict["type"] == "terminology_conflict":
                    resolution_section += (
                        " (Documenten krijgen prioriteit voor specifieke termen)"
                    )
        ...
        base_result.confidence_score = max(
            0.7, base_result.confidence_score - 0.1 * len(conflicts)
        )
    ...
    def _web_primary_fusion(
        ...
        for source_name, source_data in web_context.items():
            if isinstance(source_data, dict) and "tekst" in source_data:
                web_sources.append(source_name)
                content = (
                    source_data["tekst"][:400] + "..."
                    if len(source_data["tekst"]) > 400
                    else source_data["tekst"]
                )
                sections.append(f"\n[{source_name.upper()}]: {content}")
    ...
    def _doc_primary_fusion(
        ...
        if document_context.get("document_count", 0) > 0:
            ...
            if concepts:
                sections.append(
                    f"Geïdentificeerde concepten: {', '.join(concepts[:8])}"
                )
            if legal_refs:
        ...
        for source_name, source_data in web_context.items():
            if isinstance(source_data, dict) and "tekst" in source_data:
                web_sources.append(source_name)
                content = (
                    source_data["tekst"][:200] + "..."
                    if len(source_data["tekst"]) > 200
                    else source_data["tekst"]
                )
                sections.append(f"\n[{source_name}]: {content}")
    ...
    def _assess_fusion_quality(
        ...
        # Source diversity
        total_sources = len(fusion_result.web_sources) + len(
            fusion_result.primary_sources
        )
        if total_sources >= 4:
            score += 2
    ...
    def _create_fallback_fusion(
        ...
        if document_context.get("document_count", 0) > 0:
            fallback_content += (
                f" {document_context['document_count']} document(en) beschikbaar."
            )
