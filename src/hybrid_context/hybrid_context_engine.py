...
class HybridContextEngine:
    ...
    def create_hybrid_context(
        ...
        try:
            ...
            logger.info(f"Hybride context succesvol aangemaakt (confidence: {hybrid_context.confidence_score:.2f})")
            ...
            return self._create_fallback_context(begrip, organisatorische_context, juridische_context)
    
    def _get_document_context(self, selected_document_ids: Optional[List[str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'document_count': 0, 'aggregated_keywords': [], 'document_sources': []}
    ...
    def _execute_enhanced_web_lookup(
        ...
        try:
            ...
            enhanced_results = self._enhance_web_results(web_results, document_keywords, source_strategy)
    ...
    def _enhance_web_results(
        ...
        for source_name, source_data in enhanced_results.items():
            if isinstance(source_data, dict) and 'tekst' in source_data:
                ...
                relevance_score = self._calculate_relevance_score(source_text, document_keywords)
                ...
                enhanced_results[source_name]['enhanced_by_documents'] = relevance_score > 0.3
        
        # Voeg metadata toe over enhancement
        enhanced_results['_enhancement_metadata'] = {
            'document_keywords_used': len(document_keywords),
            'sources_enhanced': sum(1 for k, v in enhanced_results.items() 
    ...
    def _calculate_relevance_score(self, source_text: str, document_keywords: List[str]) -> float:
    ...
    def get_context_summary(self, hybrid_context: HybridContext) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_sources': len(hybrid_context.web_sources) + len(hybrid_context.document_sources),
    ...
    def _determine_enhancement_level(self, context: HybridContext) -> str:
        ...
        if context.document_context.get('document_count', 0) > 0 and len(context.web_sources) > 0:
