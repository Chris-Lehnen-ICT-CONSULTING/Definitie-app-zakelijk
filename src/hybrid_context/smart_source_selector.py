...
class SmartSourceSelector:
    ...
    def select_optimal_sources(
        ...
        try:
            ...
            context_analysis = self._analyze_formal_context(organisatorische_context, juridische_context)
            ...
            logger.info(f"Source strategy '{strategy.strategy_name}' geselecteerd voor '{begrip}'")
            return strategy
    ...
    def _analyze_document_context(self, document_context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _classify_document_type(self, keywords: List[str], concepts: List[str], legal_refs: List[str]) -> str:
        """Classificeer type document op basis van content."""
        # Legal document indicators
        legal_indicators = ['wet', 'artikel', 'wetgeving', 'juridisch', 'recht', 'besluit', 'verordening']
        legal_score = sum(1 for keyword in keywords if any(ind in keyword.lower() for ind in legal_indicators))
        
        # Policy document indicators  
        policy_indicators = ['beleid', 'procedure', 'proces', 'richtlijn', 'protocol', 'handboek']
        policy_score = sum(1 for keyword in keywords if any(ind in keyword.lower() for ind in policy_indicators))
        
        # Technical document indicators
        tech_indicators = ['systeem', 'technisch', 'implementatie', 'architectuur', 'specificatie']
        tech_score = sum(1 for keyword in keywords if any(ind in keyword.lower() for ind in tech_indicators))
        
        # Definitional document indicators
        def_indicators = ['definitie', 'betekenis', 'omschrijving', 'begrip', 'concept']
        def_score = sum(1 for keyword in keywords if any(ind in keyword.lower() for ind in def_indicators))
    ...
    def _assess_legal_complexity(self, legal_refs: List[str], keywords: List[str]) -> str:
        """Beoordeel juridische complexiteit van content."""
        if len(legal_refs) >= 5:
            return 'high'
        elif len(legal_refs) >= 2:
            return 'medium'
        elif len(legal_refs) >= 1 or any('juridisch' in kw.lower() or 'wet' in kw.lower() for kw in keywords):
            return 'low'
        else:
            return 'none'
    ...
    def _identify_topic_focus(self, keywords: List[str], concepts: List[str]) -> str:
        ...
        # Strafrecht focus
        strafrecht_terms = ['straf', 'politie', 'openbaar', 'ministerie', 'gevangenis', 'reclassering']
        strafrecht_score = sum(1 for term in strafrecht_terms if any(term in kw.lower() for kw in keywords))
        
        # Identiteit focus
        identiteit_terms = ['identiteit', 'authenticatie', 'verificatie', 'identiteitsbewijs', 'identificatie']
        identiteit_score = sum(1 for term in identiteit_terms if any(term in kw.lower() for kw in keywords))
        
        # Administratief focus
        admin_terms = ['administratie', 'registratie', 'beheer', 'systeem', 'gegevens']
        admin_score = sum(1 for term in admin_terms if any(term in kw.lower() for kw in keywords))
    ...
    def _extract_keyword_themes(self, keywords: List[str]) -> List[str]:
        ...
        # Security themes
        if any('beveiliging' in kw.lower() or 'veiligheid' in kw.lower() for kw in keywords):
    ...
    def _recommend_sources_for_content(
        ...
        if 'legal' in themes and any(term in str(themes) for term in ['european', 'international']):
    ...
    def _create_source_strategy(
        ...
        reasoning_parts = []
        if doc_analysis['has_documents']:
            reasoning_parts.append(f"Document context beschikbaar ({doc_analysis['document_type']} type)")
        if doc_analysis.get('legal_complexity', 'none') != 'none':
            reasoning_parts.append(f"Juridische complexiteit: {doc_analysis['legal_complexity']}")
        if term_analysis['term_type'] != 'general':
            reasoning_parts.append(f"Term type: {term_analysis['term_type']}")
            
        reasoning = "; ".join(reasoning_parts) if reasoning_parts else "Standaard bron selectie"
        
        return SourceStrategy(
            strategy_name=strategy_name,
            priority_sources=list(dict.fromkeys(priority_sources)),  # Remove duplicates, keep order
