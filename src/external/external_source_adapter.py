...
class ExternalDefinition:
    ...
    def to_definitie_record(self, source_config: ExternalSourceConfig) -> DefinitieRecord:
...
class ExternalSourceAdapter(ABC):
    ...
    @abstractmethod
    def update_definition(self, external_id: str, definition: ExternalDefinition) -> bool:
        """
...
class MockExternalAdapter(ExternalSourceAdapter):
    ...
    def search_definitions(
        ...
        for definition in self._mock_data:
            # Apply filters
            if query and query.lower() not in definition.begrip.lower() and query.lower() not in definition.definitie.lower():
                continue
...
class FileSystemAdapter(ExternalSourceAdapter):
    ...
    def search_definitions(self, query: str = None, categorie: str = None, 
                         context: str = None, limit: int = 100) -> List[ExternalDefinition]:
    ...
    def get_definition(self, external_id: str) -> Optional[ExternalDefinition]:
        ...
        # Placeholder - zou echte file parsing implementeren
        logger.info(f"FileSystemAdapter: get_definition({external_id}) not fully implemented")
        return None
    ...
    def update_definition(self, external_id: str, definition: ExternalDefinition) -> bool:
        ...
        logger.info(f"FileSystemAdapter: update_definition({external_id}) not fully implemented")
        return False
    
    def delete_definition(self, external_id: str) -> bool:
        """Verwijder definitie uit bestand."""
        # Placeholder - zou echte file updating implementeren
        logger.info(f"FileSystemAdapter: delete_definition({external_id}) not fully implemented")
        return False
...
def create_mock_source(source_name: str = "Mock External Source") -> MockExternalAdapter:
