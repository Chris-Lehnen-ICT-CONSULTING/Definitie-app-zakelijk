...
class DefinitieManagerCLI:
    ...
    def cmd_list(self, args):
        ...
        for i, definitie in enumerate(definities, 1):
            ...
            print(f"   Score: {definitie.validation_score:.2f}" if definitie.validation_score else "   Score: N/A")
    ...
    def cmd_show(self, args):
        ...
        print(f"\n📅 METADATA:")
        print(f"Aangemaakt: {definitie.created_at} door {definitie.created_by or 'Unknown'}")
        print(f"Gewijzigd: {definitie.updated_at} door {definitie.updated_by or 'Unknown'}")
        ...
        if issues:
            print(f"\n⚠️ VALIDATION ISSUES:")
            for issue in issues:
                print(f"   - {issue.get('rule_id', 'Unknown')}: {issue.get('description', 'No description')}")
    ...
    def cmd_check(self, args):
        ...
        if check_result.duplicates:
            ...
            for i, dup in enumerate(check_result.duplicates[:3], 1):
                print(f"   {i}. {dup.definitie_record.begrip} (Score: {dup.match_score:.2f})")
                print(f"      ID: {dup.definitie_record.id}, Status: {dup.definitie_record.status}")
    ...
    def cmd_status(self, args):
        ...
        if success:
            print(f"✅ Status van definitie {args.id} gewijzigd naar {new_status.value}")
        else:
    ...
    def cmd_import(self, args):
        ...
        successful, failed, errors = self.repository.import_from_json(
            args.file, 
            args.imported_by or "cli_user"
        )
        
        print(f"📥 IMPORT RESULTATEN:")
...
Voorbeelden:
  ...
  %(prog)s import --file externe_definities.json
    ...
    list_parser.add_argument('--status', choices=[s.value for s in DefinitieStatus], help='Filter op status')
    list_parser.add_argument('--categorie', choices=[c.value for c in OntologischeCategorie], help='Filter op categorie')
    list_parser.add_argument('--context', help='Filter op organisatorische context')
    list_parser.add_argument('--limit', type=int, default=20, help='Maximum aantal resultaten')
    ...
    check_parser.add_argument('--context', required=True, help='Organisatorische context')
    check_parser.add_argument('--juridische-context', help='Juridische context')
    check_parser.add_argument('--categorie', required=True, choices=[c.value for c in OntologischeCategorie], help='Ontologische categorie')
    
    # Generate command
    generate_parser = subparsers.add_parser('generate', help='Genereer nieuwe definitie')
    generate_parser.add_argument('--begrip', required=True, help='Te definiëren begrip')
    generate_parser.add_argument('--context', required=True, help='Organisatorische context')
    generate_parser.add_argument('--juridische-context', help='Juridische context')
    generate_parser.add_argument('--categorie', required=True, choices=[c.value for c in OntologischeCategorie], help='Ontologische categorie')
    generate_parser.add_argument('--force', action='store_true', help='Forceer nieuwe generatie ondanks duplicates')
    ...
    approve_parser.add_argument('--approved-by', required=True, help='Wie de definitie goedkeurt')
    ...
    status_parser.add_argument('--status', required=True, choices=[s.value for s in DefinitieStatus], help='Nieuwe status')
    ...
    export_parser.add_argument('--status', choices=[s.value for s in DefinitieStatus], help='Filter op status')
    export_parser.add_argument('--context', help='Filter op organisatorische context')
    export_parser.add_argument('--categorie', choices=[c.value for c in OntologischeCategorie], help='Filter op categorie')
