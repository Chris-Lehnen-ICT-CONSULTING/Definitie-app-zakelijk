...
def setup_database(db_path: str = None, include_test_data: bool = True):
    ...
    if include_test_data:
        ...
        for record in test_records:
            try:
                ...
                print(f"✅ Toegevoegd: '{record.begrip}' (ID: {record_id}) - {record.status}")
                
                # Set approval als established
                if record.status == DefinitieStatus.ESTABLISHED.value and record.approved_by:
    ...
    print(f"\n💡 CLI VOORBEELDEN:")
    print(f"   python tools/definitie_manager.py list")
    print(f"   python tools/definitie_manager.py check --begrip authenticatie --context DJI --categorie proces")
    print(f"   python tools/definitie_manager.py generate --begrip controle --context OM --categorie proces")
    print(f"   python tools/definitie_manager.py stats")


def create_sample_export():
    """Maak voorbeeld export bestand."""
    export_path = Path(__file__).parent.parent.parent / "exports" / "sample_definitions.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-test-data", action="store_true", help="Geen test data toevoegen")
    parser.add_argument("--export-sample", action="store_true", help="Maak sample export")
