...
class WorkflowService:
    ...
    def can_change_status(self, 
        ...
        if new_status == DefinitionStatus.ARCHIVED.value:
            if user_role and user_role not in self.ROLE_PERMISSIONS.get("archive", []):
                logger.warning(
                    f"User with role '{user_role}' cannot archive"
                )
                return False
        
        if (current_status == DefinitionStatus.ARCHIVED.value and 
            new_status == DefinitionStatus.DRAFT.value):
            if user_role and user_role not in self.ROLE_PERMISSIONS.get("restore_from_archive", []):
                logger.warning(
    ...
    def get_allowed_transitions(self, current_status: str, user_role: Optional[str] = None) -> List[str]:
        """
    ...
    def can_edit_definition(self, status: str, user_role: Optional[str] = None) -> bool:
        """
    ...
    def get_workflow_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_statuses": [DefinitionStatus.ESTABLISHED.value, DefinitionStatus.ARCHIVED.value]
        }