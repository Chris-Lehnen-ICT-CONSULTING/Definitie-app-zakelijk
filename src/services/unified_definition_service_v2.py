...
class UnifiedDefinitionService(LegacyService):
    ...
    def configure(
        self, config: Union[UnifiedServiceConfig, UnifiedServiceConfigV2]
    ) -> None:
        """
        ...
        self.logger.info(
            f"Service V2 geconfigureerd: mode={self.config_v2.service_mode.value}"
        )
    ...
    def _get_service_mode(self) -> ServiceMode:
        ...
        if self.config_v2.service_mode != ServiceMode.AUTO:
            ...
            self._service_mode_cache = (
                ServiceMode.NEW_SERVICES if use_new else ServiceMode.LEGACY
            )
    ...
    def _generate_direct(
        ...
        **kwargs,
        ...
        asyncio.set_event_loop(loop)
        try:
            response = loop.run_until_complete(orchestrator.create_definition(request))
        finally:
            loop.close()
        ...
        if response.success and response.definition:
            result = UnifiedResult(
                ...
                voorbeelden=(
                    {"voorbeelden": response.definition.voorbeelden}
