...
class EnhancementResult:
    ...
    def improvement_score(self) -> float:
        ...
        length_factor = abs(len(self.enhanced_text) - len(self.original_text)) / len(
            self.original_text
        )
...
class ContextIntegrationEnhancer(EnhancementStrategy):
    ...
    def enhance(self, definition: Definition) -> list[EnhancementResult]:
        ...
        # Context integratie (legacy-compat context veld kan ontbreken in V2)
        ctx = getattr(definition, "context", None)
        if ctx and "context" not in text.lower():
            enhanced_text = self._integrate_context(text, ctx)
            if enhanced_text != text:
                results.append(
                    EnhancementResult(
...
class CompletenessEnhancer(EnhancementStrategy):
    ...
    def enhance(self, definition: Definition) -> list[EnhancementResult]:
        ...
        if len(missing_aspects) >= 2:
            enhanced_text = self._add_completeness_hints(
                definition.definitie, missing_aspects, definition.begrip
            )
    ...
    def _add_completeness_hints(
        self, text: str, missing_aspects: list[str], begrip: str
    ) -> str:
        ...
        if "scope" in missing_aspects:
            addition = (
                f" Het toepassingsgebied van {begrip.lower()} is duidelijk afgebakend."
            )
            return text + addition
        ...
        for improvement_type, patterns in self.linguistic_patterns.items():
            ...
            for pattern, replacement in patterns:
                if isinstance(replacement, str) and "(" in pattern:
                    ...
                    # Simple replacement
                    new_text = enhanced_text.replace(
                        pattern.replace(r"\b", ""), replacement
                    )
                    if new_text != enhanced_text:
                        enhanced_text = new_text
                        changes_made += 1
...
class DefinitionEnhancer:
    ...
    def enhance_definition(
        self, definition: Definition, max_enhancements: int = 3
    ) -> tuple[Definition, list[EnhancementResult]]:
        """
        ...
        for strategy in self.strategies:
            try:
                ...
                logger.warning(
                    f"Enhancement strategy {strategy.get_strategy_name()} failed: {e}"
                )
        ...
        for enhancement in all_enhancements[:max_enhancements]:
            if enhancement.confidence >= self.config.enhancement_confidence_threshold:
                ...
                if enhanced_def != current_definition:
                    ...
                    logger.debug(
                        f"Applied {enhancement.enhancement_type.value} enhancement "
        ...
        if applied_enhancements:
            ...
            enhanced_metadata["enhancement_confidence"] = [
                e.confidence for e in applied_enhancements
            ]
    ...
    def _apply_enhancement(
        self, definition: Definition, enhancement: EnhancementResult
    ) -> Definition:
        ...
        for strategy in self.strategies:
            try:
                ...
                if enhancements:
                    ...
                    avg_confidence = sum(e.confidence for e in enhancements) / len(
                        enhancements
                    )
                    quality_scores[strategy_name] = (
                        1.0 - avg_confidence
                    )  # Higher confidence = lower quality score
                ...
                logger.warning(
                    f"Quality evaluation failed for {strategy.get_strategy_name()}: {e}"
                )
