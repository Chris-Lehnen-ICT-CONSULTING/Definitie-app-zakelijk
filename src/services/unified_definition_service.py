...
try:
    ...
    logger.warning(
        "Moderne architectuur modules niet beschikbaar - gebruik legacy mode"
    )
...
class UnifiedDefinitionService:
    ...
    def configure(self, config: UnifiedServiceConfig) -> None:
        ...
        self.config = config
        self.logger.info(
            f"Service geconfigureerd: processing={config.processing_mode.value}, architecture={config.architecture_mode.value}"
        )
    ...
    def generate_definition(
        ...
        **kwargs,
        ...
        if force_async or (
            self.config.processing_mode == ProcessingMode.ASYNC and not force_sync
        ):
            return asyncio.run(
                self.agenerate_definition(begrip, context_dict, **kwargs)
            )
        else:
            return self._generate_definition_sync(begrip, context_dict, **kwargs)

    async def agenerate_definition(
        self, begrip: str, context_dict: Dict[str, List[str]], **kwargs
    ) -> UnifiedResult:
        ...
        try:
            ...
            if self.config.progress_callback:
                self.config.progress_callback(
                    "Definitie gegenereerd, validatie gestart...", 0.4
                )

            # Validatie
            if self.config.enable_validation:
                validation_result = await self._validate_definition_async(
                    result.definitie_gecorrigeerd, begrip, context_dict
                )
            ...
            if self.config.enable_examples:
                ...
                voorbeelden = await genereer_alle_voorbeelden_async(
                    begrip, result.definitie_gecorrigeerd, context_dict
                )
            ...
            if len(tasks) > 1:
                parallel_results = await asyncio.gather(
                    *tasks[1:], return_exceptions=True
                )
    ...
    def _generate_definition_sync(
        ...
        try:
            # Genereer definitie
            definitie_origineel, definitie_gecorrigeerd, marker = (
                self._generate_base_definition_sync(begrip, context_dict)
            )
            ...
            if self.config.enable_validation:
                toetsresultaten, score = self._validate_definition_sync(
                    definitie_gecorrigeerd, begrip, context_dict
                )
    ...
    async def _validate_definition_async(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._validate_definition_sync, definitie, begrip, context_dict
        )
    ...
    def _lookup_sources_sync(
        ...
        try:
            ...
            bronnen_tekst = "\n".join(
                [f"- {bron['titel']} ({bron['type']})" for bron in bronnen[:5]]
            )
    ...
    async def _lookup_sources_async(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._lookup_sources_sync, begrip, context_dict
        )
    ...
    def _update_session_state(self, result: UnifiedResult) -> None:
        ...
        SessionStateManager.update_definition_results(
            result.definitie_origineel, result.definitie_gecorrigeerd, result.marker
        )
        ...
        if result.voorbeelden:
            SessionStateManager.set_value(
                "voorbeeld_zinnen", result.voorbeelden.get("sentence", [])
            )
            SessionStateManager.set_value(
                "praktijkvoorbeelden", result.voorbeelden.get("practical", [])
            )
            SessionStateManager.set_value(
                "tegenvoorbeelden", result.voorbeelden.get("counter", [])
            )
            SessionStateManager.set_value(
            ...
            SessionStateManager.set_value(
                "toelichting", explanation[0] if explanation else ""
            )
...
class DefinitionService(UnifiedDefinitionService):
    ...
    def validate_definition(
        ...
        toetsresultaten, _ = self._validate_definition_sync(
...
def get_definition_service(
    mode: Union[str, ProcessingMode] = ProcessingMode.AUTO, **kwargs
