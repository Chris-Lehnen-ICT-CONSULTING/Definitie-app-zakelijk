...
class ABTestingFramework:
    ...
    async def run_comparison(
        self, 
        terms: List[str], 
        variant: TestVariant = TestVariant.BOTH
        ...
        """
        logger.info(f"Starting A/B test voor {len(terms)} termen (variant: {variant.value})")
    ...
    async def _test_modern_service(
        ...
        performance = PerformanceMetrics(
            ...
            success_rate=(self.config.repeat_count - errors - timeouts) / self.config.repeat_count,
    ...
    def _calculate_quality_metrics(
        ...
        if any(word in term.lower() for word in ["wet", "artikel", "recht", "juridisch"]):
    ...
    def _generate_recommendation(
        ...
        if comparison.modern_performance.response_time > 0 and comparison.legacy_performance.response_time > 0:
        ...
        if comparison.modern_performance.success_rate > comparison.legacy_performance.success_rate:
            scores["modern"] += 3.0
            notes.append("Modern has higher success rate")
        elif comparison.legacy_performance.success_rate > comparison.modern_performance.success_rate:
            scores["legacy"] += 2.0
            notes.append("Legacy has higher success rate")
        
        # Results count comparison
        if comparison.modern_performance.results_count > comparison.legacy_performance.results_count:
            scores["modern"] += 1.0
            notes.append("Modern provides more results")
        elif comparison.legacy_performance.results_count > comparison.modern_performance.results_count:
            scores["legacy"] += 1.0
            notes.append("Legacy provides more results")
        
        # Quality comparison (if available)
        if hasattr(comparison, 'modern_quality') and hasattr(comparison, 'legacy_quality'):
            if comparison.modern_quality.relevance_score > comparison.legacy_quality.relevance_score:
                scores["modern"] += 2.0
                notes.append("Modern has higher relevance")
            elif comparison.legacy_quality.relevance_score > comparison.modern_quality.relevance_score:
                scores["legacy"] += 1.0
                notes.append("Legacy has higher relevance")
                
            if comparison.modern_quality.completeness_score > comparison.legacy_quality.completeness_score:
                scores["modern"] += 1.5
                notes.append("Modern has better completeness")
            elif comparison.legacy_quality.completeness_score > comparison.modern_quality.completeness_score:
                scores["legacy"] += 1.0
    ...
    def generate_report(self, results: List[ComparisonResult]) -> str:
        ...
        report_lines.append(f"  Modern preferred: {modern_count} ({modern_count/len(results)*100:.1f}%)")
        report_lines.append(f"  Legacy preferred: {legacy_count} ({legacy_count/len(results)*100:.1f}%)")
        report_lines.append(f"  Both equivalent: {both_count} ({both_count/len(results)*100:.1f}%)")
        report_lines.append("")
        
        # Performance summary
        modern_times = [r.modern_performance.response_time for r in results if r.modern_performance.response_time > 0]
        legacy_times = [r.legacy_performance.response_time for r in results if r.legacy_performance.response_time > 0]
        ...
        if modern_times and legacy_times:
            ...
            report_lines.append(f"  Performance improvement: {((avg_legacy - avg_modern) / avg_legacy * 100):.1f}%")
            report_lines.append("")
        ...
        for result in results:
            ...
            report_lines.append(f"  Recommendation: {result.recommendation.value} (confidence: {result.confidence:.2f})")
            report_lines.append(f"  Modern: {result.modern_performance.results_count} results, {result.modern_performance.response_time:.2f}s")
            report_lines.append(f"  Legacy: {result.legacy_performance.results_count} results, {result.legacy_performance.response_time:.2f}s")
