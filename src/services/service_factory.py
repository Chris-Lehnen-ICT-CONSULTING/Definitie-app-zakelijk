...
def get_definition_service(use_container_config: Optional[dict] = None) -> Union['UnifiedDefinitionService', 'ServiceAdapter']:
    """
    ...
    async def generate_definition(self, begrip: str, context_dict: dict, **kwargs):
        ...
        if response.success and response.definition:
            return {
                ...
                'toetsresultaten': response.validation.errors if response.validation else [],
                'validation_score': response.validation.score if response.validation else 0.0,
