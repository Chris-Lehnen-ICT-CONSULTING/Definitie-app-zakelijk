...
class DefinitionOrchestrator(DefinitionOrchestratorInterface):
    ...
    async def validate_and_save(self, definition: Definition) -> DefinitionResponse:
        ...
        try:
            ...
            # Check kwaliteit
            if validation_result.score < self.config.min_quality_score and not self.config.save_drafts:
                return DefinitionResponse(
                    definition=definition,
                    validation=validation_result,
                    success=False,
    ...
    async def _validate_definition(self, context: ProcessingContext) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                errors=[f"Validatie mislukt: {str(e)}"],
                score=0.0
            )
    ...
    async def _enrich_with_ai(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            if enhanced.gerelateerde_begrippen and not context.definition.gerelateerde_begrippen:
                context.definition.gerelateerde_begrippen = enhanced.gerelateerde_begrippen
    ...
    async def _save_definition(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            if context.validation_result:
                ...
                context.definition.metadata['quality_score'] = context.validation_result.score
            
            # Bepaal status
            if context.validation_result and context.validation_result.score >= 0.8:
                context.definition.metadata['status'] = 'established'
            elif context.validation_result and context.validation_result.score >= self.config.min_quality_score:
