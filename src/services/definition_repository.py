...
class DefinitionRepository(DefinitionRepositoryInterface):
    ...
    def search(self, query: str, limit: int = 10) -> List[Definition]:
        ...
        try:
            ...
            results = self.legacy_repo.search(
                search_term=query,
                limit=limit
            )
    ...
    def _definition_to_record(self, definition: Definition) -> DefinitieRecord:
        ...
        record = DefinitieRecord(
            ...
            juridische_context=definition.metadata.get('juridische_context') if definition.metadata else None,
            status=definition.metadata.get('status', DefinitieStatus.DRAFT.value) if definition.metadata else DefinitieStatus.DRAFT.value,
        ...
        if definition.toelichting:
            record.definitie = f"{definition.definitie}\n\nToelichting: {definition.toelichting}"
    ...
    def _record_to_definition(self, record: DefinitieRecord) -> Definition:
        ...
        if record.validation_issues:
            try:
                definition.metadata['validation_issues'] = json.loads(record.validation_issues)
            except:
                pass
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM definities WHERE status != ?", 
