...
class SRUService:
    ...
    def _build_query_url(
        ...
        params = {
            ...
            "query": self._build_cql_query(term, collection or config.default_collection)
        }
    ...
    def _parse_record(
        ...
        try:
            ...
            doc_type = type_elem.text.strip() if type_elem is not None and type_elem.text else ""
            ...
            confidence = self._calculate_confidence(term, title, description, subject, config)
    ...
    async def search_legislation(self, term: str, max_records: int = 5) -> List[LookupResult]:
    ...
    async def search_jurisprudence(self, term: str, max_records: int = 5) -> List[LookupResult]:
    ...
    async def search_all_sources(self, term: str, max_records_per_source: int = 3) -> List[LookupResult]:
    ...
    def get_endpoint_config(self, endpoint: str) -> Optional[SRUConfig]:
...
async def sru_search_jurisprudence(term: str, max_records: int = 5) -> List[LookupResult]:
    """
...
async def sru_search_all(term: str, max_records_per_source: int = 3) -> List[LookupResult]:
    """
