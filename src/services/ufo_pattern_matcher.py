...
class PatternMatcher:
    ...
    def _initialize_legal_vocabulary(self) -> Dict[str, Set[str]]:
        ...
        return {
            ...
            'strafrecht': {
                # Actoren
                'verdachte', 'dader', 'mededader', 'medeplichtige', 'slachtoffer',
                'benadeelde', 'getuige', 'deskundige', 'tolk', 'reclasseringswerker',
                'officier van justitie', 'advocaat-generaal', 'procureur-generaal',
                'rechter-commissaris', 'strafrechter', 'kinderrechter', 'raadsheer',

                # Processen
                'aangifte', 'aanhouding', 'arrestatie', 'inverzekeringstelling',
                'voorgeleiding', 'bewaring', 'gevangenhouding', 'voorlopige hechtenis',
                'dagvaarding', 'oproeping', 'betekening', 'zitting', 'pleidooi',
                'requisitoir', 'vonnis', 'arrest', 'uitspraak', 'veroordeling',
                'vrijspraak', 'ontslag van rechtsvervolging', 'schuldigverklaring',
                'strafbeschikking', 'transactie', 'sepot', 'schikking',

                # Documenten
                'proces-verbaal', 'strafdossier', 'tenlastelegging', 'vordering',
                'conclusie', 'memorie', 'verweerschrift', 'appèlschrift',
                'cassatieschrift', 'gratieverzoek', 'beklag', 'klaagschrift',

                # Straffen en maatregelen
                'gevangenisstraf', 'hechtenis', 'taakstraf', 'geldboete',
                'ontzegging rijbevoegdheid', 'verbeurdverklaring', 'onttrekking',
                'tbs', 'isd-maatregel', 'gedragsbeïnvloedende maatregel',
                'vrijheidsbeperkende maatregel', 'voorwaardelijke straf',
                'bijzondere voorwaarden', 'proeftijd', 'recidive', 'strafblad',

                # Delicten
                'misdrijf', 'overtreding', 'moord', 'doodslag', 'mishandeling',
                'verkrachting', 'aanranding', 'diefstal', 'verduistering',
                'oplichting', 'fraude', 'witwassen', 'corruptie', 'omkoping',
                'drugsdelict', 'opiumdelict', 'wapenbezit', 'bedreiging',

                # Onderzoek
                'opsporingsonderzoek', 'gerechtelijk vooronderzoek', 'doorzoeking',
                'inbeslagneming', 'fouillering', 'observatie', 'infiltratie',
                'pseudo-koop', 'telefoon-tap', 'heimelijk onderzoek', 'verhoor',
                'confrontatie', 'herkenning', 'reconstructie', 'schouw',

                # Rechtsmiddelen
                'hoger beroep', 'cassatie', 'verzet', 'herziening', 'beklag',
                'wraking', 'verschoning', 'schorsing', 'uitstel', 'gratie'
            },

            # BESTUURSRECHT (120+ termen)
            'bestuursrecht': {
                # Actoren
                'burger', 'belanghebbende', 'indiener', 'aanvrager', 'verzoeker',
                'vergunninghouder', 'geadresseerde', 'derde-belanghebbende',
                'bestuursorgaan', 'college van b&w', 'burgemeester', 'wethouder',
                'gemeenteraad', 'provinciale staten', 'gedeputeerde staten',
                'commissaris van de koning', 'minister', 'staatssecretaris',
                'ambtenaar', 'toezichthouder', 'handhaver', 'inspecteur',

                # Besluiten en procedures
                'beschikking', 'besluit', 'primair besluit', 'besluit op bezwaar',
                'vergunning', 'ontheffing', 'vrijstelling', 'gedoogverklaring',
                'aanwijzing', 'last onder dwangsom', 'last onder bestuursdwang',
                'bevel', 'voorschrift', 'beleidsregel', 'nadere regel',
                'algemeen verbindend voorschrift', 'verordening', 'keur',

                # Procedures
                'aanvraag', 'aanvraagprocedure', 'voorbereidingsprocedure',
                'uniforme openbare voorbereidingsprocedure', 'zienswijze',
                'bezwaar', 'bezwaarprocedure', 'bezwaarschrift', 'hoorzitting',
                'bezwarencommissie', 'advies', 'beslissing op bezwaar',
                'administratief beroep', 'beroep', 'beroepsprocedure',
                'beroepschrift', 'verweerschrift', 'repliek', 'dupliek',
                'voorlopige voorziening', 'schorsing', 'spoedprocedure',

                # Handhaving
                'handhaving', 'handhavingsbesluit', 'handhavingsbeleid',
                'beginselplicht tot handhaving', 'concreet zicht op legalisatie',
                'hersteltermijn', 'begunstigingstermijn', 'dwangsom', 'verbeurte',
                'bestuursdwang', 'spoedeisende bestuursdwang', 'feitelijk handelen',
                'toezicht', 'controle', 'inspectie', 'waarschuwing', 'aanschrijving',

                # Beginselen
                'zorgvuldigheidsbeginsel', 'motiveringsbeginsel', 'rechtszekerheidsbeginsel',
                'vertrouwensbeginsel', 'gelijkheidsbeginsel', 'evenredigheidsbeginsel',
                'fair play beginsel', 'verbod van détournement de pouvoir',

                # Rechtsbescherming
                'rechtsmiddel', 'bestuursrechter', 'rechtbank', 'afdeling bestuursrechtspraak',
                'centrale raad van beroep', 'college van beroep voor het bedrijfsleven',
                'proceskosten', 'proceskostenvergoeding', 'griffierecht', 'dwangsom bij niet tijdig beslissen'
            },

            # CIVIEL RECHT (100+ termen)
            'civiel_recht': {
                # Personen en rechtspersonen
                'natuurlijk persoon', 'rechtspersoon', 'rechtssubject',
                'handelingsbekwaamheid', 'handelingsonbekwaamheid', 'minderjarige',
                'meerderjarige', 'curandus', 'onder curatele gestelde', 'bewindvoerder',
                'mentor', 'voogd', 'toeziende voogd', 'pleegouder',

                # Contractenrecht
                'overeenkomst', 'contract', 'koopovereenkomst', 'huurovereenkomst',
                'arbeidsovereenkomst', 'aanneming van werk', 'opdracht', 'lastgeving',
                'bruikleen', 'bewaargeving', 'borgtocht', 'vaststellingsovereenkomst',
                'dading', 'schenking', 'ruiling', 'koop op afbetaling',
                'consumentenkoop', 'koop op afstand', 'garantie', 'conformiteit',

                # Partijen
                'contractpartij', 'wederpartij', 'koper', 'verkoper', 'huurder',
                'verhuurder', 'werkgever', 'werknemer', 'opdrachtgever', 'opdrachtnemer',
                'aannemer', 'hoofdaannemer', 'onderaannemer', 'leverancier',
                'afnemer', 'consument', 'kredietgever', 'kredietnemer',
                'schuldenaar', 'debiteur', 'schuldeiser', 'crediteur',

                # Goederenrecht
                'eigendom', 'eigenaar', 'bezit', 'bezitter', 'houderschap',
                'natrekking', 'vermenging', 'zaaksvorming', 'vruchtgebruik',
                'erfpacht', 'opstal', 'erfdienstbaarheid', 'kwalitatieve verplichting',
                'hypotheek', 'pand', 'pandrecht', 'zekerheidsrecht', 'voorrecht',
                'retentierecht', 'eigendomsvoorbehoud', 'cessie', 'subrogatie',

                # Verbintenissenrecht
                'verbintenis', 'verplichting', 'prestatie', 'nakoming', 'tekortkoming',
                'wanprestatie', 'verzuim', 'ingebrekestelling', 'schadevergoeding',
                'boete', 'dwangsom', 'verrekening', 'compensatie', 'kwijtschelding',
                'novatie', 'delegatie', 'schuldvernieuwing', 'hoofdelijkheid',

                # Onrechtmatige daad
                'onrechtmatige daad', 'aansprakelijkheid', 'schade', 'letselschade',
                'vermogensschade', 'immateriële schade', 'smartengeld', 'affectieschade',
                'shockschade', 'causaal verband', 'relativiteit', 'toerekening',
            ...
            'algemeen_juridisch': {
                # Rechtsbronnen
                'wet', 'wetboek', 'grondwet', 'verdrag', 'verordening', 'richtlijn',
                'besluit', 'regeling', 'beleidsregel', 'jurisprudentie', 'rechtspraak',
                'prejudiciële beslissing', 'arrest', 'vonnis', 'uitspraak', 'beschikking',

                # Rechtshandelingen
                'rechtshandeling', 'eenzijdige rechtshandeling', 'meerzijdige rechtshandeling',
                'wilsverklaring', 'aanbod', 'aanvaarding', 'herroeping', 'intrekking',
                'vernietiging', 'nietigheid', 'conversie', 'bekrachtiging',

                # Rechtsverhoudingen
                'rechtsverhouding', 'rechtsbetrekking', 'rechtsband', 'rechtsplicht',
                'rechtsvordering', 'rechtsmiddel', 'rechtsgevolg', 'rechtsgrond',
                'rechtstoestand', 'rechtszekerheid', 'rechtskracht', 'gezag van gewijsde',

                # Bevoegdheden
                'bevoegdheid', 'volmacht', 'machtiging', 'mandaat', 'delegatie',
                'attributie', 'lastgeving', 'vertegenwoordiging', 'vertegenwoordigingsbevoegdheid',
                'beschikkingsbevoegdheid', 'beheersbevoegdheid', 'genot', 'gebruik',

                # Termijnen
                'termijn', 'fatale termijn', 'termijn van orde', 'vervaltermijn',
                'verjaringstermijn', 'stuiting', 'schorsing', 'verlenging',
                'bekendmaking', 'mededeling', 'kennisgeving', 'aanzegging',

                # Procedures algemeen
                'procedure', 'procesrecht', 'procespartij', 'gedaagde', 'eiser',
                'verweerder', 'verzoeker', 'belanghebbende', 'interveniënt',
                'geding', 'instantie', 'aanleg', 'appel', 'cassatie', 'revisie',
                'dagvaarding', 'exploot', 'deurwaardersexploot', 'betekening',

                # Bewijs
                'bewijs', 'bewijslast', 'bewijsmiddel', 'bewijsstuk', 'schriftelijk bewijs',
                'getuigenbewijs', 'deskundigenbericht', 'descente', 'plaatsopneming',
                'vermoedens', 'bekentenis', 'decisoire eed', 'comparitie',

                # Rechtsgevolgen
                'rechtsgeldigheid', 'rechtmatigheid', 'onrechtmatigheid', 'nietigheid',
                'vernietigbaarheid', 'schending', 'overtreding', 'inbreuk', 'misbruik van recht',
                'misbruik van bevoegdheid', 'strijd met de wet', 'onverbindendheid'
            }
        }
    ...
    def _initialize_comprehensive_patterns(self) -> Dict[UFOCategory, Dict[str, any]]:
        ...
        return {
            ...
            UFOCategory.KIND: {
                ...
                'core_terms': {
                    'persoon', 'mens', 'individu', 'burger', 'inwoner',
                    'organisatie', 'bedrijf', 'onderneming', 'instelling', 'instantie',
                    'voorwerp', 'object', 'zaak', 'goed', 'artikel', 'product',
                    'document', 'dossier', 'akte', 'stuk', 'bescheid', 'brief',
                    'gebouw', 'pand', 'woning', 'kantoor', 'fabriek',
                    'voertuig', 'auto', 'fiets', 'vaartuig', 'schip',
                    'dier', 'plant', 'organisme', 'levensvorm'
                }
            },
            ...
            UFOCategory.EVENT: {
                ...
                'temporal_keywords': {
                    'tijdens', 'gedurende', 'doorheen', 'vanaf', 'tot', 'tussen',
                    'voorafgaand', 'volgend', 'na', 'voor', 'bij', 'rond',
                    'aanvang', 'start', 'begin', 'einde', 'afloop', 'voltooiing',
                    'doorlooptijd', 'termijn', 'periode', 'tijdvak', 'tijdstip',
                    'moment', 'datum', 'tijdsduur', 'looptijd', 'duur'
                },
                'process_verbs': {
                    'uitvoeren', 'verrichten', 'voltooien', 'afronden', 'starten',
                    'beginnen', 'eindigen', 'stoppen', 'onderbreken', 'hervatten',
                    'plaatsvinden', 'gebeuren', 'voorkomen', 'optreden', 'verlopen',
                    'behandelen', 'afhandelen', 'verwerken', 'doorlopen'
                }
            },
            ...
            UFOCategory.ROLE: {
                ...
                'role_indicators': {
                    'als', 'in de hoedanigheid van', 'in de rol van',
                    'fungerend als', 'optredend als', 'handelend als',
                    'in functie van', 'werkzaam als', 'dienst doend als',
                    'benoemd als', 'aangesteld als', 'gemachtigd als'
                },
                'contextual_roles': {
                    # Juridisch
                    'verdachte', 'beklaagde', 'getuige', 'rechter', 'advocaat',
                    'officier', 'curator', 'bewindvoerder', 'voogd', 'mentor',
                    # Bestuurlijk
                    'aanvrager', 'verzoeker', 'belanghebbende', 'gemachtigde',
                    'vergunninghouder', 'toezichthouder', 'handhaver',
                    # Contractueel
                    'koper', 'verkoper', 'huurder', 'verhuurder', 'eigenaar',
                    'schuldenaar', 'schuldeiser', 'opdrachtgever', 'opdrachtnemer',
                    # Arbeidsrechtelijk
                    'werkgever', 'werknemer', 'zelfstandige', 'stagiair'
                }
            },
            ...
            UFOCategory.PHASE: {
                ...
                'phase_keywords': {
                    'fase', 'stadium', 'status', 'staat', 'toestand',
                    'periode', 'episode', 'etappe', 'stap', 'niveau',
                    'beginfase', 'middenfase', 'eindfase', 'overgangsfase',
                    'ontwikkelingsfase', 'groeifase', 'rijpheidsfase'
                },
                'lifecycle_states': {
                    'nieuw', 'initieel', 'startend', 'beginnend',
                    'lopend', 'actief', 'gaande', 'onderweg',
                    'voltooid', 'afgerond', 'compleet', 'gereed',
                    'beëindigd', 'gesloten', 'gestopt', 'afgebroken',
                    'geschorst', 'gepauzeerd', 'opgeschort', 'uitgesteld'
                }
            },
            ...
            UFOCategory.RELATOR: {
                ...
                'contract_types': {
                    'overeenkomst', 'contract', 'convenant', 'afspraak', 'akkoord',
                    'verdrag', 'protocol', 'memorandum', 'intentieverklaring',
                    'koopovereenkomst', 'huurovereenkomst', 'arbeidsovereenkomst',
                    'aannemingsovereenkomst', 'lastgevingsovereenkomst'
                },
                'legal_relations': {
                    'huwelijk', 'partnerschap', 'voogdij', 'curatele', 'bewind',
                    'mentorschap', 'gezag', 'adoptie', 'erkenning',
                    'eigendom', 'hypotheek', 'erfpacht', 'vruchtgebruik',
                    'pandrecht', 'retentierecht', 'voorrecht'
                }
            },
            ...
            UFOCategory.MODE: {
                ...
                'state_terms': {
                    'toestand', 'staat', 'conditie', 'situatie', 'gesteldheid',
                    'hoedanigheid', 'omstandigheid', 'positie', 'status'
                },
                'intrinsic_properties': {
                    'gezondheid', 'locatie', 'positie', 'kleur', 'vorm',
                    'grootte', 'temperatuur', 'snelheid', 'richting',
                    'gemoedstoestand', 'bewustzijn', 'kennis', 'vaardigheid',
                    'bevoegdheid', 'bekwaamheid', 'vermogen', 'capaciteit'
                }
            },
            ...
            UFOCategory.QUANTITY: {
                ...
                'units': {
                    # Valuta
                    'euro', 'eur', '€', 'dollar', '$', 'pond', 'yen',
                    # Percentage
                    'procent', '%', 'promille', '‰', 'basispunt',
                    # Gewicht
                    'kilogram', 'kg', 'gram', 'g', 'ton', 'pond', 'ounce',
                    # Lengte
                    'kilometer', 'km', 'meter', 'm', 'centimeter', 'cm', 'millimeter', 'mm',
                    # Volume
                    'liter', 'l', 'milliliter', 'ml', 'kubieke meter', 'm³',
                    # Tijd
                    'uur', 'minuut', 'min', 'seconde', 'sec', 'dag', 'week', 'maand', 'jaar',
                    # Oppervlakte
                    'm²', 'km²', 'hectare', 'are'
                },
                'measurable_concepts': {
                    ...
                    'gewicht', 'massa', 'volume', 'oppervlakte', 'inhoud',
                    'snelheid', 'tempo', 'frequentie', 'intensiteit',
                    'prijs', 'kosten', 'tarief', 'honorarium', 'vergoeding'
                }
            },
            ...
            UFOCategory.QUALITY: {
                ...
                'gradations': {
                    ...
                    'ernst', 'zwaarte', 'intensiteit', 'sterkte', 'kracht',
                    'waarde', 'waardering', 'beoordeling', 'score', 'rating'
                },
                'evaluative_terms': {
                    'betrouwbaarheid', 'geloofwaardigheid', 'validiteit',
                    'waarschijnlijkheid', 'relevantie', 'belangrijkheid',
                    'urgentie', 'prioriteit', 'effectiviteit', 'efficiëntie',
                    'kwaliteit', 'waarde', 'nut', 'bruikbaarheid'
                }
            },
            ...
            UFOCategory.SUBKIND: {
                ...
                'subtype_indicators': {
                    'soort', 'type', 'variant', 'versie', 'editie',
                    'model', 'merk', 'klasse', 'categorie', 'groep'
                }
            },
            ...
            UFOCategory.CATEGORY: {
                ...
                'category_terms': {
                    'categorie', 'klasse', 'groep', 'verzameling',
                    'type', 'soort', 'familie', 'genus', 'divisie'
                }
            },
            ...
            UFOCategory.MIXIN: {
                ...
                'mixin_indicators': {
                    'verschillende', 'diverse', 'meerdere', 'allerlei',
                    'gemeenschappelijk', 'gedeeld', 'gezamenlijk',
                    'algemeen', 'generiek', 'universeel'
                }
            },
            ...
            UFOCategory.ROLEMIXIN: {
                ...
                'rolemixin_terms': {
                    'rolmixin', 'functiemix', 'rolcombinatie',
                    'multifunctioneel', 'polyvalent'
                }
            },
            ...
            UFOCategory.PHASEMIXIN: {
                ...
                'phasemixin_terms': {
                    'fasemixin', 'stadiummix', 'toestandcombinatie',
                    'multi-fase', 'fase-overstijgend'
                }
            },
            ...
            UFOCategory.COLLECTIVE: {
                ...
                'collective_terms': {
                    'collectie', 'verzameling', 'groep', 'set', 'serie',
                    'team', 'commissie', 'raad', 'college', 'bestuur',
                    'vloot', 'leger', 'korps', 'brigade', 'divisie'
                }
            },
            ...
            UFOCategory.VARIABLECOLLECTION: {
                ...
                'variable_indicators': {
                    'variabel', 'veranderlijk', 'dynamisch', 'flexibel',
                    'groeiend', 'krimpend', 'fluctuerend', 'wisselend'
                }
            },
            ...
            UFOCategory.FIXEDCOLLECTION: {
                ...
                'fixed_indicators': {
                    'vast', 'onveranderlijk', 'statisch', 'gefixeerd',
                    'bepaald', 'vastgesteld', 'gesloten', 'definitief'
                }
            }
        }
            ...
            if 'patterns' in data:
                for i, pattern in enumerate(data['patterns']):
                    try:
                        ...
                        logger.warning(f"Could not compile pattern {i} for {category}: {e}")

            # Compileer ook woord sets als patterns
            for key in ['core_terms', 'temporal_keywords', 'role_indicators',
                       'contract_types', 'units', 'gradations']:
                if key in data and isinstance(data[key], set):
                    ...
                    if terms:
                        ...
                        try:
                            ...
                            logger.warning(f"Could not compile {key} for {category}: {e}")
        ...
        for category, pattern_dict in self.compiled_patterns.items():
            for pattern_id, pattern in pattern_dict.items():
                try:
                    ...
                    logger.debug(f"Pattern matching error for {category}.{pattern_id}: {e}")
    ...
    def _apply_disambiguation(self, text: str, existing_matches: List[PatternMatch]) -> List[PatternMatch]:
        """
        ...
        disambiguation_rules = {
            'zaak': [
                ...
                (r'roerende zaak|onroerende zaak|zaak als object', UFOCategory.KIND, 0.9),
                (r'zaak van', UFOCategory.ABSTRACT, 0.7)
            ],
            'huwelijk': [
                (r'huwelijks(?:voltrekking|sluiting|ceremonie)', UFOCategory.EVENT, 0.9),
                (r'huwelijk tussen|huwelijks(?:band|relatie)', UFOCategory.RELATOR, 0.9),
            ...
            if term in text:
                for pattern, category, confidence in rules:
                    if re.search(pattern, text, re.IGNORECASE):
                        disambiguation_matches.append(PatternMatch(
                            category=category,
                            matched_text=term,
    ...
    def _calculate_match_confidence(self,
        ...
        if category == UFOCategory.KIND:
            ...
            if any(marker in full_text for marker in ['vindt plaats', 'wordt uitgevoerd']):
                base_confidence += 0.15
    ...
    def explain_matches(self, matches: List[PatternMatch]) -> str:
        ...
        for category, cat_matches in matches_by_category.items():
            ...
            explanations.append(f"  Gevonden termen: {', '.join(list(unique_texts)[:10])}")
