...
class ValidatorConfig:
    ...
    enabled_rule_categories: Set[str] = field(default_factory=lambda: {
        "CON", "ESS", "INT", "SAM", "STR", "VER", "ARAI"
    })
    ...
    def validate(self, definition: Definition) -> ValidationResult:
        ...
        for i, (regel_id, regel_data) in enumerate(self.rules.items()):
            ...
            if resultaat.startswith("✔️"):
                ...
                violation = self._create_violation(regel_id, regel_data, resultaat, is_warning=True)
                violations.append(violation)
        ...
        is_acceptable = (
            ...
            not any(v for v in violations if v.startswith("❌ ESS-"))  # Essentiële regels zijn verplicht
        )
        ...
        self._stats['average_score'] = ((current_avg * (total - 1)) + overall_score) / total
    ...
    def _generate_suggestions(
        ...
        for violation in violations:
            if "werkwoord" in violation.lower():
                ...
                suggestions.append("Vermijd het letterlijk noemen van de context in de definitie")
                ...
                suggestions.append("Vermijd het gebruik van het te definiëren begrip in de definitie zelf")
                ...
                suggestions.append("Gebruik enkelvoud voor het begrip, tenzij het een plurale tantum betreft")
