...
class GenerationMonitor:
    ...
    def start_generation(self, begrip: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def finish_generation(self, generation_id: str, success: bool = True, 
                         error: Optional[str] = None, definition: Optional[Definition] = None):
        """
        ...
        # Record metrics
        if metrics.duration:
            self._record_metric(MetricType.GENERATION_DURATION, metrics.duration, 
                              {"begrip": metrics.begrip})
        
        self._record_metric(MetricType.SUCCESS_RATE, 1 if success else 0, 
                          {"begrip": metrics.begrip})
        ...
        logger.debug(f"Finished monitoring generation '{metrics.begrip}' "
                    f"(duration: {metrics.duration:.3f}s, success: {success})")
    
    def record_context_metrics(self, generation_id: str, sources: int, 
                             confidence: float, richness_score: float):
        ...
        if generation_id in self.active_generations:
            ...
            self._record_metric(MetricType.CONTEXT_RICHNESS, richness_score, 
                              {"begrip": metrics.begrip})
    
    def record_prompt_metrics(self, generation_id: str, prompt_length: int, strategy: str):
        ...
        if generation_id in self.active_generations:
            ...
            self._record_metric(MetricType.PROMPT_LENGTH, prompt_length,
                              {"strategy": strategy})
    ...
    def _record_definition_metrics(self, definition: Definition, metrics: GenerationMetrics):
        ...
        self._record_metric(MetricType.DEFINITION_LENGTH, definition_length,
        ...
        for metric_type, entries in self.metrics.items():
            ...
            if recent_entries:
                ...
                summary[metric_type.value] = {
                    ...
                    "min": min(values),
                    "max": max(values),
                    "total": sum(values) if metric_type == MetricType.GENERATION_COUNT else None
                }
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_metrics_recorded": sum(len(entries) for entries in self.metrics.values()),
...
def get_monitor(config: Optional[MonitoringConfig] = None) -> GenerationMonitor:
    """
