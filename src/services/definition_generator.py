...
from utils.exceptions import handle_api_error, APIError
...
class DefinitionGenerator(DefinitionGeneratorInterface):
        ...
        try:
            ...
            if self.config.enable_ontology:
                try:
                    ...
                    categorie_enum, analyse_resultaat = await analyzer.bepaal_ontologische_categorie(
                        request.begrip,
                        request.context or "",
                        request.domein or ""
                    )
                    ...
                    categorie_info = {
                        ...
                        'categorie_scores': analyse_resultaat.get('categorie_resultaat', {}).get('test_scores', {})
                    }
                    logger.info(f"Ontologische categorie bepaald: {categorie}")
                    
                except Exception as e:
                    logger.warning(f"Ontologische analyse mislukt, gebruik fallback: {e}")
            ...
            origineel, gecorrigeerd, marker = await self._generate_base_definition(
                request.begrip,
                self._build_context_dict(request)
            )
    ...
    async def enhance(self, definition: Definition) -> Definition:
        ...
        try:
            ...
            if enhancements.get('gerelateerde_begrippen'):
                definition.gerelateerde_begrippen = enhancements['gerelateerde_begrippen']
    ...
    def _simple_category_detection(self, begrip: str) -> str:
        ...
        if any(begrip_lower.endswith(p) for p in ['atie', 'ing', 'eren']):
            return 'proces'
        elif any(w in begrip_lower for w in ['verificatie', 'authenticatie', 'controle']):
            return 'proces'
        
        # Type patronen
        elif any(w in begrip_lower for w in ['document', 'bewijs', 'systeem', 'middel']):
            return 'type'
        
        # Resultaat patronen
        elif any(w in begrip_lower for w in ['resultaat', 'uitkomst', 'besluit', 'rapport']):
            return 'resultaat'
    ...
    def _parse_enhancement_response(self, response: str) -> Dict[str, any]:
        ...
        for line in lines:
            if line.startswith('TOELICHTING:'):
                ...
                result['gerelateerde_begrippen'] = [g.strip() for g in gerelateerd_text.split(',')]
