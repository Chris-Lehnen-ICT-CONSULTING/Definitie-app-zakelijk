...
class DefinitionEditRepository(DefinitionRepository):
    ...
    def get_version_history(self, definitie_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        """
            ...
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if entry.get('context_snapshot'):
                        try:
                            entry['context_snapshot'] = json.loads(entry['context_snapshot'])
    ...
    def save_with_history(self, definition: Definition, wijziging_reden: str = None, 
                         gewijzigd_door: str = "system") -> int:
        """
    ...
    def get_latest_auto_save(self, definitie_id: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def bulk_update_status(self, definitie_ids: List[int], new_status: str, 
                          gewijzigd_door: str = "system") -> int:
        """
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
                logger.info(f"Bulk updated {updated_count} definitions to status {new_status}")
    ...
    def search_with_filters(self, 
        ...
        try:
            with self._get_connection() as conn:
                ...
                if not any([search_term, categorie, status, context_filter, date_from, date_to, source_type]):
                    try:
                        cursor.execute(
                            """
                        ...
                        logger.warning(f"Fallback listing failed, falling back to dynamic query: {e}")
    ...
    def _add_history_entry(self, definitie_id: int, begrip: str, 
                          oude_waarde: str, nieuwe_waarde: str,
                          wijziging_type: str, wijziging_reden: str,
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (definitie_id, begrip, oude_waarde, nieuwe_waarde,
                     wijziging_type, wijziging_reden, gewijzigd_door))
                conn.commit()
