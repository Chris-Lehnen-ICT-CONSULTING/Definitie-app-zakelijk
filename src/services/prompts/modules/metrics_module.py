...
class MetricsModule(BasePromptModule):
    ...
    def execute(self, context: ModuleContext) -> ModuleOutput:
        ...
        try:
            ...
            # Karakterlimieten
            sections.append(f"**Karakterlimieten:**")
            ...
            if self.include_detailed_metrics:
                ...
                sections.append(f"- Complexiteitsscore: {metrics['complexity_score']}/10")
            ...
            if org_contexts and len(org_contexts) > 1:
                sections.append(f"**Context complexiteit:**")
                sections.append(f"- Aantal contexten: {len(org_contexts)}")
                sections.append(f"- Multi-context uitdaging: {'Hoog' if len(org_contexts) > 3 else 'Gemiddeld'}")
                sections.append("")
            ...
            return ModuleOutput(
                ...
                success=False,
                error_message=f"Failed to generate metrics: {str(e)}"
            )
    ...
    def _generate_scoring_advice(self, metrics: dict[str, Any]) -> list[str]:
        ...
        if metrics["estimated_words"] > 40:
            advice.append(
                "⚠️ Mogelijk te lang voor één zin - focus op kernbegrip"
            )
        elif metrics["estimated_words"] < 15:
            advice.append(
                "💡 Ruimte voor meer detail indien nodig"
            )
        
        # Kwaliteitscheck advies
        failed_checks = [
            check for check, passed in metrics["quality_checks"].items() 
            if not passed
        ]
        if failed_checks:
            advice.append(
                f"🔍 Aandachtspunten: {', '.join(failed_checks)}"
            )
        
        # Positieve feedback
        if metrics["complexity_score"] <= 5 and not failed_checks:
            advice.append(
                "✅ Goede balans tussen detail en leesbaarheid"
            )
