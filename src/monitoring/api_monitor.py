...
class CostCalculator:
    ...
    def estimate_monthly_cost(cls, daily_requests: int, avg_tokens: int) -> float:
        ...
        daily_cost = (daily_requests * 
                     cls.calculate_cost('gpt-4', input_tokens, output_tokens))
...
class MetricsCollector:
    ...
    async def _generate_snapshot(self, endpoint: str):
        ...
        recent_calls = [
            ...
            if (call.endpoint == endpoint and 
                (now - call.timestamp).total_seconds() < 300)  # Last 5 minutes
        ]
        ...
        hourly_cost = sum(call.cost for call in recent_calls) * 12  # 5-min window * 12 = hourly
        ...
        while (self.metrics_history[endpoint] and 
               self.metrics_history[endpoint][0].timestamp < cutoff):
    ...
    async def _check_alerts(self):
        ...
        for alert in self.alerts:
            ...
            if (alert.last_triggered and 
                (now - alert.last_triggered).total_seconds() < alert.cooldown_minutes * 60):
                continue
            ...
            metric_value = await self._calculate_metric_value(alert.metric_type, window_start)
            ...
            if alert.comparison == 'gt' and metric_value > alert.threshold_value:
                ...
                triggered = True
            elif alert.comparison == 'eq' and abs(metric_value - alert.threshold_value) < 0.001:
                triggered = True
    ...
    async def _calculate_metric_value(self, metric_type: MetricType, window_start: datetime) -> float:
    ...
    def get_realtime_metrics(self, endpoint: Optional[str] = None) -> Dict[str, Any]:
        ...
        for ep in endpoints:
            ...
            if ep_calls:
                ...
                endpoint_metrics[ep] = {
                    ...
                    'avg_response_time': sum(ep_response_times) / len(ep_response_times) if ep_response_times else 0,
                    'error_count': len(ep_calls) - len(ep_successful),
                    'cache_hit_rate': len([call for call in ep_calls if call.cache_hit]) / len(ep_calls),
        ...
        return {
            ...
            'active_alerts': [
                ...
                for alert in self.alerts if alert.last_triggered and 
                (now - alert.last_triggered).total_seconds() < 3600  # Last hour
            ]
        }
    ...
    def export_metrics_csv(self, filename: Optional[str] = None) -> str:
        ...
        with open(filepath, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'endpoint', 'function_name', 'duration', 'success',
                'error_type', 'tokens_used', 'cost', 'cache_hit', 'priority', 'retry_count'
            ]
