...
class DutchTextValidator:
    ...
    def load_validation_rules(self):
        ...
        general_rules = [
            DutchValidationRule(
                ...
                text_types=[DutchTextType.GENERAL, DutchTextType.LEGAL, DutchTextType.GOVERNMENT],
        ...
        self.validation_rules[DutchTextType.GOVERNMENT] = government_rules + general_rules
        self.validation_rules[DutchTextType.DEFINITION] = definition_rules + general_rules
    ...
    def load_dutch_dictionaries(self):
        ...
        basic_words = {
            "identiteit", "authenticatie", "autorisatie", "verificatie", "validatie",
            "proces", "procedure", "behandeling", "verwerking", "controle",
            "veiligheid", "beveiliging", "bescherming", "toegang", "rechten",
            "gegevens", "informatie", "persoon", "burger", "gebruiker",
            "systeem", "applicatie", "platform", "service", "dienst",
            "overheid", "organisatie", "instantie", "autoriteit", "beheer",
            "wetgeving", "regeling", "voorschrift", "richtlijn", "norm"
        }
        self.dutch_dictionary.update(basic_words)
        
        # Government terminology
        government_terms = {
            "burgerservicenummer", "bsn", "identiteitsbewijs", "paspoort", "rijbewijs",
            "digid", "mijnoverheid", "basisregistratie", "brp", "gbav",
            "elektronische", "digitale", "handtekening", "certificaat", "pki",
            "eidas", "eulogin", "saml", "oauth", "openid",
            "gemeentelijke", "provinciale", "nationale", "europese", "internationale",
            "ministerie", "rijksoverheid", "gemeente", "provincie", "waterschap",
            "rechtspraak", "openbaar", "ministerie", "politie", "belastingdienst"
        }
        self.government_terms.update(government_terms)
        
        # Legal terminology
        legal_terms = {
            "wetboek", "strafrecht", "burgerlijk", "bestuursrecht", "grondwet",
            "wet", "algemene", "verordening", "regeling", "besluit",
            "jurisprudentie", "rechtspraak", "cassatie", "hoger", "beroep",
            "artikel", "lid", "onder", "aanhef", "slot",
            "rechtspersoon", "natuurlijk", "persoon", "rechtsbetrekking", "rechtsgevolg",
            "aansprakelijkheid", "verplichting", "bevoegdheid", "rechtsgrond", "rechtsmiddel"
        }
        self.legal_terms.update(legal_terms)
    
    def validate_text(self, text: str, text_type: DutchTextType = DutchTextType.GENERAL) -> DutchValidationResult:
    ...
    def _check_spelling(self, text: str, text_type: DutchTextType) -> List[Dict[str, Any]]:
        ...
        for word in words:
            if len(word) > 3:  # Skip short words
                ...
                if text_type == DutchTextType.GOVERNMENT:
                    if word not in self.dutch_dictionary and word not in self.government_terms:
                        issues.append({
                    ...
                    if word not in self.dutch_dictionary and word not in self.legal_terms:
                        issues.append({
    ...
    def suggest_improvements(self, text: str, text_type: DutchTextType = DutchTextType.GENERAL) -> List[str]:
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        avg_issues_per_validation = total_issues / total_validations if total_validations > 0 else 0
    ...
    def export_validation_report(self, filename: Optional[str] = None) -> str:
        ...
        report = {
            ...
            "validation_rules": {
                text_type.value: len(rules) for text_type, rules in self.validation_rules.items()
            },
...
def validate_dutch_text(text: str, text_type: DutchTextType = DutchTextType.GENERAL) -> DutchValidationResult:
    """Convenience function for Dutch text validation."""
    validator = get_dutch_validator()
    return validator.validate_text(text, text_type)


def suggest_dutch_improvements(text: str, text_type: DutchTextType = DutchTextType.GENERAL) -> List[str]:
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            for i, arg in enumerate(args):
                if isinstance(arg, str) and len(arg) > 10:  # Only validate substantial text
                    result = validate_dutch_text(arg, text_type)
                    if not result.passed:
                        error_messages = [issue['description'] for issue in result.issues if issue['severity'] in ['error', 'critical']]
                        if error_messages:
                            raise ValueError(f"Dutch text validation failed: {'; '.join(error_messages)}")
...
async def test_dutch_validator():
    ...
    dutch_text = "Dit is een Nederlandse tekst voor identiteitsbehandeling binnen de overheid."
    ...
    # Test suggestions
    suggestions = validator.suggest_improvements(definition_text, DutchTextType.DEFINITION)
