...
class ValidationSchema:
    ...
    def length(self, field_name: str, min_length: int = None, max_length: int = None, message: str = ""):
        ...
        rule = ValidationRule(
            ...
            message=message or f"Field '{field_name}' length must be between {min_length} and {max_length}"
        )
        self.add_rule(rule)
        return self
    ...
    def range_check(self, field_name: str, min_value: Union[int, float] = None, max_value: Union[int, float] = None, message: str = ""):
    ...
    def custom(self, field_name: str, validator: Callable[[Any], bool], message: str = ""):
    ...
    def business_rule(self, rule_name: str, validator: Callable[[Dict[str, Any]], bool], message: str = ""):
...
class InputValidator:
    ...
    def load_built_in_schemas(self):
        ...
        definition_schema.required("begrip", "Term is required for definition generation") \
                        .type_check("begrip", str) \
                        .length("begrip", min_length=1, max_length=200) \
                        .pattern("begrip", r"^[a-zA-Z0-9\s\-_()]+$", message="Term contains invalid characters") \
                        .required("context_dict") \
                        .type_check("context_dict", dict) \
                        .custom("context_dict", self._validate_context_dict, "Invalid context dictionary structure")
        
        self.schemas["definition_generation"] = definition_schema
        
        # User input schema
        user_input_schema = ValidationSchema("user_input")
        user_input_schema.required("voorsteller", "Proposer name is required") \
                        .type_check("voorsteller", str) \
                        .length("voorsteller", min_length=1, max_length=100) \
                        .pattern("voorsteller", r"^[a-zA-Z\s\-']+$", message="Proposer name contains invalid characters") \
                        .type_check("datum", str) \
                        .pattern("datum", r"^\d{4}-\d{2}-\d{2}$", message="Date must be in YYYY-MM-DD format") \
                        .type_check("ketenpartners", list) \
                        .custom("ketenpartners", self._validate_ketenpartners, "Invalid ketenpartners format")
        
        self.schemas["user_input"] = user_input_schema
        
        # Context validation schema
        context_schema = ValidationSchema("context_validation")
        context_schema.type_check("organisatorisch", list) \
                     .type_check("juridisch", list) \
                     .type_check("wettelijk", list) \
                     .custom("organisatorisch", self._validate_organisatorisch_context, "Invalid organisatorisch context") \
                     .custom("juridisch", self._validate_juridisch_context, "Invalid juridisch context") \
                     .custom("wettelijk", self._validate_wettelijk_context, "Invalid wettelijk context")
    ...
    def _validate_ketenpartners(self, ketenpartners: List[str]) -> bool:
        ...
        valid_partners = {
            "Openbaar Ministerie", "Politie", "Reclassering", "DJI", 
            "Rechtspraak", "CJIB", "Halt", "Raad voor de Kinderbescherming",
            "Veiligheidshuizen", "Slachtofferhulp", "Anders"
        }
    ...
    def _validate_organisatorisch_context(self, context: List[str]) -> bool:
        ...
        valid_contexts = {
            "Strafrechtketen", "Civiele rechtketen", "Bestuursrechtketen",
            "Handhaving", "Toezicht", "Preventie", "Nazorg", "Anders"
        }
    ...
    def _validate_juridisch_context(self, context: List[str]) -> bool:
        ...
        valid_contexts = {
            "Strafrecht", "Civiel recht", "Bestuursrecht", "Europees recht",
            "Internationaal recht", "Grondwet", "Verdragen", "Anders"
        }
    ...
    def validate(self, data: Dict[str, Any], schema_name: str) -> List[ValidationResult]:
        ...
        try:
            ...
            validation_attempt['errors_count'] = sum(1 for r in results if not r.passed and r.severity == ValidationSeverity.ERROR)
            validation_attempt['warnings_count'] = sum(1 for r in results if not r.passed and r.severity == ValidationSeverity.WARNING)
            validation_attempt['success'] = all(r.passed or r.severity in [ValidationSeverity.INFO, ValidationSeverity.WARNING] for r in results)
    ...
    def _validate_single_rule(self, value: Any, rule: ValidationRule, full_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                passed=passed,
                message=rule.message if not passed else f"Validation passed for {rule.field_name}",
                actual_value=value
            )
    ...
    def is_valid(self, data: Dict[str, Any], schema_name: str) -> bool:
        ...
        return all(r.passed or r.severity in [ValidationSeverity.INFO, ValidationSeverity.WARNING] for r in results)
    
    def get_errors(self, data: Dict[str, Any], schema_name: str) -> List[ValidationResult]:
        """Get only validation errors."""
        results = self.validate(data, schema_name)
        return [r for r in results if not r.passed and r.severity == ValidationSeverity.ERROR]
    
    def get_warnings(self, data: Dict[str, Any], schema_name: str) -> List[ValidationResult]:
        """Get only validation warnings."""
        results = self.validate(data, schema_name)
        return [r for r in results if not r.passed and r.severity == ValidationSeverity.WARNING]
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        successful_validations = sum(1 for v in self.validation_history if v.get('success', False))
        ...
        total_warnings = sum(v.get('warnings_count', 0) for v in self.validation_history)
        ...
        return {
            ...
            'most_used_schema': max(schema_usage.items(), key=lambda x: x[1])[0] if schema_usage else None
        }
    ...
    def export_validation_report(self, filename: Optional[str] = None) -> str:
        ...
        report = {
            ...
            'schemas': {name: len(schema.rules) for name, schema in self.schemas.items()},
            'recent_validations': self.validation_history[-100:] if len(self.validation_history) > 100 else self.validation_history
        }
...
def validate_input_decorator(schema_name: str):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if errors:
                error_messages = [f"{error.field_name}: {error.message}" for error in errors]
