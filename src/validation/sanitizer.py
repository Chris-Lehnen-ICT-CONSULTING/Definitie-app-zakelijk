...
class ContentSanitizer:
    ...
    def load_sanitization_rules(self):
        ...
        dutch_rules = [
            SanitizationRule(
                ...
                pattern=r'[^\w\s\-\.,;:!?()\[\]{}""''`~@#$%^&*+=|\\/<>àáâãäåæçèéêëìíîïñòóôõöøùúûüýÿ]',
                replacement="",
        ...
        self.rules[ContentType.GOVERNMENT_TERM] = government_rules + dutch_rules + text_rules
        ...
        try:
            ...
            if content_type in self.rules:
                for rule in self.rules[content_type]:
                    if rule.level.value <= level.value or level == SanitizationLevel.PARANOID:
                        old_value = sanitized_value
                        sanitized_value = re.sub(rule.pattern, rule.replacement, sanitized_value, flags=re.IGNORECASE)
                        
                        if old_value != sanitized_value:
                            changes_made.append(f"Applied rule: {rule.name}")
                            logger.debug(f"Sanitization rule '{rule.name}' applied to content")
    ...
    def get_sanitization_stats(self) -> Dict[str, Any]:
        ...
        # Changes statistics
        total_changes = sum(record.get('changes_made', 0) for record in self.sanitization_history)
        total_warnings = sum(record.get('warnings', 0) for record in self.sanitization_history)
        ...
        return {
            ...
            'average_changes_per_sanitization': total_changes / total_sanitizations if total_sanitizations > 0 else 0
        }
    
    def export_sanitization_log(self, filename: Optional[str] = None) -> str:
        """Export sanitization log to file."""
        if filename is None:
            filename = f"sanitization_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        log_data = {
            ...
            'recent_sanitizations': self.sanitization_history[-100:] if len(self.sanitization_history) > 100 else self.sanitization_history
        }
...
async def test_sanitizer():
    ...
    print(f"✅ XSS removal: '{result.sanitized_value}' (changes: {len(result.changes_made)})")
    ...
    print(f"✅ Dutch text filter: '{result.sanitized_value}' (changes: {len(result.changes_made)})")
    
    # Test definition sanitization
    definition_text = "Een proces waarbij 123456789 wordt gebruikt"
    result = sanitizer.sanitize(definition_text, ContentType.DEFINITION, SanitizationLevel.MODERATE)
    print(f"✅ Definition sanitization: '{result.sanitized_value}' (changes: {len(result.changes_made)})")
