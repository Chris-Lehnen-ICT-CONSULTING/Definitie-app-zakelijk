...
class OptimizedResilienceSystem:
        ...
        **kwargs
        ...
        try:
            ...
            if mode == ResilienceMode.BASIC:
                ...
                result = await self._execute_enhanced(func, *args, endpoint_name=endpoint_name, **kwargs)
            elif mode == ResilienceMode.SMART:
                result = await self._execute_smart(func, *args, endpoint_name=endpoint_name, priority=priority, timeout=timeout, **kwargs)
            elif mode == ResilienceMode.FULL:
                result = await self._execute_full(func, *args, endpoint_name=endpoint_name, priority=priority, timeout=timeout, **kwargs)
            elif mode == ResilienceMode.CRITICAL:
                result = await self._execute_critical(func, *args, endpoint_name=endpoint_name, priority=priority, timeout=timeout, **kwargs)
            else:
            ...
            if self.config.enable_fallback:
                fallback_result = await self._try_fallback(func, args, kwargs, endpoint_name)
    ...
    async def _execute_enhanced(self, func: Callable, *args, endpoint_name: str, **kwargs) -> Any:
    ...
    async def _execute_smart(self, func: Callable, *args, endpoint_name: str, priority: RequestPriority, timeout: Optional[float], **kwargs) -> Any:
    ...
    async def _execute_full(self, func: Callable, *args, endpoint_name: str, priority: RequestPriority, timeout: Optional[float], **kwargs) -> Any:
        ...
        return await self._execute_enhanced(func, *args, endpoint_name=endpoint_name, **kwargs)
    
    async def _execute_critical(self, func: Callable, *args, endpoint_name: str, priority: RequestPriority, timeout: Optional[float], **kwargs) -> Any:
        ...
        return await self._execute_full(func, *args, endpoint_name=endpoint_name, priority=priority, timeout=extended_timeout, **kwargs)
    ...
    async def _record_failure(self, endpoint_name: str, duration: float, error: Exception):
    ...
    async def _try_fallback(self, func: Callable, args: tuple, kwargs: dict, endpoint_name: str) -> Any:
        ...
        if cache_key in self.fallback_cache:
            ...
            if datetime.now() - cached_entry['timestamp'] < timedelta(seconds=self.config.fallback_cache_duration):
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'metrics': self.metrics_collector.get_realtime_metrics() if self.metrics_collector else {}
        }
    
    def get_health_report(self) -> Dict[str, Any]:
        """Get detailed health report."""
        healthy_endpoints = [m for m in self.health_metrics.values() if m.status == HealthStatus.HEALTHY]
        degraded_endpoints = [m for m in self.health_metrics.values() if m.status == HealthStatus.DEGRADED]
        unhealthy_endpoints = [m for m in self.health_metrics.values() if m.status == HealthStatus.UNHEALTHY]
        down_endpoints = [m for m in self.health_metrics.values() if m.status == HealthStatus.DOWN]
        
        return {
            'overall_health': 'healthy' if len(unhealthy_endpoints) == 0 and len(down_endpoints) == 0 else 'degraded',
            'total_endpoints': len(self.health_metrics),
            'healthy_count': len(healthy_endpoints),
            'degraded_count': len(degraded_endpoints),
            'unhealthy_count': len(unhealthy_endpoints),
            'down_count': len(down_endpoints),
            'recent_failures': self.failed_requests[-10:] if self.failed_requests else [],
...
async def get_resilience_system(config: Optional[ResilienceConfig] = None) -> OptimizedResilienceSystem:
