...
class DeadLetterQueue:
    ...
    async def add(self, failed_request: FailedRequest):
        ...
        async with self._lock:
            if len(self.queue) >= self.max_size:
                ...
                logger.warning(f"Expired request from dead letter queue: {expired.request_id}")
            
            self.queue.append(failed_request)
            self.stats['total_added'] += 1
            logger.debug(f"Added request to dead letter queue: {failed_request.request_id}")
    ...
    async def get_retryable_requests(self) -> List[FailedRequest]:
        ...
        async with self._lock:
            ...
            for req in self.queue[:]:  # Copy list to avoid modification during iteration
                # Check if request should be retried
                time_since_failure = (now - req.timestamp).total_seconds()
                min_wait_time = min(300, 2 ** req.retry_count)  # Exponential backoff, max 5 min
                
                if (req.retry_count < req.max_retries and 
                    time_since_failure >= min_wait_time):
...
class HealthMonitor:
    ...
    async def register_endpoint(self, name: str, health_check_func: Optional[Callable] = None):
    ...
    async def _update_health_status(self, endpoint: str):
        ...
        if metrics.consecutive_failures >= 5 or metrics.success_rate < self.config.unhealthy_threshold:
            new_status = ServiceHealth.UNHEALTHY
        elif metrics.consecutive_failures >= 3 or metrics.success_rate < self.config.degraded_threshold:
        ...
        if old_status != new_status:
            ...
            logger.warning(f"Health status changed for {endpoint}: {old_status.value} → {new_status.value}")
        ...
        while not self._shutdown:
            try:
                ...
                for endpoint, metrics in self.endpoints.items():
                    ...
                    recent_history = [
                        (timestamp, status) for timestamp, status in self.health_history[endpoint]
                    ...
                    if recent_history:
                        ...
                        for i, (timestamp, status) in enumerate(recent_history):
                            if status in [ServiceHealth.HEALTHY, ServiceHealth.DEGRADED]:
                                if i < len(recent_history) - 1:
                                    next_timestamp = recent_history[i + 1][0]
                                    healthy_time += (next_timestamp - timestamp).total_seconds()
                                else:
    ...
    def get_health_status(self, endpoint: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            name: asdict(metrics) for name, metrics in self.endpoints.items()
        }
...
class ResilienceFramework:
    ...
    async def start(self):
        ...
        self._queue_processor_task = asyncio.create_task(self._process_dead_letter_queue())
    ...
    async def execute_with_resilience(
        ...
        **kwargs
        ...
        try:
            ...
            if health_status and health_status.get('status') == ServiceHealth.DOWN.value:
                if enable_fallback:
                ...
                raise Exception(f"Service {endpoint_name} is down and no fallback available")
            ...
            # Record failure
            response_time = time.time() - start_time
            await self.health_monitor.record_request(endpoint_name, False, response_time)
            ...
            if enable_fallback:
                fallback_result = await self._get_fallback_response(func, args, kwargs)
                if fallback_result is not None:
                    logger.warning(f"Using fallback response for {endpoint_name} after error: {e}")
                    return fallback_result
            
            # Re-raise original exception
            raise
    
    async def _get_fallback_response(self, func: Callable, args: tuple, kwargs: dict) -> Optional[Any]:
    ...
    async def _cache_fallback_response(self, func: Callable, args: tuple, kwargs: dict, result: Any):
        """Cache successful response for fallback."""
        cache_key = self._generate_cache_key(func, args, kwargs)
            ...
            try:
                ...
                for request in retryable_requests:
                    try:
                        ...
                        logger.info(f"Retrying request: {request.request_id} (attempt {request.retry_count + 1})")
                        ...
                        if request.retry_count >= request.max_retries:
                            await self.dead_letter_queue.remove(request.request_id, processed=False)
                            logger.error(f"Max retries exceeded for request: {request.request_id}")
                        
                    except Exception as e:
                        logger.error(f"Error retrying request {request.request_id}: {e}")
...
async def get_resilience_framework(config: Optional[ResilienceConfig] = None) -> ResilienceFramework:
...
async def test_resilience_framework():
    ...
    try:
        ...
        print(f"📊 Dead Letter Queue: {health['dead_letter_queue']['queue_size']} items")
