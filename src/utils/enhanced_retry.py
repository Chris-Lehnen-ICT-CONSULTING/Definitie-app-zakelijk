...
class AdaptiveRetryManager:
    ...
    async def should_retry(self, error: Exception, attempt: int) -> bool:
        ...
        async with self._lock:
            if self.circuit_state.state == CircuitState.OPEN:
                ...
                if (self.circuit_state.last_failure_time and 
                    datetime.now() - self.circuit_state.last_failure_time > 
    ...
    async def get_retry_delay(self, error: Exception, attempt: int) -> float:
        ...
        logger.debug(f"Calculated retry delay: {delay:.2f}s for {error_type} (attempt {attempt})")
        return delay
    ...
    async def record_success(self, duration: float, endpoint: str = ""):
        ...
        async with self._lock:
            ...
            self.request_history = [m for m in self.request_history if m.timestamp > cutoff]
    ...
    async def _record_failure(self, error: Exception):
        ...
        if (self.circuit_state.state == CircuitState.CLOSED and 
            self.circuit_state.failure_count >= self.config.failure_threshold):
            self.circuit_state.state = CircuitState.OPEN
            logger.warning(f"Circuit breaker OPEN after {self.circuit_state.failure_count} failures")
        ...
        recent_errors = [m for m in self.request_history 
    ...
    def get_health_metrics(self) -> Dict[str, Any]:
        ...
        recent_requests = [m for m in self.request_history if m.timestamp > recent_cutoff]
        ...
        return {
            ...
            'last_failure_time': self.circuit_state.last_failure_time.isoformat() if self.circuit_state.last_failure_time else None,
                ...
                try:
                    ...
                    if attempt > 0:
                        if not await retry_manager.should_retry(last_error, attempt):
                            logger.error(f"Max retries exceeded or circuit breaker open for {func.__name__}")
                            raise last_error
