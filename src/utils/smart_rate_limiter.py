...
class SmartRateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        self.config = config
        self.token_bucket = TokenBucket(config.tokens_per_second, config.bucket_capacity)
        ...
        try:
            ...
            if history_file.exists():
                with open(history_file, 'r') as f:
                    data = json.load(f)
                    self.current_rate = data.get('optimal_rate', self.config.tokens_per_second)
                ...
                for priority in RequestPriority:
                    ...
                    while queue and await self.token_bucket.acquire():
                        ...
                        if not request.future.done():
                            queue_wait_time = (datetime.now() - request.timestamp).total_seconds()
                            
                            # Apply priority weighting to queue time
                            weight = self.config.priority_weights[priority]
                            if queue_wait_time * weight < 30.0:  # Max weighted wait time
