...
class OntologischeAnalyzer:
    ...
    async def bepaal_ontologische_categorie(
        ...
        try:
            ...
            logger.info(f"Ontologische analyse voltooid voor '{begrip}': {categorie_resultaat['primaire_categorie']}")
            
            return OntologischeCategorie(categorie_resultaat['primaire_categorie']), analyse_resultaat
    ...
    async def _stap1_lexicale_verkenning(self, begrip: str) -> Dict[str, Any]:
        ...
        try:
            ...
            zoek_resultaten = await self.definitie_zoeker.zoek_definities(
                begrip, 
                context={'rechtsgebied': 'algemeen'}
            )
            
            # Analyseer semantische kenmerken
            kenmerken = self._analyseer_semantische_kenmerken(begrip, zoek_resultaten.definities)
            ...
            return {
                ...
                'sleutelwoorden': self._extracteer_sleutelwoorden(zoek_resultaten.definities),
            ...
            return {
                'definities': [],
                'semantische_kenmerken': self._analyseer_semantische_kenmerken(begrip, []),
                'synoniemen': [],
                'sleutelwoorden': [],
                'bron_kwaliteit': 0.0
            }
    ...
    async def _stap2_context_analyse(
        ...
        try:
            ...
            juridische_verwijzingen = zoek_wetsartikelstructuur(f"{begrip} {jur_context}")
            ...
            domein_analyse = self._analyseer_domein_context(begrip, org_context, jur_context)
            ...
            return {
                ...
                'afhankelijkheden': self._bepaal_context_afhankelijkheden(begrip, org_context, jur_context)
            }
    ...
    async def _stap3_formele_categorietoets(
        ...
        secundaire_aspecten = [
            cat for cat, score in test_resultaten.items() 
    ...
    def _stap6_documentatie(
        ...
        categorie = categorie_resultaat['primaire_categorie']
        template = self.category_templates.get(categorie, "Een {begrip} dat {kenmerken}")
    ...
    def _analyseer_semantische_kenmerken(self, begrip: str, definities: List) -> Dict[str, bool]:
    ...
    def _extracteer_sleutelwoorden(self, definities: List) -> List[str]:
        ...
        for definitie in definities:
            ...
            belangrijke_woorden = [
                ...
                if len(woord) > 3 and woord.lower() not in ['een', 'het', 'van', 'voor', 'met', 'door']
            ]
    ...
    def _analyseer_domein_context(self, begrip: str, org_context: str, jur_context: str) -> Dict[str, Any]:
        ...
        if any(woord in context_tekst for woord in ['straf', 'strafrecht', 'strafbaar']):
            domein['rechtsgebied'] = 'strafrecht'
        elif any(woord in context_tekst for woord in ['bestuurs', 'bestuur', 'gemeente']):
    ...
    def _bepaal_context_afhankelijkheden(self, begrip: str, org_context: str, jur_context: str) -> List[str]:
        ...
        if any(woord in context_tekst for woord in ['procedure', 'proces', 'handeling']):
            afhankelijkheden.append('Organisatorisch proces')
        
        # Technische afhankelijkheden
        if any(woord in context_tekst for woord in ['systeem', 'technisch', 'digitaal']):
    ...
    async def _identiteit_type(self, begrip: str, categorie_resultaat: Dict) -> Dict[str, Any]:
    ...
    async def _identiteit_proces(self, begrip: str, categorie_resultaat: Dict) -> Dict[str, Any]:
    ...
    async def _identiteit_resultaat(self, begrip: str, categorie_resultaat: Dict) -> Dict[str, Any]:
    ...
    async def _identiteit_exemplaar(self, begrip: str, categorie_resultaat: Dict) -> Dict[str, Any]:
    ...
    async def _identiteit_fallback(self, begrip: str, categorie_resultaat: Dict) -> Dict[str, Any]:
    ...
    def _identificeer_basis_entiteit(self, begrip: str) -> Optional[str]:
        ...
        if any(woord in begrip_lower for woord in ['persoon', 'natuurlijk', 'individu']):
            return 'Persoon'
        elif any(woord in begrip_lower for woord in ['organisatie', 'bedrijf', 'instelling']):
            return 'Organisatie'
    ...
    def _genereer_definitie(self, begrip: str, categorie: str, template: str, rol_analyse: Dict) -> str:
    ...
    def _generate_comprehensive_reasoning(
        ...
        reasoning = f"Ontologische categorisering van '{begrip}' als {categorie.upper()} "
...
class QuickOntologischeAnalyzer:
    ...
    def quick_categoriseer(self, begrip: str) -> Tuple[OntologischeCategorie, str]:
        ...
        if any(begrip_lower.endswith(patroon) for patroon in self.proces_patronen):
            return OntologischeCategorie.PROCES, f"Proces patroon gedetecteerd (eindigt op {[p for p in self.proces_patronen if begrip_lower.endswith(p)][0]})"
        
        # 2. Is het een ding? → TYPE
        if any(patroon in begrip_lower for patroon in self.type_patronen):
            return OntologischeCategorie.TYPE, f"Type patroon gedetecteerd"
        
        # 3. Is het een kenmerk? → RESULTAAT
        if any(patroon in begrip_lower for patroon in self.resultaat_patronen):
            return OntologischeCategorie.RESULTAAT, f"Resultaat patroon gedetecteerd"
        
        # 4. Is het een positie/functie? → EXEMPLAAR
        if any(patroon in begrip_lower for patroon in self.exemplaar_patronen):
            return OntologischeCategorie.EXEMPLAAR, f"Exemplaar patroon gedetecteerd"
        
        # 5. Default → TYPE
        return OntologischeCategorie.TYPE, "Standaard categorie (geen specifieke patronen)"