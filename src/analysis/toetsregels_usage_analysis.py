...
def analyze_rule_usage():
    ...
    for categorie in categorieën:
        ...
        print_category_analysis(categorie.value, generation_rules, validation_rules, comparison)
...
def analyze_generation_rules(generator: DefinitieGenerator, categorie: OntologischeCategorie) -> Dict[str, Any]:
    ...
    for instructie in result.gebruikte_instructies:
        ...
        used_rules[rule_id] = {
            'guidance': instructie.guidance[:100] + "..." if len(instructie.guidance) > 100 else instructie.guidance,
...
def analyze_validation_rules(validator: DefinitieValidator, categorie: OntologischeCategorie) -> Dict[str, Any]:
    ...
    for criterium in criteria:
        ...
        rule_ids.add(rule_id)
        used_rules[rule_id] = {
            'description': criterium.description[:100] + "..." if len(criterium.description) > 100 else criterium.description,
...
def compare_rule_usage(
    ...
    return {
        ...
        'overlap_percentage': len(gen_set & val_set) / len(gen_set | val_set) * 100 if (gen_set | val_set) else 0
    }
...
def print_category_analysis(
    ...
    print(f"🔧 GENERATIE:")
    print(f"   Regels gebruikt: {generation_rules['count']} ({comparison['coverage_generation']:.1f}% van totaal)")
    print(f"   Regel IDs: {generation_rules['rule_ids']}")
    
    print(f"\n🔍 VALIDATIE:")
    print(f"   Regels gebruikt: {validation_rules['count']} ({comparison['coverage_validation']:.1f}% van totaal)")
...
def print_overall_analysis(results: Dict[str, Any], all_rules: List[str]):
    ...
    print(f"🔧 Gebruikt voor generatie: {len(all_gen_rules)} ({len(all_gen_rules)/len(all_rules)*100:.1f}%)")
    print(f"🔍 Gebruikt voor validatie: {len(all_val_rules)} ({len(all_val_rules)/len(all_rules)*100:.1f}%)")
    print(f"⚡ Gebruikt door beide: {len(all_gen_rules & all_val_rules)}")
    print(f"📊 Totaal gebruikt: {len(total_used)} ({len(total_used)/len(all_rules)*100:.1f}%)")
    print(f"⚠️  Totaal ongebruikt: {len(unused_overall)} ({len(unused_overall)/len(all_rules)*100:.1f}%)")
    ...
    for category, data in results.items():
        ...
        print(f"   {category.upper():<12}: Gen={gen_count:<2} Val={val_count:<2} Overlap={data['comparison']['overlap_percentage']:.0f}%")
...
def detailed_rule_analysis():
    ...
    belangrijke_regels = ["CON-01", "CON-02", "ESS-01", "ESS-02", "ESS-05", "INT-03", "STR-01"]
    ...
    for rule_id in belangrijke_regels:
        ...
        print(f"   🔍 Validatie: {len(val_criterion.patterns_to_avoid)} patronen, {val_criterion.severity.value} severity")
