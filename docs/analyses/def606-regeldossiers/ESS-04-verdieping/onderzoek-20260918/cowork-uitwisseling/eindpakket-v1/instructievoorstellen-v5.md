...
18 september 2026 — v5, definitieve onderzoeksvoorstellen met synthesecontrole verwerkt; herziene Codex-teksten na kruisreview; synthesecontrole verwerkt (zie synthesecontrole-verwerking-v1.md). Alleen toepassen na normbesluit en aparte uitvoeringsopdracht. Appvindplaatsen zijn in codebasis-4cdb8ea43/; actieve skillkopieën staan in feitenbasis/. Bestaande CON-01/02 en ESS-01/02 blijven behouden.
...
Cases: non_numeric/ambiguous_measure/observable/empty; E04-N01, E04-N07, E04-N09, E04-N14, E04-N17, E04-N21, E04-N23.
...
1. **definitie-toetsregels/reference.md:47**, vervang de ESS-04-tabelrij door de volgende vier celwaarden: **ESS-04**; **Toetsbaarheid van begripskenmerken**; **midden**; **Beschrijf navolgbaar toepasbare criteria; kwalitatief waar passend, kwantitatief alleen met onderbouwde grens en relevante meetcontext. Signalen vervangen geen menselijke beoordeling.** Cases E04-N01, E04-N03, E04-N05, E04-N06.
...
## Exacte kandidaat bij patroonoptie A

Onderstaande oorspronkelijke Cowork-kandidaat maakt de keuze reviewbaar. Nog niet getest of geïnstalleerd; `voldoende` is hier zichtbaar gehouden als expliciet te bespreken overlap. Woorden zoals redelijk/in beginsel kunnen bedoeld en toetsbaar zijn; een hit vraagt uitsluitend aandacht. C14-woordaanhechting, getalwoorden en regelscope zijn kalibratiegevallen; de kandidaat belooft daar geen volledige dekking.
...
## Concrete weergave- en hersteltekst bij implementatie

**Aanhechting in evaluator:** `src/services/validation/evaluators/judgment_review.py`, `evaluate`, geeft voor ESS-04 de kale toetsvraag. `_reden_met_passages` bestaat al voor ESS-01/02. Voorstel: ESS-04 daarop aansluiten met eigen normkop en neutrale passagevraag uit de meldingen; ook zonder hit blijft inhoudelijke beoordeling nodig. Alleen de reden verbeteren maakt haar nog niet altijd zichtbaar: ook onderstaande UI-lus moet aansluiten.

**Huidige vindplaats:** `src/ui/components/validation_view.py:730–739` toont de reviewreden direct alleen voor ESS-01 en ESS-02. **Voorstel:** toon voor ESS-04 altijd de bestaande open-status plus de vraag en reden uit G/T/H; bij beschikbare signalen de letterlijk aangewezen passage met neutrale beoordelingsvraag. Ook zonder passage blijft de vraag zichtbaar. Geen uitbreiding naar andere regels zonder hun eigen contract. Behandel passage als gewone tekst, zoals het huidige patroon voor ESS-01/02. Deze codelezing is geen uitgevoerde UI-proef.

**Huidige pass-uitleg:** `src/ui/components/validation_renderer.py:353` bevat voor ESS-04 nog de heuristische verklaring dat een vereist element is herkend. In de onderzochte automatische route komt ESS-04 niet bij passed_rules; geen huidige foutieve pass aangetoond. **Voorstel:** verwijder ESS-04 uit deze generieke heuristische toelichting bij latere implementatie; als een werkelijk geldig menselijk voldoet-oordeel wordt weergegeven, toon uitsluitend diens opgeslagen motivering, nooit een woord- of getalsverklaring als vervanging.

**Huidige herstelhint:** `src/services/validation/modular_validation_service.py:1938` geeft bij reason=testable en ESS-04 nog een suggestie voor termijn/meetgrens. Die violation is niet geproduceerd door de onderzochte review_required-route. **Exacte vervangende hulptekst voor een eventuele ondersteunde diagnostische route:** “Benoem welk criterium onvoldoende bepaald is en welke betekenisgrond nodig is. Gebruik alleen een onderbouwde kwalitatieve of kwantitatieve afbakening; voeg geen termijn of grens toe om een signaal te verkrijgen.” Koppel dit niet automatisch aan iedere review_required-status en activeer geen autoherstel.
