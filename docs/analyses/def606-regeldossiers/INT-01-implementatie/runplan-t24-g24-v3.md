...
| Proef ronde 3 na de tweede review: 16 gevallen (v2-set plus ‘categorie A. Heeft’, ‘categorie A. Registratie’ en ‘dr. J. Jansen’; ‘J. Jansen’ zonder titel nu onzeker) × 2 laadpaden, 32/32 conform; opslag-, lees- en exportroute inclusief de enkele CSV-export 16/16 conform, update met historie en wijziging buiten de laag om conform, offline-gate actief | [`proef-na-implementatie-v3.py`](proef-na-implementatie-v3.py), [`proefuitkomsten-na-implementatie-v3.json`](proefuitkomsten-na-implementatie-v3.json) | idem: ontwerp-/regressiegevallen, **geen ongeziene eindset** |
| Unit- en ketentests (segmentatie, beide laadpaden, opslag/teruglezen/export met binding inclusief enkele CSV, herladen, bewerk-tab met tekstbinding, UI-renderer, prompt); in-memory mutatiematrix 8/8 (ronde 2) en 4/4 (ronde 3) gevangen | `tests/unit/validation/test_def770_int01_zinsgrenzen.py`, `tests/unit/services/test_def770_int01_keten.py`, `tests/unit/services/test_def770_int01_opslag.py`, `tests/unit/ui/test_def770_int01_editor_binding.py`, `tests/unit/services/prompts/test_def770_int01_promptnorm.py` | synthetische invoer; geen modeluitvoer, geen gebruikers |
...
- **Opslag van de deeluitkomst (opgelost in ronde 2).** Versie 1 meldde dat ‘één zin, compactheid en begrijpelijkheid open’ nergens werd opgeslagen of geëxporteerd. Nu bewaart de persistentielaag bij elke schrijfactie die de kern zet (nieuw record, update, bronvoorsteltoepassing — alle via `create_definitie`/`update_definitie`; de enige andere tekstschrijver, `get_or_create_draft`, maakt een leeg concept zonder uitkomst, en de eerste tekst daarop loopt weer via `update_definitie`) de INT-01-deeluitkomst onder `int01_beoordeling` in de bestaande JSON-registratie `generation_prompt_data`: status, reden, alle onderdelen, en de binding (sha256 van de kern zonder toelichting, recordversie, contractversie). Een vorige uitkomst gaat naar `int01_beoordeling_history`. Lezen geeft `applied: false` met reden als de kern of contractversie niet meer overeenkomt. Export: JSON (bulk en enkel, onder `validatie`), CSV/Excel-kolom `int01_beoordeling` op UITGEBREID en COMPLEET, de enkele CSV-export (ronde 3) met dezelfde compacte JSON, TXT (bulk en enkel) leesbaar. Geen nieuwe databasekolom en geen wijziging van het 2.1.0-resultaatcontract. Grenzen: records van vóór DEF-770 hebben pas na de eerstvolgende gewone schrijfactie een uitkomst (tot dan: ‘niet beoordeeld’ in de enkele TXT, lege cel elders); het BASIS-niveau exporteert, net als het bronbewijs, geen validatievelden.
- **Weergave in de bewerk-tab (gecorrigeerd in ronde 3).** Versie 2 stelde onjuist dat de UI bij herladen opnieuw toetst; zonder sessieresultaat toonde de bewerk-tab géén INT-01-uitkomst, en na een toetsing bleef bij een gewijzigde formuliertekst de oude deelstatus (zinsstructuur ‘voldoet’) staan. Nu: bij openen zonder sessieresultaat toont de tab de opgeslagen uitkomst als die aan de actuele formuliertekst en contractversie bindt, anders zichtbaar niet toepasbaar (‘gewijzigd’) of niet beoordeeld — zonder de oude onderdelen. Een sessieresultaat draagt de vingerafdruk van de getoetste tekst; wijkt de formuliertekst af, dan is INT-01 open en niet toepasbaar tot opnieuw getoetst wordt. Er wordt bij herladen of tekstwijziging niet automatisch opnieuw getoetst. De ESS-03-herbinding is ongewijzigd.
- **Naaminitialen (ronde 3).** Een losse hoofdletter met punt gevolgd door een hoofdletter geldt alleen als naaminitiaal na een titel of een andere initiaal (‘dr. J. Jansen’); anders is de grens onzeker met passage (‘categorie A. Registratie’, ook ‘J. Jansen’ zonder titel). Dat kost bij namen zonder titel een beoordelingsvraag, maar voorkomt stil vals ‘één zin’.
