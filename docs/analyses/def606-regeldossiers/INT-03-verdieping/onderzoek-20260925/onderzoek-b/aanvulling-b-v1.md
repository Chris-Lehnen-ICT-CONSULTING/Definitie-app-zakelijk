# INT-03 — onafhankelijke aanvulling onderzoeker B — v1

25 september 2026. **Eerste bijdrage; voorstel, geen normbesluit of implementatie.**
...
Onderzoeker B: Codex CLI, sessie `01a0d782-026f-7a70-b451-84c5f82137dd`. De eigen `turn_context`-metadata in `/Users/chrislehnen/.codex/sessions/2026/09/25/rollout-2026-09-25T09-40-16-01a0d782-026f-7a70-b451-84c5f82137dd.jsonl` bevestigt `model=gpt-6-astra`, `effort=high`. Geen bewijs uitsluitend uit config. A coördineert in Cowork; C onderzoekt zelfstandig in Claude Code. Geen nieuwe sessies gestart; geen nieuwe conclusies van A/C of bestaande runlogs in onderzoek-b gelezen vóór deze v1. Alleen bestandsnamen gecontroleerd om overschrijven te voorkomen.
...
Alle bronnen zijn werkelijk gelezen binnen het genoemde bereik; hashes staan in [bewijsmanifest-b-v1.json](bewijsmanifest-b-v1.json). Repositorypaden hieronder zijn relatief aan de projectroot; `../`-paden relatief aan deze map.
...
| CON-CIRC-001 | Record verbiedt letterlijk lemma in kern; E03/E04 faalt hier | Blind herhalen is geen veilige reparatie; herformuleer met betekenisbehoud of leg conflict voor. |
...
**Statisch S4:** JudgmentReviewEvaluator heeft geen INT-03-tak; alleen ESS-01/02/04 hebben passagehulp. INT-03 geeft altijd review. Signalen worden over cleaned_text verzameld. De service verklaart definition_text beschikbaar zonder niet-leegtest: E14 blijft INT-03-review, terwijl VAL-EMP-001 faalt. Dit bewijst geen toegestane vaststelling van lege invoer.
...
| Review/UI | Statisch validation_view.py:241 toont open codes; :803 toont direct alleen ESS-01/02/04-redenen | Geen live UI-test of INT-03-antecedentkeuze. In deze renderer staat INT-03 vooral in de open-statuslijst. |
...
De bestaande positieve passage over betrekkelijke bijzinnen behouden. Beide SKILL.md-bestanden hebben geen INT-03-contractsectie. Exact voorstel voor een nieuwe sectie in beide:

> INT-03: gebruik de gedeelde norm voor de definitiekern. Een signaal is geen afkeuring en geen signaal is geen goedkeuring. Beoordeel verwijzingen met de toetsinstructie T-B1 en bescherm de betekenis volgens H-B1. Geef alleen een advies over de exacte aangeleverde versie; dit advies is geen opgeslagen menselijke beoordeling, vaststelling of exportgoedkeuring. Ontbrekende informatie, open oordeel en technische fout blijven onderscheiden.

T-B1/H-B1 hieronder moeten bij uitvoering daadwerkelijk in het gedeelde skillcontract staan; verwijzingen naar alleen dit onderzoeksdocument zijn geen geïnstalleerd contract. Beheerde bron in ~/Projecten/_claude-global-setup/skills en werkelijk gebruikte kopieën gecontroleerd publiceren. De generieke score-/blokkadeclaims in de toetsregelskill onder DEF-624 synchroniseren, zonder nieuwe score aan INT-03 toe te voegen.
...
|---|---|---|
| Kruisreview/synthese | A draagt eigen volledige v1 over nadat eerste versies opgeslagen zijn; B reviewt op vervolgopdracht; A organiseert C-verwerking | Dit volledige pakket gaat nu naar A. Geen andere conclusies gelezen. |
...
| Transport/opslag/stale/poorten | DEF-624/626/630 testen synthetische keten met versiegebonden INT-03-oordeel | Bij daadwerkelijke levering, vóór volledige appclaim. |
...
**Opleverstatus:** onafhankelijke eerste bijdrage B met actuele offline nulmeting. Geen gezamenlijk afgerond onderzoek, geen technische oplevering en geen aangetoonde kwaliteitswinst. Volgende overdracht: dit v1-pakket aan A, daarna wederzijdse review, verwerking en synthesecontrole op opdracht. Het manifest bindt de vijf andere eigen bestanden; de SHA-256 van het manifest zelf staat afzonderlijk in de oplevering om circulaire zelfhashing te vermijden.
