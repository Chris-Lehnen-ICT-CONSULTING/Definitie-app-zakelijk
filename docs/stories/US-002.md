...
title: Build Modular Prompt Template System with Context-Aware Selection
status: GEREED
priority: HOOG
story_points: 5
sprint: Voltooid
Afhankelijkheden: [US-001]
aangemaakt: 2025-01-02
...
**gegeven** templates stored in `config/prompts/templates/`
**wanneer** a request for context "strafrecht" is made
**dan** the system:
- Loads `criminal_law_template.yaml`
- Merges with `base_template.yaml`
- Includes only strafrecht validatieregels
- Total prompt size < 3,000 tokens

### Criterion 2: Dynamic Variable Substitution
**gegeven** a template with variables `{term}`, `{context}`, `{domain}`, `{examples}`
**wanneer** values are provided: term="vonnis", context="strafrecht", domain="OM"
**dan**:
- All variables are replaced with actual values
- Unused optional variables are removed (not left as placeholders)
- Special characters in values are properly escaped
- Dutch formatting rules are preserved (e.g., quotes, capitalization)

### Criterion 3: Template validatie & Completeness
**gegeven** a generated prompt from templates
**wanneer** valiDatumd before sending to GPT-4
**dan** it must contain:
...
- **Justice stEnards**:
  - OM: openbaar ministerie-focused language en structure
...
- [x] PromptServiceV2 implemented with modular loading
...
- [x] Prestaties tests valiDatum < 3,000 tokens
...
- Template design: 2025-01-05
- Initial Implementatie: 2025-01-08
- Token optimization: 2025-01-12
- Hot-reload added: 2025-01-15
- productie deployment: 2025-01-20
