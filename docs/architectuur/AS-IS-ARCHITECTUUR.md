...
graph TB
    ...
    class T1,T2,T3,TOETS_ENGINE,TOETS_RULES,TOETS_MGR,REPO working
...
| Externe Bronnen | ❌ Broken | Web lookup kapot |
...
| Web Scraping | BeautifulSoup | 4.x | ❌ |
...
└── web_lookup/          # ❌ Broken lookup services
```
...
### Kritiek (P0)
1. **Web Lookup Volledig Broken**: 5 verschillende implementaties, geen werkend
...
- OpenAI API (kritiek)
- wetten.nl (broken)
- officielebekendmakingen.nl (broken)
