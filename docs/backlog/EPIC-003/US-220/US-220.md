...
**dan** is er een configureerbare timeout actief (default 6.0s via `WEB_LOOKUP_TIMEOUT_SECONDS`)
...
- Test timeout behavior (configureerbaar; default 6.0s)
...
### Risico's
- **Performance**: Elke generatie doet nu web lookup (mitigatie: configureerbare timeout; default 6.0s)
