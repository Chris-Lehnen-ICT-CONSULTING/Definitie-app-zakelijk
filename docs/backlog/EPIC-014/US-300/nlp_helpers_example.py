...
def identify_legal_domain(text: str, context: List[str] = None) -> str:
    ...
    # Domain indicators
    domains = {
        'strafrecht': ['verdachte', 'strafbaar', 'delict', 'misdrijf', 'overtreding', 'Sr', 'Sv'],
        'bestuursrecht': ['beschikking', 'bezwaar', 'beroep', 'Awb', 'bestuursorgaan'],
        'civielrecht': ['overeenkomst', 'contract', 'aansprakelijk', 'schadevergoeding', 'BW'],
        'arbeidsrecht': ['arbeidsovereenkomst', 'werkgever', 'werknemer', 'ontslag', 'cao'],
...
def extract_definition_structure(text: str) -> Dict[str, Any]:
    ...
    # Check for examples
    if re.search(r'\bzoals\b|\bbijvoorbeeld\b|\bonder andere\b|\bo\.a\.\b', text, re.IGNORECASE):
        structure['has_examples'] = True

    # Check for conditions
    if re.search(r'\bmits\b|\bindien\b|\bop voorwaarde\b|\bvoor zover\b', text, re.IGNORECASE):
        structure['has_conditions'] = True

    # Check for exceptions
    if re.search(r'\bbehalve\b|\btenzij\b|\buitzonderingen?\b|\bmet uitzondering\b', text, re.IGNORECASE):
        structure['has_exceptions'] = True

    # Check for enumeration
    if re.search(r'\b[a-z]\)|[1-9]\.|ten eerste|ten tweede|\bi+\)', text, re.IGNORECASE):
