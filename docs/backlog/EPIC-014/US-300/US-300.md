...
id: US-300
epic: EPIC-014
titel: "US-300: titel: Automatische bepaling van UFO‑categorie (OntoUML/UFO)"
status: open
prioriteit: HOOG
story_points: 5
...
# US-300: Automatische bepaling van UFO‑categorie

## Gebruikersverhaal
Als expert‑gebruiker wil ik dat de applicatie automatisch de juiste UFO‑categorie (OntoUML/UFO) voor een begrip voorstelt, zodat ik minder handmatig werk heb en de classificatie consistenter wordt.

## Context
- UFO‑categorie (OntoUML/UFO metamodel) is nu handmatig selecteerbaar in Generator, Bewerk en Expert‑review tabs
- De dataset bevat nog geen betrouwbare trainingslabels; start met heuristieken/detector + fallback naar handmatige keuze
- 16 UFO-categorieën gedefinieerd in database schema, vrijwel ongebruikt (0% van actieve definities heeft categorie)
- 80+ voorbeelddefinities beschikbaar voor initial testing

## Acceptatiecriteria (SMART)
- [ ] De app toont automatisch een voorstel voor `ufo_categorie` bij generatie of laden in edit/review, met confidence‑indicatie
- [ ] Gebruiker kan het voorstel overschrijven (manual > auto)
- [ ] Minimaal **95% precisie** op interne validatieset voor duidelijke gevallen (≥0.8 confidence)
...
- [ ] Batch processing: Voor initiële 80+ definities (geen performance vereiste)

## Technische Implementatie

### Service Architectuur (Single-User Focus)
- **UFOClassificationService**: Simpele service geïntegreerd met ServiceContainer
- **Rule Engine**: 100+ zorgvuldig gecureerde regels voor Nederlandse juridische context
- **Kwaliteit over snelheid**: Grondige analyse met alle 16 UFO categorieën
...
class UFOClassificationService:
    def classify(self, term: str, definition: str, context: Optional[Dict] = None) -> UFOClassification:
        ...
        Returns:
        - primary_category: Een van de 16 UFO categorieën
        - secondary_tags: Alle relevante subcategorieën voor volledigheid
...
1. Is het zelfstandig, substantieel "ding" (kan zonder drager bestaan)? → **Kind**
2. Is het tijdsgebonden (gebeurtenis/proces)? → **Event**
3. Is het een rol die een entiteit kan aannemen? → **Role**
4. Is het een levensfase van een entiteit? → **Phase**
5. Medieert het een relatie tussen entiteiten? → **Relator**
6. Is het een intrinsieke eigenschap met drager? → **Mode**
7. Is het een meetbare grootheid? → **Quantity**
8. Is het een kwalitatieve eigenschap? → **Quality**
9. Verfijn met subcategorieën: **Subkind**, **Category**, **Mixin**, **RoleMixin**, **PhaseMixin**, **Collective**, **VariableCollection**, **FixedCollection**
...
### Signalen per categorie (NL‑specifiek)
- **Kind**: zelfstandige naamwoorden die zelfstandig bestaan (mens, auto, zaak, organisatie); definities zonder drager/relatie
- **Event**: werkwoorden/nominalisaties (arrestatie, zitting, beoordeling); tijd/duur indicaties (op, tijdens, van‑tot)
...
- **Relator**: contract/legale binding (overeenkomst, huwelijk, dagvaarding, vergunning, mandaat); ≥2 deelnemers
- **Mode**: toestanden/attributen (gezondheid, locatie, kleur, gemoedstoestand, adres); vereist drager
- **Quantity**: eenheden/meetbaarheid (€/%/km/kg; aantal, duur, percentage)
...
### Feature‑extractie Pipeline (Grondig & Volledig)
- **Linguïstisch** (alle features extraheren):
  - POS‑tags met spaCy nl_core_news_lg model (fallback: regex patterns)
...
- **Semantisch** (diepgaande analyse):
  ...
  - Complete deelnemers detectie (niet alleen ≥2)
...
- **Domeinwoordenlijsten** (alles in geheugen voor snelle toegang):
  ...
  - Rijke metadata per term (domein, frequentie, ambiguïteit)
...
with st.spinner("🔍 Grondige UFO-analyse bezig (alle 16 categorieën)..."):
    suggestion = classifier.classify(term, definition)

st.info(f"🤖 Voorgestelde categorie: {suggestion.primary_category} ({suggestion.confidence:.0%})")

# Altijd uitgebreide uitleg tonen voor juridische verantwoording
with st.expander("📋 Volledige analyse", expanded=True):
    st.write("**Alle overwogen categorieën:**")
...
Score berekening: `score[cat] = Σ(weight × match) - conflicts`
...
def test_all_16_categories():
    """Test dat ALLE 16 UFO categorieën correct gedetecteerd worden."""
    ...
    for term, definition, expected_category in test_cases:
        ...
        assert len(result.all_scores) == 16  # Alle categorieën overwogen
...
|--------|--------|------------|
| **Precisie alle categorieën** | ≥95% | KRITIEK |
| **Disambiguatie correctheid** | ≥90% | KRITIEK |
...
- **Precisie**: ≥95% voor alle categorieën (geen shortcuts)
- **Disambiguatie**: ≥90% correct voor ambigue termen
...
- **Leercurve**: Gebruiker leert UFO-categorieën via uitgebreide uitleg
...
- **Missende categorieën**: Alle 16 UFO categorieën geïmplementeerd
...
- [ ] 500+ Nederlandse juridische termen geïmplementeerd
- [ ] Initiële classificatie van 80+ bestaande definities
