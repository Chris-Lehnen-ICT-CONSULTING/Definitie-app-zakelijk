...
class TestUFOClassifierCorrectness:
    ...
    def test_definitions(self) -> List[Tuple[str, str, UFOCategory]]:
        ...
        return [
            ...
            ("voorlopig",
             "Stadium dat nog niet definitief is",
             UFOCategory.PHASE),

            ("gesloten",
             "Status van een dossier dat is afgerond",
             UFOCategory.PHASE),

            ("actief",
             "Toestand waarin iets operationeel is",
             UFOCategory.PHASE),

            ("concept",
             "Ontwerpfase van een document",
             UFOCategory.PHASE),
            ...
            ("adres",
             "Aanduiding van de woonplaats van een persoon",
             UFOCategory.MODE),
    ...
    def test_all_16_categories(self, classifier, test_definitions):
        ...
        if failures:
            ...
            for f in failures[:5]:  # Toon eerste 5
                print(f"  - {f['term']}: {f['expected']} → {f['actual']} ({f['confidence']:.2%})")

        # Assert 95% precisie target
        assert accuracy >= 0.95, f"Precisie {accuracy:.1%} < 95% target. Failures: {len(failures)}/{total}"
    ...
    def test_disambiguation_zaak(self, classifier):
        ...
        result1 = classifier.classify(
            "rechtszaak",
            "Een strafzaak die voor de rechter wordt behandeld"
        )
        ...
        result2 = classifier.classify(
            "zaak",
            "Een roerende zaak zoals een auto of fiets"
        )
        assert result2.primary_category == UFOCategory.KIND

        # Check dat disambiguatie is toegepast
        assert len(result1.disambiguation_notes) > 0 or len(result2.disambiguation_notes) > 0
    ...
    def test_disambiguation_huwelijk(self, classifier):
        ...
        result1 = classifier.classify(
            "huwelijkssluiting",
            "Het voltrekken van een huwelijk door de ambtenaar"
        )
        assert result1.primary_category == UFOCategory.EVENT
        ...
        result2 = classifier.classify(
            "huwelijk",
            "De juridische band tussen twee gehuwde personen"
        )
    ...
    def test_disambiguation_overeenkomst(self, classifier):
        ...
        result1 = classifier.classify(
            "contractsluiting",
            "Het aangaan van een overeenkomst tussen partijen"
        )
        assert result1.primary_category == UFOCategory.EVENT
    ...
    def test_all_matched_patterns_reported(self, classifier):
        ...
        assert len(pattern_types) >= 2, "Verschillende pattern types moeten worden gevonden"
    ...
    def test_secondary_categories_identified(self, classifier):
        ...
        assert UFOCategory.KIND in result.secondary_categories or \
               UFOCategory.ROLE in [result.primary_category] + result.secondary_categories
    ...
    def test_legal_domain_recognition(self, classifier):
        ...
        result2 = classifier.classify(
            "beschikking",
            "Een besluit van een bestuursorgaan gericht op rechtsgevolg"
        )
    ...
    def test_collection_categories(self, classifier):
        ...
        result1 = classifier.classify(
            "rechtbank",
            "Een groep rechters die gezamenlijk rechtspreken"
        )
        ...
        result2 = classifier.classify(
            "college",
            "Een vast aantal leden dat gezamenlijk bestuurt"
        )
        assert result2.primary_category in [UFOCategory.COLLECTIVE, UFOCategory.FIXEDCOLLECTION]
    ...
    def test_legal_vocabulary_coverage(self, classifier):
        ...
        assert len(all_terms) >= 500, f"Slechts {len(all_terms)} termen geladen, minimaal 500 vereist"
...
class TestPatternMatcher:
    ...
    def test_disambiguation_rules_complete(self, matcher):
        ...
        expected_terms = ['zaak', 'huwelijk', 'overeenkomst', 'procedure', 'vergunning', 'besluit']
...
class TestAccuracyBenchmark:
    ...
    def benchmark_set(self) -> List[Tuple[str, str, UFOCategory]]:
        ...
        return [
            ...
            # Strafrecht cases
            ("aangifte", "Melding bij politie van een strafbaar feit", UFOCategory.EVENT),
            ("voorarrest", "Fase waarin verdachte in hechtenis zit voor het proces", UFOCategory.PHASE),
            ("strafblad", "Document met justitiële antecedenten van een persoon", UFOCategory.KIND),

            # Bestuursrecht cases
            ("bezwaarschrift", "Document waarmee bezwaar wordt gemaakt tegen een besluit", UFOCategory.KIND),
            ("handhaving", "Het afdwingen van naleving van regels door het bestuur", UFOCategory.EVENT),
            ("gedoogbeschikking", "Besluit waarbij wordt afgezien van handhaving", UFOCategory.RELATOR),

            # Civiel recht cases
            ("hypotheek", "Zekerheidsrecht op onroerend goed voor een geldlening", UFOCategory.RELATOR),
            ("erfenis", "Vermogen dat overgaat bij overlijden", UFOCategory.KIND),
            ("vruchtgebruik", "Recht om goederen van een ander te gebruiken", UFOCategory.RELATOR),
