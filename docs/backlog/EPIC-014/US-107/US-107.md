# US-107: Implementeer FeedbackBuilder Priority Algorithm
...
## Acceptance Criteria
- [ ] Implementeer 5-staps prioritering: Kritiek → Groepering → Type-specifiek → Leren → Versterking
...
- [ ] Emoji gebruik voor prioriteit indicatie (🚨 voor kritiek, 💡 voor suggesties)
...
def prioritize_feedback(feedback_items):
    # 1. Critical items met 🚨
    critical = [f for f in feedback if f.startswith("🚨")]
    
    # 2. Concrete suggesties met 💡
    suggestions = [f for f in feedback if "💡" in f or "Voeg toe:" in f or "Vervang" in f]
...
## Test Cases
1. Test met 10+ violations → max 5 feedback items
2. Test met 5 critical violations → max 3 critical in output
