...
1. **draft** - Initiële status na generatie
2. **review** - Na "Submit voor Review" (werkt al)
3. **established** - Na goedkeuring door expert (ONTBREEKT IN UI)
...
- [ ] Prefill toont alle context (organisatorisch, juridisch, wettelijke basis) read‑only
- [ ] Expert Review tab toont "Vaststellen" knop voor definities met status "In review"
- [ ] Expert Review tab toont "Afwijzen" knop met verplicht reden‑veld
- [ ] Na vaststellen wordt status "Vastgesteld" (DB: established)
- [ ] Na afwijzen wordt status terug naar "Concept" (DB: draft) met feedback/reden
...
- [ ] Voor definities met status "Vastgesteld" is er een actie "Maak bewerkbaar" (status terugzetten) met verplicht reden‑veld en logging
...
- [ ] Alle UI‑labels in het Nederlands; DB‑statuscodes blijven ongewijzigd
- [ ] Approval workflow met confirmatie dialoog
- [ ] Afwijzen verplicht een reden
- [ ] Optioneel: batch vaststellen voor meerdere definities tegelijk
- [ ] Filter op status in overzichten; "Vastgesteld" zichtbaar/selecteerbaar
...
- Het systeem bevat voorbeelddata in `schema.sql` met één definitie die direct als "established" is ingevoerd (demonstratiedoeleinden).
...
### Terminologie (NL labels in UI; DB‑codes blijven)
- draft → "Concept"
- review → "In review"
...
def _render_approval_actions(self, definitie: DefinitieRecord):
    ...
    with col2:
        if st.button("❌ Afwijzen", key=f"reject_{definitie.id}"):
...
def _approve_definition(self, definition_id: int, notes: str = ""):
    ...
    if result.success:
        st.success(f"✅ Definitie goedgekeurd (status: {result.new_status})")
        st.rerun()
    else:
        st.error(f"❌ Goedkeuring mislukt: {result.error_message}")
```

### 1b. Prefill + read‑only context
- Generator zet `selected_review_definition_id` na succesvol bewaren/genereren.
- Expert‑tab toont bovenaan (indien aanwezig) een read‑only contextpaneel met ALLEEN de vastgelegde context:
  - Begrip, definitie (samenvatting)
  - Organisatorische context (leeg = “—”)
  - Juridische context (leeg = “—”)
  - Wettelijke basis (lijst; leeg = “—”)
...
def _render_status_badge(self, status: str):
    ...
    icon, color = badges.get(status, ('❓', 'gray'))
...
### 4. Status terugzetten (Vastgesteld → Concept)
- In Expert‑tab bij status Vastgesteld: actie "🔓 Maak bewerkbaar" met verplicht reden‑veld.
...
- Edit‑tab wordt weer bewerkbaar.

### 5. Zoeken op status (incl. Vastgesteld)
- Zoekfunctie behoudt statusfilter en toont ook "Vastgesteld".
- Resultaten voor Vastgesteld openen in Expert‑tab (read‑only); via actie "Maak bewerkbaar" kan status worden teruggezet (met logging) om daarna te bewerken in de Bewerk‑tab.
...
- **Vastgesteld** (established): Read‑only; via Expert‑tab kan de status expliciet worden teruggezet
- **Gearchiveerd** (archived): Read‑only
...
1. **Happy Path**
   ...
   - Prefill toont alle context read‑only
   - Vaststellen → status wordt Vastgesteld
...
3. **Edit Protection**
   ...
   - Probeer te bewerken in Bewerk‑tab → read‑only met duidelijke melding
   - Expert‑tab: "Maak bewerkbaar" met verplicht reden‑veld
   - Na terugzetten status = Concept → Bewerk‑tab weer bewerkbaar

4. **Zoek op status**
   - Zoekfilter op "Vastgesteld" toont alle vastgestelde definities (incl. sample uit schema)
   - Open één record → read‑only; status kan via Expert‑tab teruggezet worden (met logging)
...
- [ ] Prefill toont laatst gegenereerde definitie met alle context (read‑only)
- [ ] Filter op status (incl. Vastgesteld) werkt
- [ ] Edit bescherming voor Vastgesteld (read‑only) + expliciet terugzetten mogelijk
- [ ] Tests voor approval workflow
- [ ] Documentatie bijgewerkt

## Dependencies

- **US-072**: DefinitionWorkflowService (✅ Completed)
- **US-064**: Definition Edit Interface (✅ Completed)  
