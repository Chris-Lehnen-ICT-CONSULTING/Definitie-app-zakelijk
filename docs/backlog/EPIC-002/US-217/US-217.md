---
id: US-217
titel: "US-217: Standaardiseer error handling door de hele applicatie met consistente patterns, duidelijke error messages en proper e..."
epic: EPIC-002
status: open
prioriteit: MEDIUM
story_points: 5
...
# US-217 — Implementeer Consistente Error Handling Patterns
...
- Creëer custom exception hierarchy
...
def display_error(error: Exception) -> None:
    ...
    if isinstance(error, DefinitieAppException):
        ...
        if error.code == ErrorCode.RATE_LIMIT_EXCEEDED:
            st.info("💡 Probeer het over enkele minuten opnieuw")
        elif error.code == ErrorCode.SERVICE_UNAVAILABLE:
            st.info("💡 De service is tijdelijk niet beschikbaar")
