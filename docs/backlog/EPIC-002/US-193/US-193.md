...
## Voortgang
- Gereed:
  - UI-detailpaneel bouwt “📋 Alle Toetsregels Resultaten” nu af uit V2‑resultaat (geen legacy validator nodig voor deze weergave).
  - V2‑violations bevatten nu ook `description` (mirror van `message`) zodat UI consistent tekst toont.
  - Tests toegevoegd voor description‑veld en initiële goldens (INT‑01/CON‑01).
- Nog te doen:
  - Legacy aanroep in `tabbed_interface.py` volledig verwijderen en overal V2 als enige bron hanteren.
  - Uitbreiden van goldens voor volledige regelscope (afhankelijk van US‑192).
