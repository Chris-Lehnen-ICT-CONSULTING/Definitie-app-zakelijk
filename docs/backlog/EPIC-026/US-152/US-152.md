...
id: US-152
epic: EPIC-026
titel: "US-152: titel: Refactor FeedbackBuilder naar moderne service"
status: open
prioriteit: P2
story_points: 5
...
# US-152: Refactor FeedbackBuilder naar moderne service
...
## Acceptance Criteria
- [ ] FeedbackServiceV2 geïmplementeerd met alle legacy functionaliteit
- [ ] Minimaal 11 validation rules hebben feedback mappings (Fase 1); uitbreiden via mapping‑config richting 45
- [ ] Prioritering algoritme identiek aan legacy (Kritiek → Suggesties → Overig)
...
- [ ] Single import: geïmporteerde definitie levert deterministische, geprioriteerde top‑5 feedback
- [ ] Bulk import: CSV/Excel per definitie (score + top‑3 feedback) en samenvattingsmetrics (acceptatie‑ratio, top violations, p95 duur)
- [ ] Test coverage >95%
...
```
ValidationService (bestaand) → [Technische Output] → FeedbackServiceV2 (nieuw) → [Menselijke Feedback]
```
...
### Integratie Strategie (DefinitionOrchestratorV2, post‑validatie – Aanbevolen)
```python
# Schematisch, async V2 voorbeeld
...
1. **Feedback Prioritering**
   ...
   - Volgorde: Critical → High → Medium → Low

2. **Iteratie Strategieën** 
...
## Implementation Plan
📄 **[Gedetailleerd Implementation Plan](./IMPLEMENTATION_PLAN.md)**

## Dependencies

### Direct Dependencies
- ValidationOrchestratorV2 moet violations kunnen leveren (`services/orchestrators/validation_orchestrator_v2.py`)
- ServiceContainer voor dependency injection  
- Validation interfaces voor data types (`services/validation/interfaces.py`)
- Minimaal 11 validation rules voor eerste mappings; mapping‑config voor uitbreiding
...
**⚠️ Kritiek:** US-064 (Edit Interface) bepaalt of feedback van informatief naar actionable gaat
...
## Definition of Done
- [ ] Code geïmplementeerd volgens V2 patterns
- [ ] Alle tests groen
...
- [EPIC-012 Overview](../../EPIC-012/EPIC-012.md)
