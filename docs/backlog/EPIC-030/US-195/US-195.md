---
id: US-195
titel: "US-195: title: Duplicate‑gate bij genereren definitie met 3‑contextkeuze"
epic: EPIC-030
...
### Doelen
- Duplicate-detectie vóór API-call met begrip + drie contextlijsten.
- Heldere keuze-UI die hergebruik stimuleert maar geforceerde generatie mogelijk maakt.
- Gedeelde normalisatie-implementatie tussen database, services en validator.

### Buiten scope
- Geen uitbreiding van businessregels (categorie blijft buiten duplicate).
- Geen versiebeheer- of audittrailwijzigingen; notificatie volstaat.
- Geen introductie van background jobs of queues.

### Definition of Ready
- [ ] UX-schets (wireframe of screenshot) van de dialoog is beschikbaar.
- [ ] Lijst van bestaande doublures (min. 5 voorbeelden) aangeleverd voor testdata.
- [ ] Besluit bevestigd dat geforceerde generatie zichtbaar blijft met CON-01 violation.
- [ ] Database backup/restore procedure ready voor migratie.

### Werkpakketten
- [ ] Normalisatie helper + migratiescript (`scripts/migrations/normalize_wettelijke_basis.py`) met unit tests.
- [ ] Repository/service update voor orde-onafhankelijke duplicate check.
- [ ] UI dialoog en state management in `definition_generator_tab` inclusief disabling van generate-knop.
- [ ] Validator-aanpassing voor CON-01 metadata (`existing_definition_id`, `status`) + logging.
- [ ] Observability: metric `duplicate_prevented_total` en structured log entries voor beslissingen.

## Definitie van Gereed
- [ ] Dialoog verschijnt vóórdat de OpenAI-call wordt aangeroepen.
- [ ] Bestaande definitie wordt leesbaar weergegeven (tekst + 3 contexten + status).
- [ ] Geforceerde generatie registreert CON-01 violation met metadata en logging.
- [ ] Duplicate matching blijft identiek bij permutaties van `wettelijke_basis` (unit + integratie test).
- [ ] Migratiescript is uitgevoerd en idempotent (tweede run levert 0 wijzigingen).
- [ ] Telemetry toont teller `duplicate_prevented_total` en sample log met hergebruikbeslissing.
