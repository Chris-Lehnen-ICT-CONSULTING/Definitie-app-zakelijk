...
class UFOClassifier:
    ...
    PATTERNS = {
        UFOCategory.KIND: re.compile(
            ...
            r"zaak|dossier|proces-verbaal|rapport|verslag)\b",
            re.IGNORECASE,
        ),
        UFOCategory.EVENT: re.compile(
            ...
            r"start|begin|einde|afloop|voltooiing)\b",
            re.IGNORECASE,
        ),
        UFOCategory.ROLE: re.compile(
            ...
            r"voogd|ouder|kind|echtgenoot|partner)\b",
            re.IGNORECASE,
        ),
        UFOCategory.PHASE: re.compile(
            ...
            r"minnelijk|gerechtelijk|buitengerechtelijk)\b",
            re.IGNORECASE,
        ),
        UFOCategory.RELATOR: re.compile(
            ...
            r"volmacht|machtiging|toestemming|instemming)\b",
            re.IGNORECASE,
        ),
        UFOCategory.MODE: re.compile(
            ...
            r"geloof|overtuiging|mening|standpunt)\b",
            re.IGNORECASE,
        ),
        UFOCategory.QUANTITY: re.compile(
            ...
            r"euro|gulden|dollar|valuta|munt|\d+)\b",
            re.IGNORECASE,
        ),
        ...
        UFOCategory.COLLECTIVE: re.compile(
            ...
            r"gemeenschap|samenleving|maatschappij)\b",
            re.IGNORECASE,
        ),
    }
    ...
    def classify(self, term: str, definition: str) -> UFOResult:
        ...
        if not term or not definition:
            return UFOResult(term or "", UFOCategory.UNKNOWN, 0.1, "Ongeldige invoer")
        ...
        if not scores:
            return UFOResult(term, UFOCategory.UNKNOWN, 0.3, "Geen patterns gevonden")
        ...
        if len(scores) > 1:
            others = [
                f"{c.value}" for c, s in scores.items() if c != best_cat and s > 0.2
            ]
            if others:
