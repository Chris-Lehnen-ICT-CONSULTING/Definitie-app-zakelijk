...
class SAM02Validator:
    ...
    def validate(
        ...
        for fout in self.foute_voorbeelden:
            if fout.lower() in tekst_lc:
                return (
                    False,
                    f"❌ {self.id}: vage kwantoren aangetroffen (fout voorbeeld)",
                    0.0,
                )
        ...
        for goed in self.goede_voorbeelden:
            if goed.lower() in tekst_lc:
                return (
                    True,
                    f"✔️ {self.id}: concrete aantallen gebruikt (goed voorbeeld)",
                    1.0,
                )
    ...
    def get_generation_hints(self) -> list[str]:
        ...
        hints.append(
            "Als exacte aantallen niet mogelijk zijn, gebruik dan 'één of meer'"
        )

        return hints


def create_validator(config_path: str | None = None) -> SAM02Validator:
    """
