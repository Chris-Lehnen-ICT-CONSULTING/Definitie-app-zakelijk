...
class ESS05Validator:
    ...
    def validate(
        ...
        if gevonden:
            ...
            return (
                True,
                f"✔️ {self.id}: onderscheidende patroon(en) herkend ({labels})",
                1.0,
            )
    ...
    def get_generation_hints(self) -> list[str]:
        ...
        hints.append(
            "Maak expliciet waarin het begrip zich onderscheidt van verwante begrippen"
        )
        hints.append(
            "Gebruik formuleringen zoals: 'in tegenstelling tot', 'verschilt van', 'specifiek voor'"
        )
        ...
        if self.goede_voorbeelden:
            hints.append(
                f"Voorbeeld van goede onderscheiding: {self.goede_voorbeelden[0]}"
            )

        return hints


def create_validator(config_path: str | None = None) -> ESS05Validator:
    """
