...
class ValidationStatusChecker:
    ...
    async def check_modular_validation_service(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await service.validate_definition("test", "Dit is een test definitie voor validatie")
            
            return {
                "status": "working",
                "message": "Service operational",
                "test_score": result.get("overall_score", 0),
                "rules_tested": len(result.get("passed_rules", [])) + len(result.get("violations", [])),
    ...
    def generate_status_report(self) -> str:
        ...
        report.append(f"Overall Health: {self.status_data.get('overall_health', 'Unknown').upper()}")
...
async def main():
    ...
    print(f"   - validation-status.json (machine readable)")
    print(f"   - validation-status-report.txt (human readable)")
